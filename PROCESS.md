...
# [Scoring] — Auto-chấm câu short_answer (PHẦN III) qua `/recipe-fullstack-implement` đầy đủ (S#44, 2026-08-02, branch `feat/rating-system`)

## Yêu cầu
Engineer: "logic module chấm câu tự luận cho đề" — qua slash-command `/recipe-fullstack-implement`, chạy full quy trình orchestration (requirement-analyzer → codebase-analyzer×2 + ui-analyzer → UI Spec → 2 Design Doc (backend/frontend) → code-verifier → document-reviewer → design-sync → acceptance-test-generator → work-planner → task-decomposer → 9 task tự động thực thi). Làm rõ phạm vi qua AskUserQuestion trước khi vào design: chỉ `short_answer` (PHẦN III, đã có ô nhập text sẵn trong player), KHÔNG bao gồm `essay` (chưa có UI nhập liệu, để dành tương lai); quy tắc so khớp = chuẩn hoá text + tương đương số học (`"1,04"` = `"1.04"` = `"1.040"`); không backfill bài đã nộp trước đó.

## Doc chain
`docs/ui-spec/short-answer-scoring-ui-spec.md` → `docs/design/short-answer-scoring-backend-design.md` (v1.1, 1 vòng revision sau code-verifier phát hiện 1 test cũ `topicBreakdown` sẽ vỡ) → `docs/design/short-answer-scoring-frontend-design.md` → `docs/plans/20260802-feature-short-answer-scoring.md` (2 phase + Final Phase QA, 9 task file — đã xoá theo quy ước "docs/plans/tasks/ ephemeral" sau khi hoàn tất, work plan giữ lại).

## Việc đã làm (backend trước, frontend sau, đúng thứ tự Work Plan)
1. **`computeScore.ts`**: thêm `isShortAnswerCorrect`/`parseShortAnswerNumber`/`normalizeShortAnswerText` (private, cùng file với `isScored()`/test — đúng precedent `isTrueFalseCorrect`), numeric-equivalence-first (so khớp số sau khi unify dấu phẩy/chấm thập phân, từ chối khi có nhiều dấu phân cách mơ hồ) + fallback normalized-text. `isScored()` thêm guard `short_answer` (chỉ chấm khi có `essayAnswer` ground truth, thiếu → `scored:false`, không phạt oan).
2. **`actions.ts`**: `submitExam()` thiếu hẳn `essay_answer` trong SELECT (bug có sẵn, chưa từng phát hiện) — bổ sung select + mapping `essayAnswer`, cùng 1 commit với bước 1 (tránh cửa sổ "silent no-op": logic chấm đúng nhưng không có dữ liệu để chấm).
3. **Bug phát hiện giữa chừng**: `computeScore.test.ts`'s helper `shortAnswer(id, topic, essayAnswer = "1260")` có default-parameter — JS thay default bất cứ khi nào argument resolve về `undefined`, **kể cả khi gọi tường minh `undefined`** — khiến 2 test case "không có ground truth" âm thầm nhận `"1260"` thay vì `undefined`. Fix: bỏ default, buộc mọi call site truyền tường minh.
4. **`result/detail/page.tsx`**: nhánh "scored" trước đây chỉ render `q.choices` (MCQ-only) — thêm nhánh `short_answer` hiển thị đáp án đã nộp + đáp án đúng (đọc `q.essayAnswer`, KHÔNG đọc `r.correct` vì field này chỉ dành ChoiceId/mcq), tái dùng màu fern/destructive có sẵn.
5. **`QuestionRenderer.tsx`**: sửa caption short_answer "not auto-scored yet" → "auto-scored after you submit" (essay giữ nguyên, ngoài phạm vi).
6. **Test**: `submitExam.int.test.ts` (mock-boundary Supabase, spy call-through `computeScore` không stub), `QuestionRenderer.test.tsx` (RTL, không mock), fixture-e2e driver `short-answer-scoring.fixture.e2e.test.ts` (Playwright-MCP thủ công, theo đúng convention `rating`/`history` fixture-e2e) — cả 2 vòng review integration-test-reviewer đều có 1 lần `needs_revision` (thiếu annotation Behavior/ROI chuẩn; 1 assertion sai — check "không có màu fern nào trong card essay" nhưng card essay THẬT SỰ có `text-[#4F7942]` cho dòng "Stored answer" từ code có sẵn, phải thu hẹp lại đúng phần tử mang màu correctness).

## Verify & kết quả
323/323 vitest pass, `tsc --noEmit` sạch, `eslint` sạch trên mọi file chạm tới, `next build` qua. Post-implementation: `code-verifier` backend 90/100 (2 lỗi nhỏ: doc quên cập nhật signature helper sau khi fix bug default-param; SA-BE-011 đúng theo code nhưng chưa có unit test riêng cho case dương short_answer trong topicBreakdown — không chặn), `code-verifier` frontend 100/100 (0 discrepancy). 6 commit trên `feat/rating-system`: `e9d14c6`(docs) `9ff81f9`(RED) `3ad229d`(GREEN) `1e680af`(int test) `ddbbdc2`(doc fix) `9887bbe`(frontend render) `6b1365e`(fixture-e2e) `377587e`(footnote) `f572b71`(test) `05fe5dc`(doc citation).

## Phát hiện ngoài phạm vi — `security-reviewer` trả về BLOCKED, engineer quyết định ghi nợ chứ không fix ngay
RLS trên `public.questions` chỉ lọc theo hàng (ai xem được đề nào), KHÔNG có bảo vệ cấp cột — `essay_answer`/`correct_answer`/`sub_answers` đều có thể bị bất kỳ user đã đăng nhập nào đọc trực tiếp qua Supabase REST API (anon key + token cá nhân, đều lộ sẵn trên browser), bỏ qua hoàn toàn ranh giới `PublicQuestion` (chỉ là quy ước tầng code, không phải chặn ở DB). Lỗ hổng có từ MVP (`correct_answer`) và từ true_false (`sub_answers`) — feature này chỉ thêm `essay_answer` vào đúng nhóm đã hở, KHÔNG làm xấu đi về kiến trúc. Engineer chọn "ghi nhận nợ kỹ thuật, hoàn tất feature ngay" thay vì dừng lại fix RLS (fix thật cần REVOKE cột nhạy cảm khỏi role `authenticated` hoặc chuyển qua RPC `SECURITY DEFINER`, đụng tới toàn bộ bảng `questions` chứ không riêng feature này).

## CÒN NỢ / lưu ý cho phiên sau
- **[ƯU TIÊN CAO, bảo mật]** RLS column-level gap ở trên — `correct_answer`/`sub_answers`/`essay_answer` đều lộ được qua REST API trực tiếp. Cần 1 phiên riêng để thiết kế fix (REVOKE cột hoặc RPC `SECURITY DEFINER`), không lẫn vào feature khác.
...
## CÒN NỢ / lưu ý cho phiên sau
- ~~**[CHẶN] Migration §15 CHƯA ÁP lên DB prod**~~ — **ĐÃ XONG cùng phiên.** Agent không có management token lẫn DB password nên không chạy được DDL; engineer tự paste khối alter §15 vào Supabase prod SQL Editor.
- ~~**Đề test hỏng còn nằm trong DB prod**~~ — **ĐÃ XOÁ cùng phiên, và đó chính là cách verify bản vá.** Xoá `KIEM TRA THU NGHIEM BBOX` bằng đúng nút Delete trên `/me/exams` (đề đang `published`, có 2 attempt — đúng điều kiện làm bug tái hiện trước đó). Kiểm ở TẦNG DB chứ không tin UI: `exams` 0 · `questions` 0 · `exam_attempts` 0 · `attempt_answers` 0 · `exam_results` 0 · cả 2 bucket storage 0. Cascade chạy đủ chuỗi `exams → exam_attempts → attempt_answers/exam_results`. **Bug xoá đề coi như đóng, đã chứng thực end-to-end trên production.**
