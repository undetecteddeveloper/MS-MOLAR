...
| 1.3 | 2026-09-18 | **Partially superseded for `/exams` by `docs/prd/exam-shelves-prd.md`** (Kho đề theo kệ). **AC-002** ("zero visual change") is superseded — the shelves are a deliberate visual change to that page. **AC-024** ("cold start is visually indistinguishable") is superseded — the "Các môn cần luyện" shelf renders only for students with submitted history. **D1** ("not a widget; no card or slot above the grid") is superseded for the bare URL — the shelves are the browse surface, and the ranking survives inside them. **D7's withdrawal** ("no reason label of any kind") is superseded — shelf subtitles and the `Hot nhất` ribbon re-adopt labelling against an approved visual spec. **AC-016, AC-017 and AC-037 are explicitly unchanged**, as is every other AC for the flat-grid path. **0 ACs renumbered, 0 removed, 0 weakened** — see the block immediately below this table. |
...
## Superseded for `/exams` by `docs/prd/exam-shelves-prd.md` (added in v1.3, 2026-09-18)

*(Scope of this block: the **bare `/exams` view only**. Nothing is renumbered, removed or weakened; each item stays binding wherever the shelves do not render — that is, on every URL carrying a filter, a keyword, a sort, a direction or a page.)*

| Item | Superseded because |
|---|---|
| **AC-002** — "zero visual change" | The shelves feature is a deliberate visual change to `/exams`: three shelf sections, a new `ExamShelf` component, an optional ribbon slot on `ExamCard`, and new Vietnamese strings |
| **AC-024** — "a cold-start page is visually indistinguishable from any other student's" | The "Các môn cần luyện" shelf renders exactly when the student has submitted history, so the two states now differ on purpose |
| **D1** — "the ranking IS the browse order; no separate recommendation card or slot above the grid" | For the bare URL the shelves **are** the browse surface; the personalised ranking survives inside them and unchanged in the flat grid |
| **D7 (withdrawn)** — "no reason label of any kind" | Shelf subtitles and the `Hot nhất` ribbon are the labelling D7 withdrew, re-adopted against an approved visual spec |

**Unchanged and still binding, restated so no reader infers otherwise**: **AC-016**, **AC-017** and **AC-037** — an explicit `?sort=` still produces exactly today's SQL ordering with no personalisation, `?dir` with a `?sort` is unchanged, `?dir` without one still falls to the personalised order, and the query-construction assertions in `SOURCE/features/exams/__tests__/rating.int.test.ts:317-440` keep passing **unmodified**. Every other AC of this document continues to bind the flat-grid path, which the shelves feature does not modify.
...
Layer 2's browse surface today has no ordering worth the name. With no `?sort` in the URL, `listExams` ends with `.order("id")` (`SOURCE/features/exams/queries.ts:136-138`) — stable, reproducible, and semantically empty. A student who opens `/exams` gets the catalogue in primary-key order, which is to say in upload order, which is to say in an order chosen by nobody for nobody.

Everything needed to do better already exists and is already loaded on that page:

- **The page is already per-user and per-request.** `SOURCE/lib/supabase/server.ts:14` awaits `cookies()`, so every `/exams` render is dynamic; `SOURCE/app/(exams)/exams/page.tsx:57-62` already awaits `listMySubmittedExamIds()` and `getCurrentUser()` in the same `Promise.all` that fetches the exams. There is no `dynamic`/`revalidate` export on this route, no `use cache`, no `unstable_cache` anywhere in `SOURCE`, and no PPR in `next.config.ts`. **Personalisation costs no caching, because there is no caching to lose.**
...
- [ ] **R1 — Personalised ranking is the browse order, and nothing else changes** (D1, D7-withdrawn): with no `?sort` in the URL, `/exams` renders in personalised order instead of today's `.order("id")` (`SOURCE/features/exams/queries.ts:136-138`). No new card, no new slot, no new page, **no new pixel**.
...
- [ ] **R4 — Filters narrow; an explicit sort wins** (D3).
  ...
  - AC-037 *(new in v1.2 — the gap review finding **I005** found)*: Given `?dir` with **no** `?sort`, when the list renders, then the **personalised order applies**. D3 stands personalisation down for an explicit `?sort=` only, and a direction with no axis to apply it to is not a statement about order. This **replaces** today's behaviour, where `listExams({ dir: "asc" })` falls back to `.order("id")` — pinned at `SOURCE/features/exams/__tests__/rating.int.test.ts:432-441`, whose title says so outright ("dir without sort is a no-op — falls back to .order(id) same as no filters at all"). That assertion stays green under ADR-0015's placement, because `listExams` keeps its `.order("id")` base order and the reordering happens above it; what changes is what the **page** renders, which is asserted separately.
...
- [ ] **R8 [v2 — deferred in v1.2]** — **Telemetry: which exam was ranked first** (D8). *(R7 was withdrawn in v1.1; its number is retired, not reused.)* **The whole requirement, AC-025–AC-029, is deferred to v2 together with the `schema.sql` change it needs** — see "Release partition" for the deferral reason and the condition that re-adopts it. The ACs below are preserved verbatim except for AC-025, which review finding **I004** required to be made decidable before it could be handed to anyone.
  ...
  - AC-028 **[v2]**: Given a telemetry write that fails (DB error or thrown exception), when it fails, then the page still renders — best-effort exactly as `recordRouteTelemetry()` does at `SOURCE/features/analytics/queries.ts:70-88`, swallowing both the returned `error` and a thrown exception. **0** ranking renders broken by a logging failure.
...
4. **Explicit sort untouched**: the `?sort=hardest|newest|oldest` and `?level` query-construction assertions in `SOURCE/features/exams/__tests__/rating.int.test.ts:317-440` pass **unmodified**. Measured by running that file with no edits to those cases (AC-016, AC-017).
5. **Default order contract is re-pinned, not silently dropped** *(rewritten in v1.2 — review finding **I005** found an undercount, and ADR-0015 then changed the remedy)*. Two facts v1.1 got wrong. **First, there are two no-sort `.order("id")` assertions, not one**: `SOURCE/features/exams/__tests__/rating.int.test.ts:390` for `listExams({})` (test at `:384`) and `:438` for `listExams({ dir: "asc" })` (test at `:432`, titled "dir without sort is a no-op"). They sit in different `describe` blocks (`:317` and `:395`), which is how one of them went uncounted. **Second, under ADR-0015 neither is replaced.** The ranking is composed *above* `listExams`, which keeps its `.order("id")` base order and stays observably identical — so both assertions **stay green, unmodified**, and their role changes from "the browse default" to "the base-fetch contract". The gate is therefore: **both assertions still pass**; **both have their comments re-scoped** so neither reads as the statement of the browse default; **1 new assertion** pins the new default contract on the composition function; **0** tests are deleted. R-d's failure mode (delete rather than replace under deadline pressure) is what this criterion exists to prevent, and the v1 placement removes most of its opportunity.
...
| `SOURCE/features/exams/__tests__/rating.int.test.ts` | changed — **additive**: a new assertion pinning the composition function's default order, plus the round-trip budget assertion (Decision 6). The two `.order("id")` assertions at `:390` and `:438` stay green with re-scoped comments (Success Criteria #5) | test |
...
- **`SOURCE/features/exams/queries.ts`** — `listExams` (`:99-142`) is the ordering site. Its `.eq("status","published")` (`:105`), its `?sort` branches (`:126-138`) and `EXAM_COLUMNS`/`toExam` (`:37-55`) are all load-bearing here.
- **`SOURCE/app/(exams)/exams/page.tsx`** — already resolves search params and already awaits `listMySubmittedExamIds()` and `getCurrentUser()` (`:57-62`). Whether the ranking hand-off and the telemetry write live here or inside `queries.ts` is U10.
...
- `SOURCE/features/exams/queries.ts` — `listExams` (`:99-142`), the `.order("id")` default (`:136-138`), `DEFAULT_ASCENDING` (`:71-75`), the published-only guard (`:105`), the no-FK note on `question_ids` (`:418-424`), `getResult()`'s embed through the frozen view (`:374-384`).
- `SOURCE/app/(exams)/exams/page.tsx` — search-param resolution, the existing per-user `Promise.all` (`:57-62`), the pass-through to `ExamBrowser` (`:112-116`), the unreachable `isLoggedIn` prop (`:115`).
- `SOURCE/features/exams/components/ExamBrowser.tsx` (`:20-42` the unsorted pass-through, `:49` the unreachable branch) — **not changed by this feature** (AC-002).
- `SOURCE/features/exams/components/ExamCard.tsx`, `ExamFilters.tsx` (`:63-67`, `:123-159`) — **not changed by this feature** (AC-002, AC-017).
...
- `SOURCE/features/analytics/queries.ts` — `getSkillRecommendation()` and the best-effort telemetry precedent (`:70-88`, `:136`), which U10 weighs.
- `SOURCE/features/analytics/components/SkillRecommendationCard.tsx` — the dead-end this feature routes out of; unchanged here.
...
- `SOURCE/features/exams/__tests__/rating.int.test.ts` (`:317-440`), `SOURCE/supabase/test-rls.ts` (`:405-408`, `:1521-1562`), `SOURCE/tests/e2e/fixture/rating.fixture.e2e.test.ts` (`:100-107`).
