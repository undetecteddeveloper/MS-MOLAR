...
| **Scale** | LARGE — fullstack. 31 file bị ảnh hưởng, cả backend lẫn frontend. Tầng thanh toán ĐẦU TIÊN của dự án: bảng mới trong `schema.sql` (đơn hàng + quyền lợi), route handler thứ HAI của toàn app (webhook payOS — hiện chỉ có `SOURCE/app/auth/callback/route.ts`), bộ đếm hạn mức mới trên Upstash Redis, sửa hai đường tiêu quota AI đang chạy production — **đường gia sư sửa ở `SOURCE/features/exams/tutorActions.ts`** (nơi `explainStep()` đã import `guard` ở `:36` và gọi nó ở `:175`) và **đường trích xuất UGC sửa ở `SOURCE/features/authoring/actions.ts` + `SOURCE/lib/ugc/gemini.ts`** — thay `LIMITS.MAX_UPLOADS_PER_DAY`, và hai trang công khai mới (Điều khoản, Chính sách hoàn tiền). **`SOURCE/lib/tutor/callTutor.ts` GIỮ NGUYÊN trách nhiệm hiện tại** ("gọi model và phân loại thất bại"); kiểm soát truy cập và hạn mức không đặt vào đó — xem Dependencies. DDL vẫn apply bằng tay trên hai project Supabase (TD-005). |

## Revision History

| Version | Date | Change |
|---|---|---|
| 1.6 | 2026-08-18 | **AC-057 đổi từ một trần PHẲNG sang một trần CÓ ĐIỀU KIỆN THEO BẬC — quyết định của chủ sở hữu dự án ngày 2026-08-18.** Bản 1.2–1.5 đòi `RATE_LIMITS.explainStep` nâng lên `≥ 50`/ngày cho mọi trường hợp; yêu cầu đó **phá chính cơ chế kiểm mà nó viện dẫn**: `SOURCE/lib/security/rateLimit.test.ts` khai `SUPPLIER_DAILY_QUOTA = 20` và khẳng định (a) mọi trần thuộc nhóm bị-nhà-cung-cấp-chặn đều `≤ 20`, (b) `explainStep.limit×1 + uploadExam.limit×3 ≤ 20` (hôm nay `3 + 5×3 = 18`) — một số 50 phẳng làm đỏ cả hai. Con số 20 đó là **trần của bậc MIỄN PHÍ**, thứ mà R14 gỡ bỏ, nên bất biến này phụ thuộc BẬC chứ không tuyệt đối. AC-057 nay đọc: `limit` suy ra từ `isPaidTierEnabled()` (`SOURCE/lib/billing/paidTier.ts:28`) — **bậc trả phí BẬT → ≥ 50/24h**, **TẮT → 3/24h (giữ nguyên)**, cửa sổ 24 giờ ở cả hai nhánh. Lý do chọn hướng này: (1) không phá assertion nào đang có — CI không đặt `GEMINI_PAID_TIER_ENABLED` nên test đọc đúng giá trị bậc miễn phí; (2) trần phẳng 50 mở một cửa sổ hỏng có thật từ lúc ship tới lúc R14 được bật, khi MỘT tài khoản tiêu được `50×1 + 5×3 = 65` request đối diện trần 20/ngày toàn dự án — đúng loại lỗi lệch đơn vị mà commit `e8d91a4` đã vá ngày 2026-08-16; (3) trần nâng đúng lúc gói cần tới nó tồn tại, vì cùng một biến gác cả nút mua (AC-049/AC-054). Chi phí chấp nhận: hằng số trong `rateLimit.ts` không còn là một số tĩnh duy nhất. Sửa đồng bộ ở 8 chỗ: AC-057, đoạn "Số phận của entry `explainStep`" trong R5, carve-out của AC-013, thân R14, mục Đơn giá, ô mitigation R-j, Dependencies (`rateLimit.ts`), References và Glossary (thêm mục "trần chống spam theo bậc"). **Ràng buộc triển khai không được chép vào đây** — chúng thuộc `docs/design/subscription-backend-design.md` v1.2 mục B-01, tài liệu đó sở hữu chúng. |
| 1.5 | 2026-08-18 | **Sửa một khẳng định sai về code, ở chỗ nó chặn một điều kiện mở bán.** Mục Dependencies viết `quotaTracker.ts` "đã ghi `usageMetadata` của mỗi lượt gọi" — thực tế `recordUsage()` **chưa từng được gọi ở đâu**, và bốn chỗ chạm `usageMetadata` đều chỉ nằm trên lối THẤT BẠI. U2 vì thế đang đứng trên một tiền đề sai: không có dữ liệu nào để đo. Đã sửa tại chỗ trong cùng phiên (tách `inputTokens`/`outputTokens` gồm cả `thoughtsTokenCount`, thêm vai trò `tutor`, nối `recordUsage()` vào cả bốn lối gọi trước mọi nhánh phân loại). Giới hạn còn lại — file tạm không sống qua nhiều instance trên Vercel — chuyển thành một hạng mục có DDL của Design Doc. |
| 1.4 | 2026-08-18 | **U1 resolved by engineer**: payOS xác nhận **không có sandbox** (kiểm trực tiếp, không qua điều kiện leo thang chờ-tới-ngày-Design-Doc). Theo đúng quy định của chính mục U1, hệ quả cố định: kiểm thử đầu-cuối webhook bắt buộc dùng **một giao dịch tiền thật giá trị nhỏ trên domain production**, cần engineer duyệt trước — đưa vào Work Plan như một hạng mục test, không phải một rủi ro còn bỏ ngỏ. Đồng thời `webhooks.confirm(url)` chỉ đăng ký được với domain production (Preview đổi URL mỗi lần build), nên mọi test trên Preview phải đi đường đối soát chủ động (R10) thay vì chờ webhook. Bảng "Ba điều kiện CHẶN việc mở bán" cập nhật dòng U1. |
...
**Một hệ quả không được bỏ qua**: 500 lượt/tháng cho **một** người Premium ≈ 16 lượt/ngày, trong khi key ở gói miễn phí chỉ có **20 lượt/ngày cho cả dự án**. Nghĩa là **gói miễn phí của Gemini không phục vụ nổi dù chỉ MỘT thuê bao Premium**. Vì thế "bật bậc trả phí trên project Gemini" là **điều kiện tiên quyết để bán gói đầu tiên**, không phải một việc dọn dẹp làm sau — xem R14. **Cùng phép so sánh đó là lý do trần chống spam theo người của `explainStep` cũng phải đi theo bậc thay vì đặt phẳng**: 50 lượt/ngày cho một người chỉ có nghĩa **sau** khi trần 20 lượt/ngày toàn dự án biến mất, nên nó được suy ra từ đúng biến `GEMINI_PAID_TIER_ENABLED` (AC-057), không phải viết cứng một lần cho cả hai thời kỳ.
...
- [ ] **R5 — Hạn mức gia sư theo KỲ 30 NGÀY**: Free **5 lượt/kỳ**, Premium **500 lượt/kỳ**. Cửa sổ là **đúng một kỳ 30 ngày chạy theo chu kỳ của người dùng** (A4 cho Premium, A6 cho Free) — **không** phải tháng dương lịch, và **không bao giờ** là giờ. **`RATE_LIMITS.explainStep` — hôm nay là `3` lượt/NGÀY/người, một trần TẠM đặt chung cho mọi người (`rateLimit.ts:137`, chú thích `:123–136`) — không còn là thứ quyết định quyền gọi gia sư.** Trần theo gói của R5 thay thế nó: sau feature này, hạn mức đọc từ gói của người dùng, không còn là một hằng số chung.
  ...
  **Số phận của entry `explainStep` — chốt để hai người triển khai không viết hai bộ test khác nhau: GIỮ NGUYÊN entry, KHÔNG xoá, nhưng đổi vai và đổi giá trị.** Vai mới là **trần chống spam theo người**, không phải hạn mức gói: `guard()` là lớp duy nhất chặn một vòng lặp tự động nện Server Action, và xoá entry đi là gỡ mất lớp đó cho đúng hành động tốn tiền nhất. Giá trị **có ĐIỀU KIỆN THEO BẬC Gemini, không phải một số phẳng** — quyết định của chủ sở hữu dự án ngày **2026-08-18**: `limit` được suy ra từ `isPaidTierEnabled()` (`SOURCE/lib/billing/paidTier.ts:28`), **bậc trả phí BẬT → ≥ 50 lượt/ngày**, **bậc trả phí TẮT → giữ nguyên 3 lượt/ngày**. Quy tắc chốt con số 50 vẫn như cũ và chỉ áp cho nhánh BẬT: trần chống spam ≥ **3 lần** mức ngày của gói rộng rãi nhất (500 lượt/kỳ ≈ **16,7 lượt/ngày**) — đủ cao để không bao giờ chạm phải người thật, đủ thấp để chặn vòng lặp máy. Nó không áp cho nhánh TẮT vì trước ngày R14 bật, gói Premium **chưa bán được** (AC-049/AC-054) nên chưa có ai cần tới 50, trong khi trần **20 request/ngày cho cả dự án** của bậc miễn phí thì vẫn còn nguyên hiệu lực. AC-057 ghim cả hai nhánh.
  - AC-013: Cho toàn bộ cấu hình hạn mức của mọi hành động có tiêu quota AI, khi rà, thì **0** cấu hình dùng cửa sổ theo giờ, và **0** cấu hình dùng cửa sổ theo ngày cho hạn mức gia sư/upload — cửa sổ duy nhất được chấp nhận cho hai hạn mức đó là kỳ 30 ngày của người dùng. Kiểm bằng một unit test trên bảng hằng, không phải bằng mắt. *(Hai thứ nằm NGOÀI phạm vi test này, mỗi thứ một lý do: trần ngân sách TOÀN DỰ ÁN của R7 vẫn theo NGÀY — đó là cửa sổ của nhà cung cấp, không phải hạn mức người dùng. Và entry chống spam `RATE_LIMITS.explainStep` vẫn theo NGÀY — ở **cả hai** nhánh bậc trả phí của AC-057 — nó không phải hạn mức gói, xem đoạn trên.)*
  - AC-057 *(sửa 2026-08-18 theo quyết định của chủ sở hữu dự án — trước đó ghi một trần PHẲNG ≥ 50)*: Cho `RATE_LIMITS.explainStep` sau khi R5 ship, khi rà, thì `windowMs` là **24 giờ ở cả hai nhánh**, còn `limit` **được suy ra từ `isPaidTierEnabled()`** (`SOURCE/lib/billing/paidTier.ts:28`, đọc `GEMINI_PAID_TIER_ENABLED`, mặc định fail-closed như AC-054) — **không** phải một hằng số phẳng:
    - **bậc trả phí BẬT** → `limit` ≥ **50**;
    - **bậc trả phí TẮT** → `limit` = **3**, đúng như hôm nay.

    Kiểm bằng **hai ca tách bạch** trên chính bộ test phân loại đã dựng ở commit `e8d91a4` (nhóm "bị nhà cung cấp chặn"): nhánh **TẮT** đi qua nguyên vẹn các bất biến đang có của nhóm đó — cửa sổ 24 giờ, `limit ≤ 20`, và tổng số request Gemini một tài khoản tiêu được trong ngày ≤ 20; nhánh **BẬT** chỉ khẳng định `limit ≥ 50` và cửa sổ 24 giờ, và **không** được áp trần 20 lên nó, vì 20 là **sự thật của bậc miễn phí** chứ không phải một bất biến vĩnh viễn (AC-025 đã ghi rằng trần này đổi tính chất vào đúng ngày R14 bật).

    **Vì sao có điều kiện chứ không phẳng — ba lý do, mỗi lý do kiểm được:**
    1. **Nhánh BẬT phải nâng**, nếu không thì một người Premium dùng 17 lượt trong một ngày — hành vi **bình thường** của gói họ đã trả tiền (500 lượt/kỳ ≈ 16,7 lượt/ngày) — bị chặn bởi một lớp mà chính họ đã mua quyền đi qua, và triệu chứng trông y hệt lỗi hết hạn mức.
    2. **Một trần phẳng 50 mở một cửa sổ hỏng có thật.** Giữa ngày thay đổi này ship và ngày R14 thật sự được bật, trần toàn dự án của bậc miễn phí vẫn là **20 request/ngày**, trong khi một trần phẳng 50 cho phép **MỘT** tài khoản tiêu `50×1 + 5×3 = 65` request Gemini. Đó đúng là loại lỗi lệch đơn vị mà commit `e8d91a4` đã vá ngày **2026-08-16** — một trần theo người đọc thì hợp lý khi đứng một mình, nhưng cộng lại thì vượt trần của nhà cung cấp — mở lại ở quy mô gấp ba.
    3. **Hai cổng đi cùng một công tắc.** Cùng `GEMINI_PAID_TIER_ENABLED` quyết định nút mua có bật hay không (AC-049/AC-054) và trần chống spam là 3 hay ≥ 50. Nên trần chỉ nâng đúng lúc — và chỉ khi — tồn tại một gói có mức dùng ngày cần tới nó. Ở nhánh TẮT không ai bị thiệt: gói Free chỉ có 5 lượt/kỳ, nên `3`/ngày không phải thứ chặn họ trước.

    **Chi phí được chấp nhận có ý thức**: hằng số một người rà đọc thấy trong `rateLimit.ts` **không còn là một con số tĩnh duy nhất**. Các ràng buộc triển khai đi kèm quyết định này — giữ nguyên byte-for-byte các assertion đang có, chỗ đặt phép suy ra, stub `server-only`, thời điểm đánh giá lúc nạp module, và ca tắt cờ sau khi đã có thuê bao — thuộc **Design Doc backend `docs/design/subscription-backend-design.md` v1.2, mục B-01**, và PRD **không** lặp lại chúng ở đây.
...
- [ ] **R6 — Hạn mức upload theo KỲ 30 NGÀY, và đường xử-lý-lại cũng phải bị đếm**: Free **3 lượt/kỳ**, Premium **15 lượt/kỳ**, cùng cửa sổ kỳ 30 ngày với R5 (A4/A6), thay cho `LIMITS.MAX_UPLOADS_PER_DAY = 30`. Đường **xử lý lại** một đề đã tồn tại phải tiêu suất như một lượt upload, vì nó vẫn gọi Gemini.
  ...
  - AC-019: Cho một người dùng bấm **xử lý lại** một đề đã có (nhánh `if (rerunExamId)` trong `extractAndAssemble` của `SOURCE/features/authoring/actions.ts`, bắt đầu ở `:248`), khi lượt đó chạy, thì nó tiêu một suất trong hạn mức kỳ — đóng đúng khoảng hở hiện tại, nơi nhánh này **không** đi qua bộ đếm (bộ đếm chỉ nằm trong nhánh `else` bắt đầu ở `:310`, khối đếm `:311–322`, câu lệnh chặn ở `:317`) nhưng vẫn gọi Gemini. *(Trích dẫn số dòng đúng tại thời điểm rà 2026-08-16; `actions.ts` dài hơn 700 dòng và sẽ trôi — khi đối chiếu, hãy tìm theo **tên ký hiệu** `rerunExamId`, `LIMITS.MAX_UPLOADS_PER_DAY` thay vì tin vào số dòng.)*
...
- [ ] **R7 — Trần ngân sách AI TOÀN DỰ ÁN theo NGÀY, có phần bảo lưu cho Premium, và FAIL-CLOSED**: Một bộ đếm theo ngày cho toàn bộ key Gemini, giữ trên Upstash Redis (hạ tầng đã có — `SOURCE/lib/security/rateLimitStore.ts`, `@upstash/redis`), được kiểm **trước** cả đường gia sư (trong `explainStep()` ở `SOURCE/features/exams/tutorActions.ts`, **trước** lời gọi `generateHint()` — không đặt bên trong `lib/tutor/callTutor.ts`) lẫn đường trích xuất UGC (`SOURCE/features/authoring/actions.ts` + `lib/ugc/gemini.ts`). Đây là bộ đếm duy nhất ở đúng cấp mà trần của nhà cung cấp tồn tại (D6).
...
- [ ] **R14 — Không bán Premium khi key Gemini còn ở gói miễn phí (cổng phát hành)**: Bật bậc trả phí trên project Gemini là **điều kiện tiên quyết** để nhận đồng đầu tiên. **Nguồn sự thật của trạng thái này trong ứng dụng là một biến môi trường tường minh, `GEMINI_PAID_TIER_ENABLED`, do người đặt BẰNG TAY sau khi AC-048 được xác minh bằng một lời gọi thật** — Google không cung cấp API trạng thái thanh toán nào dùng được trên đường nóng, nên mọi cách "tự phát hiện" đều là đoán.
  **Từ 2026-08-18, biến này có người tiêu thụ thứ HAI**: ngoài việc quyết định nút mua có khả dụng hay không (AC-049/AC-054), nó còn quyết định trần chống spam `RATE_LIMITS.explainStep` là **3** hay **≥ 50** lượt/ngày (AC-057). Hai cổng dùng chung một công tắc là **cố ý**: trần chỉ được nâng đúng vào lúc tồn tại một gói có mức dùng ngày cần tới nó, và không sớm hơn một ngày nào.
...
- Trình bày theo token "Mực & Sơn mài" của `SOURCE/app/globals.css` (nguồn chuẩn duy nhất cho theme từ khi `PROJECT_OVERVIEW.md §2` bị xoá 2026-08-06, xem `.claude/MEMORY.md` §3). Lưu ý quy tắc cứng liên quan trực tiếp tới màn hình bán hàng: **đỏ son không phủ khối lớn**; giá tiền là text lớn nên dùng `foreground` trên nền ngà, không dùng khối đỏ.
...
- **`SOURCE/lib/security/rateLimit.ts`** — `RATE_LIMITS.explainStep` **hiện là `{ limit: 3, windowMs: 24 * 60 * 60 * 1000 }` (`:137`)** sau commit `e8d91a4` ngày 2026-08-16: đơn vị cửa sổ đã khớp đơn vị hạn ngạch của nhà cung cấp, lỗi lệch giờ/ngày của Q-1 **đã đóng**. Cái còn lại là **một trần TẠM 3 lượt/ngày dùng chung cho mọi người**, và chú thích tại chỗ (`:123–136`) đã tự khai như vậy: nó chờ được thay bằng hạn ngạch theo gói. **R5 là thứ thay nó** — mốc xuất phát của Design Doc là 3/ngày/người, không phải 20/giờ/người. Thứ thay thế **không** phải một hằng số phẳng mới: sau R5, `explainStep.limit` được **suy ra từ `isPaidTierEnabled()`** — 3/ngày khi bậc trả phí tắt, ≥ 50/ngày khi bật (AC-057; ràng buộc triển khai thuộc `docs/design/subscription-backend-design.md` v1.2 mục B-01). `guard()` vẫn giữ vai trò chống spam cho các Server Action mới (mua, đối soát).
- **`SOURCE/features/exams/tutorActions.ts`** — **nơi thật sự phải sửa** cho đường gia sư: `explainStep()` đã là chỗ đặt kiểm soát truy cập và rate limit (import `guard` ở `:36`, gọi ở `:175`), nên bộ kiểm ngân sách của R7 và hạn mức theo gói của R5 đặt tại đây, **trước** lời gọi `generateHint()`.
...
- **`SOURCE/lib/ugc/gemini.ts` + `SOURCE/features/authoring/actions.ts`** — đường tiêu quota thứ hai. Một lượt upload gọi Gemini **tối đa ba** lần (`extractQuestions` `:406`, `extractAnswers` `:419`, `extractMeta` `:427` — lời gọi thứ ba chỉ chạy khi `metaCall === true`, xem `:425`). Bộ đếm upload hiện tại nằm trong nhánh `else` của `extractAndAssemble` (`:311–322`, câu lệnh chặn `if ((count ?? 0) >= LIMITS.MAX_UPLOADS_PER_DAY)` ở `:317`): nó đếm số row `exams` tạo trong 24h **và bị bỏ qua hoàn toàn ở nhánh xử-lý-lại** `if (rerunExamId)` (bắt đầu `:248`) — R6/AC-019 đóng khoảng hở này. *(File này >700 dòng và sẽ trôi; khi đối chiếu hãy tìm theo tên ký hiệu `rerunExamId`, `metaCall`, `LIMITS.MAX_UPLOADS_PER_DAY`.)*
- **`SOURCE/lib/ugc/limits.ts`** — `MAX_UPLOADS_PER_DAY = 30` phải được thay bằng hạn mức theo gói (R6).
- **`SOURCE/lib/ugc/quotaTracker.ts`** — ⚠ **SỬA MỘT KHẲNG ĐỊNH SAI VỀ CODE (2026-08-18)**: bản 1.0–1.3 viết rằng file này "đã ghi `usageMetadata` của mỗi lượt gọi, nên đo được token thật". **Không đúng.** `recordUsage()` được định nghĩa nhưng **chưa từng được gọi ở bất kỳ đâu trong repo** — đọc bằng grep toàn bộ `SOURCE/`, chỉ có đúng một kết quả là chính dòng khai báo. Thứ thật sự chạm vào `usageMetadata` là bốn câu `logExtractorExit`/`logTutorExit`, và cả bốn **chỉ nằm trên lối THẤT BẠI** (`finishReason ≠ STOP`, text rỗng) — lối thành công không ghi gì cả. Nghĩa là trước 2026-08-18, U2 **không có dữ liệu nào để đo**, và điều kiện chặn số 2 dựa trên một tiền đề sai về chính codebase.
  **Đã sửa 2026-08-18** (cùng phiên): (a) `UsageRecord` tách `inputTokens` (`promptTokenCount`) và `outputTokens` (`candidatesTokenCount` + **`thoughtsTokenCount`** — token suy luận tính giá output, bỏ nó là báo thiếu tiền ở đúng phần đắt nhất), giữ `totalTokens` để `scripts/dev-status.mjs` không vỡ; (b) thêm vai trò `"tutor"` — đường gia sư dùng chung key và chung trần nên phải đếm chung một chỗ; (c) **`recordUsage()` đã được nối vào cả bốn lối gọi** (`extractQuestions`/`extractAnswers`/`extractMeta`/`generateHint`), đặt NGAY sau khi có `response` và TRƯỚC mọi nhánh phân loại — vì một lượt gọi hỏng vẫn tiêu token thật và vẫn trừ vào trần request/ngày.
  **Giới hạn CÒN LẠI, chưa sửa được ở tầng code**: đích ghi vẫn là **file tạm của tiến trình**. Trên Vercel serverless mỗi instance có `tmpdir` riêng và mất khi instance chết, nên bật `UGC_QUOTA_LOG=1` trên production **không** cho một tổng cộng dồn đáng tin — chỉ cho số của một instance trong vòng đời của nó. Muốn đo thật trên production phải đổi đích ghi sang DB; đó là thay đổi **có DDL** (apply tay trên 2 database, TD-005) nên thuộc Design Doc, không phải một bản vá tại chỗ.
...
| **R-j — Đơn vị thời gian lại lệch lần nữa.** Lỗi này (cửa sổ 1 giờ đấu với hạn ngạch tính theo ngày) **đã từng tồn tại trên production** mà mọi cổng đều xanh; nó **đã được vá bằng tay** ở commit `e8d91a4` ngày **2026-08-16** (cửa sổ về 24 giờ, `limit` về 3). Rủi ro còn lại không phải là lỗi cũ đang chạy, mà là **nó quay lại** khi R5 viết lại toàn bộ chỗ này — và lần trước không có gì tự động bắt được nó. | Medium | Low | Chỉ số #6 / AC-013 là một unit test trên bảng hằng — biến "đừng lệch đơn vị" từ một điều cần nhớ thành một cổng tự động, để một lỗi đã xảy ra một lần không quay lại lần hai; và **AC-057 giữ trần chống spam ở dạng suy ra theo bậc thay vì một số phẳng**, nên bất biến "một tài khoản không vét quá 20 request Gemini/ngày" vẫn còn hiệu lực trên đúng giá trị mà CI đọc (`GEMINI_PAID_TIER_ENABLED` không được đặt trong CI) — một trần phẳng 50 sẽ mở lại đúng lỗi cũ ở quy mô gấp ba (`50×1 + 5×3 = 65` request đối diện trần 20) trong toàn bộ khoảng thời gian từ lúc ship tới lúc R14 được bật |

## Undetermined Items

- [x] **U1 — payOS có môi trường sandbox hay không** — ✅ **RESOLVED 2026-08-18 (engineer)**: **payOS không có sandbox**, xác nhận trực tiếp. Hệ quả (từ chính điều kiện leo thang đã ghi ở đây): (1) kiểm thử đầu-cuối webhook bắt buộc dùng **một giao dịch tiền thật giá trị nhỏ trên domain production**, cần engineer duyệt trước khi Work Plan đưa hạng mục này vào phần test; (2) `webhooks.confirm(url)` chỉ đăng ký được với domain production (Preview đổi URL mỗi lần build) — mọi test trên Preview đi đường đối soát chủ động (R10), không chờ webhook.
- [ ] **U2 — Đơn giá chưa được đo thật (CHẶN trước khi coi hạn mức là chốt cuối)** — chủ sở hữu: **engineer**, trước khi bán. Toàn bộ mục "Đơn giá" là ước tính. Việc cần làm, cụ thể — ~~mở rộng `quotaTracker.ts` để tách `promptTokenCount`/`candidatesTokenCount`~~ **✅ XONG 2026-08-18** (tách rồi, gồm cả `thoughtsTokenCount` ở vế output, và `recordUsage()` đã được nối vào cả bốn lối gọi — trước đó nó là code chết, xem Dependencies). **Còn lại hai bước, cả hai đều là quyết định của engineer**: (a) chọn đích ghi dùng được trên production — `UGC_QUOTA_LOG=1` một mình **KHÔNG đủ** vì file tạm không sống qua nhiều instance serverless, nên thực chất là phải chuyển sang DB (có DDL → Design Doc); (b) sau khi có số thật, tính VNĐ/lượt cho **cả hai** đường (gia sư, trích xuất) theo đơn giá $0,30/1M input và $2,50/1M output.
  **Đo tạm được ngay trên máy dev**: `npm run dev` + `scripts/dev-status.mjs` nay hiện `in`/`out` tách riêng cho cả bốn vai trò, gồm `tutor`. Đủ để có một ước lượng bậc nhất trước khi dựng đường ghi production — không đủ để chốt hạn mức, vì tải dev không đại diện cho tải thật. **Điều kiện leo thang — hai ngưỡng, cả hai là điểm HOÀ VỐN tính từ chính các con số của tài liệu này, không phải một bội số cảm tính**:
...
- [ ] **U3 — Nội dung pháp lý của Điều khoản và Chính sách hoàn tiền** — chủ sở hữu: **engineer** (quyết định kinh doanh/pháp lý, không phải quyết định kỹ thuật). R11 chốt rằng hai trang đó **phải tồn tại** và phải nêu rõ việc không tự động gia hạn; nội dung cụ thể thì không — cần chốt tối thiểu: điều kiện và thời hạn hoàn tiền, cách xử lý khi người mua là trẻ vị thành niên theo quy định bảo vệ người tiêu dùng Việt Nam, và pháp nhân/cá nhân đứng tên bán. **Không được để agent tự soạn** — một trang điều khoản bịa ra còn tệ hơn không có trang nào.
  **Tiến độ 2026-08-18**: engineer đã soạn bản đầu của **Chính sách hoàn tiền** → `docs/legal/refund-policy.md`. Còn thiếu, và đều là quyết định của engineer chứ không phải việc kỹ thuật: (a) ba chỗ `[điền …]` — ngày cập nhật, số ngày phản hồi yêu cầu hoàn tiền, số ngày chuyển tiền hoàn; (b) §6 "Đơn vị cung cấp dịch vụ" mới ghi tên thương hiệu, chưa ghi cá nhân/pháp nhân đứng tên bán (payOS onboard bằng CCCD cá nhân — xem ADR-0013); (c) **trang Điều khoản dịch vụ thì chưa có bản nháp nào** — R11 đòi HAI trang, hiện mới có một. Số hạn mức trong bảng của file đó đã được điền khớp D5/D7 (Free 5 lượt gia sư/kỳ).
...
|---|---|---|---|
| 1 | **U1** — ✅ resolved: payOS **không có sandbox** | engineer | xác nhận trực tiếp 2026-08-18; hạng mục kiểm thử tiền-thật trên production vẫn cần engineer duyệt riêng khi Work Plan tới bước đó |
...
- ~~`SUBSCRIPTION-FEATURE.md`~~ — bản khởi thảo, **đã XOÁ khỏi repo 2026-08-18** khi dọn workspace. Toàn bộ nội dung của nó đã được nâng cấp vào chính PRD này (6 câu hỏi mở ở §3 file đó nay là quyết định D1–D10), nên nó không còn là nguồn cho bất cứ điều gì. Cần đọc lại bản gốc thì lấy từ git: `git show aabc3de:SUBSCRIPTION-FEATURE.md`.
...
- `SOURCE/lib/ugc/gemini.ts`, `SOURCE/lib/ugc/quotaTracker.ts`, `SOURCE/lib/ugc/limits.ts`, `SOURCE/features/authoring/actions.ts` — đường trích xuất UGC, bộ ghi token, hạn mức upload hiện tại và khoảng hở ở nhánh xử-lý-lại.
...
- `docs/design/subscription-backend-design.md` (v1.2) — mục **B-01** ("Recorded Decisions") **sở hữu các ràng buộc triển khai** của AC-057 phiên bản có điều kiện theo bậc: những assertion nào của `rateLimit.test.ts` phải giữ nguyên không sửa, chỗ đặt phép suy ra, stub `server-only`, thời điểm đánh giá lúc nạp module, và ca tắt cờ sau khi đã có thuê bao. PRD không lặp lại chúng.
- `SOURCE/lib/billing/paidTier.ts` — `isPaidTierEnabled()` ở `:28` (đọc `GEMINI_PAID_TIER_ENABLED`, tập giá trị BẬT ở `:26`), nguồn sự thật dùng chung cho AC-049/AC-054 và cho nhánh trần của AC-057.
...
- **Trần chống spam theo bậc (tier-conditional anti-spam ceiling)**: giá trị `RATE_LIMITS.explainStep.limit` sau R5 — **không** phải một số phẳng mà là một giá trị suy ra từ `isPaidTierEnabled()`: `3`/ngày khi `GEMINI_PAID_TIER_ENABLED` chưa bật (trần 20 request/ngày toàn dự án của bậc miễn phí vẫn còn), ≥ `50`/ngày khi đã bật (trần đó đã biến mất, và một người Premium dùng ~16,7 lượt/ngày là bình thường). Cửa sổ là 24 giờ ở cả hai nhánh. Quyết định của chủ sở hữu dự án ngày 2026-08-18 (AC-057; ràng buộc triển khai ở B-01 của Design Doc backend).
