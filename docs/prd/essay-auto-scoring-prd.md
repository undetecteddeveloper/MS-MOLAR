...
| Version | Date | Change |
|---|---|---|
| 1.3 | 2026-08-29 | **Closes OQ-7: the grading latency target is relaxed from ≤ 60 seconds to ≤ 3 minutes for ≤ 5 essays (engineer's decision).** The old target was **not reachable on this account and never had been** — not through any defect in this design, but because the provider's `qwen/qwen3.8-27b` allows **8 000 tokens per minute** and one grading request costs **~3 000 tokens**, so five essays need **~15 000 tokens ≥ ~1.9 minutes** of pure token budget before any latency is counted. The engineer chose to **relax the target** rather than accept it as known debt or leave the free tier, on the reasoning that **four display surfaces already imply the promise to students**: a target the provider cannot physically meet is not a documentation problem that debt-tracking fixes, it is a promise the product would keep breaking. Recorded with the arithmetic so it can be recomputed rather than re-guessed, plus the constraint that the target must stay **below `ESSAY_PASS_BUDGET_MS` (4 min)** — a target above the writer's own wall-clock cap is unmeetable by construction. **Consequence outside this document, landed in the same commit**: the UI Spec's poller stopped at **120 s**, which under the corrected target would show "stopped updating" on a **healthy** run; its two caps move to **30 refreshes / 240 s** and are re-anchored to `ESSAY_PASS_BUDGET_MS` rather than to this PRD's target (UI Spec v1.4, frontend Design Doc v1.2). **No AC number changes; AC-061's separation of the polling bound from the read-time deadline is preserved** — the deadline stays 10 minutes and is still derived from the platform's duration ceiling, not from latency. |
...
| D8 | **Result update: a client polling component** under `SOURCE/features/exams/components/`, polling until every essay resolves, then refreshing. No new DB surface for the mechanism, no realtime | Smallest mechanism that closes the loop on a page the student is already looking at | R5 |
...
- `record_skill_mastery()` excludes a per-question row **only** when that row carries `scored:false` — `where coalesce((pq->>'scored')::boolean, true)` (`SOURCE/supabase/schema.sql:1354`). A row with the key *missing* defaults to **included**.
...
flowchart TB
    subgraph IN["In Scope — this release"]
        ...
        I["Client polling component under features/exams/components/ that stops when every essay resolves"]
...
- [ ] **R1 — Grading is asynchronous and never sits on the submit path (D4).**
  - **AC-001**: `submitExam()` emits **zero** grading requests synchronously. Asserted by an integration test in the style of `SOURCE/features/exams/__tests__/submitExam.int.test.ts`: for an attempt containing essay questions, the provider boundary is called 0 times before the action returns.
...
- [ ] **R3 — Fractional points live in new fields; the legacy score triple is untouched (D5, D12).**
  ...
  - **AC-057**: while **any** essay in an attempt is unresolved (`pending`, or `failed` and still retryable), `SOURCE/features/exams/components/ScoreCard.tsx` and the `/history` row (`SOURCE/app/(history)/history`, formatted by `SOURCE/lib/history/format.ts`) render a **"đang chấm" marker** alongside the attempt's number. The number itself keeps today's meaning (D5) — the marker is what tells the student it is not yet final. Exact copy and placement are pinned in the UI Spec.
  - **AC-058**: **PDF export is blocked while any essay in the attempt is unresolved.** The save/share controls in `SOURCE/features/exams/components/ResultActions.tsx` are disabled — as genuinely disabled controls with an accessible reason, not silently inert — and become available once every essay is `graded`, `failed` past its retry cap, or ungradeable (AC-018). Rationale: a PDF is a permanent artifact the learner shares; exporting one whose score changes an hour later is a worse outcome than a short wait, and it is the exact "shared a 0" defect U3 option (c) would have shipped.
...
- [ ] **R5 — The result page resolves itself without the student doing anything (D8).**
  - **AC-020**: a client component under `SOURCE/features/exams/components/` polls while at least one essay in the attempt is `pending`, and **stops** once every essay is `graded` or `failed`.
...
- [ ] **R12 — The product stops telling the student their essay will not be read.**
  - **AC-051**: `player.essayNotScored` is replaced in **both** `SOURCE/lib/i18n/dictionaries/vi.ts` and `SOURCE/lib/i18n/dictionaries/en.ts` with copy stating that essays are auto-scored after submit. Exact strings are pinned in the UI Spec. The `short_answer` slice set the precedent with `player.shortAnswerScored` (`"Trả lời ngắn — chấm tự động sau khi bạn nộp bài."`).
    **The same change is required in the four in-code assertions that state the old rule as fact**, because they are what the next reader (human or agent) will trust over the UI copy:
    ...
    - `SOURCE/types/result.ts` — the `scored` field comment (`essay LUÔN false ("stored, not auto-scored" …)`, lines 14–17). Under W1 `scored` stays `false`, so this comment must be corrected in its **reason**, not its value.
    - `SOURCE/features/exams/components/QuestionRenderer.tsx:179` — `Vẫn KHÔNG chấm tự động (computeScore không bao giờ chấm essay) — chữ dưới ô nói đúng điều đó thay vì hứa hẹn.` This comment justifies the footnote copy AC-051 is replacing; leaving it makes the new copy look like the bug it warns against.
...
- **Grading latency target**: for an attempt with ≤ 5 essay questions, the median time from `exam_results.created_at` to the last essay reaching a **writer-landed** terminal state is **≤ 3 minutes**. Measuring this requires a per-grade timestamp; per U4 that is another **key inside the `per_question` entry** (W2), pinned in the Design Doc alongside the earned/max keys — not a column.
  - **This target was ≤ 60 seconds until 2026-08-29, and it was never reachable** (OQ-7, closed by the engineer). The number is set by the **provider's** ceiling, not by anything this codebase does: the account's `qwen/qwen3.8-27b` carries **TPM 8 000**, and one grading request is **~3 000 tokens**, so 5 essays ≈ **15 000 tokens ÷ 8 000 TPM ≥ ~1.9 minutes of pure token budget** before a single millisecond of network or model latency is counted. No implementation can go faster than the tokens are allowed to flow. **3 minutes** is that floor plus honest headroom for call latency and for the in-pass 429 retries that a TPM ceiling makes routine.
  - **The target sits below `ESSAY_PASS_BUDGET_MS` (4 minutes) on purpose.** A target above the pass's own wall-clock cap would be unmeetable by construction — the writer stops before the target could be hit. Any future move of one of these two numbers must check the other.
  - **~3 000 tokens/request is an estimate, not a measurement** (backend Design Doc v1.5 states this in the same terms). The first real grading run logs actual prompt + completion token counts; if that figure moves materially, this target moves with it, and so do `GROQ_MAX_CONCURRENCY` and `GROQ_BUDGET_DAILY_LIMIT` (OQ-1).
...
- **`SOURCE/features/exams/components/ScoreCard.tsx`, `SOURCE/app/(history)/history` + `SOURCE/lib/history/format.ts`, `SOURCE/features/exams/components/ResultActions.tsx` + `SOURCE/lib/pdf/generateAttemptPdf.ts`** — the three non-result-page surfaces U3 decides (W8): pending marker on the first two (AC-057), export blocked on the third (AC-058). `ResultActions` renders the save/share buttons over `AttemptPdfData`, so the block is a state on those controls, not a change to the PDF generator's contract.
- **`SOURCE/supabase/schema.sql`** — **two** hand-applied changes, not three: the raised `attempt_answers_answer_check` ceiling (R11) and the `telemetry_log` CHECK widening (R13). **U4 removes the third** — the earned/max storage lives in the existing `per_question` jsonb (W2), so R3 carries **no DDL and no `record_exam_result()` signature change**. **Both remaining changes are subject to TECH-DEBT TD-005** (schema applied by hand to two Supabase projects, no migration tool). `npm run verify:schema` is the gate that distinguishes "applied" from "present in git only"; prod's `schema_version.fingerprint` was **`021dd1387945`** on 2026-08-27, matching the literal then at `SOURCE/supabase/schema.sql:1862` — i.e. prod was in sync at the time this PRD was revised, and any drift found later post-dates that measurement. **And it did: superseded 2026-08-28.** Commit `004d628` (exam rating 1–10 → 1–5 stars) moved the fingerprint to **`29931beeb950`**, now the literal at `schema.sql:1871`, and that migration is already applied to prod. **The Phase 3.5 baseline for this feature is `29931beeb950`, not the value above.** Comparing against the superseded value would read prod as behind when it is in sync — the most dangerous direction of that error.
...
| `schema_version.fingerprint` on prod | **`021dd1387945`** as measured 2026-08-27, matching the literal **then at** `SOURCE/supabase/schema.sql:1862` | Prod was **in sync** with git at the time of measurement (TD-005 context, C3). **Superseded 2026-08-28** — the fingerprint moved to **`29931beeb950`** (now at `schema.sql:1871`) via commit `004d628` (exam rating 1–10 → 1–5 stars), and that migration has been applied to prod. Prod remains in sync; the clean Phase 3.5 baseline for this feature's two new SQL functions is `29931beeb950`. See ADR-0018 Update History. |
