...
> | **OQ-5** | **CÒN MỞ — chủ sở hữu: kỹ sư/sản phẩm** (Task E4) | Quyết định về `upload.essayStored`: (a) để nguyên, (b) sửa chuỗi cùng lượt bật cờ, (c) mở mục UI Spec cho `(layer4)`. **Không chặn ship**; nó chặn việc *coi bề mặt tác giả là đã đúng*. Chuỗi `upload.essayScored` đã tồn tại trong cả `en.ts` lẫn `vi.ts` và `QuestionEditor.tsx:410` đã chọn theo cờ, nên phương án (b) là **một dòng, đã lắp sẵn** — nhưng lựa chọn vẫn là của kỹ sư |
