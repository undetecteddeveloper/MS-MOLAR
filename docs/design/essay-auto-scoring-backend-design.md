...
- Sáu trạng thái render RS-0…RS-6 của UI Spec là **đầu ra** của hàm suy diễn ở § `deriveEssayView()`; bảng ánh xạ RS ↔ giá trị trả về nằm ở § State Transitions and Invariants.
...
- [x] `SOURCE/app/(HM)/queries.ts` — `listMyHistory()` thêm `per_question, created_at` vào select; `MyHistoryEntry` nhận **đúng một** trường boolean `hasUnresolvedEssay`.
...
- **Một số dòng được trích trong PRD/ADR/UI Spec đã trôi lệch** sau khi `main` được merge vào nhánh này — bảy trường hợp, tất cả nằm ở nửa sau `schema.sql` (§ Bảng đối chiếu số dòng).
...
Nhiệm vụ bắt buộc kiểm lại từng số dòng thay vì tin theo. Bốn trong số đó đã trôi lệch — **tất cả** nằm ở nửa sau `schema.sql`, và độ lệch nhất quán (+9) cho thấy nguyên nhân là các khối DDL chèn thêm ở đoạn giữa file chứ không phải một trích dẫn sai từ đầu.
...
| **D-03** | `/history` — `MyHistoryEntry` không mang `per_question`, nên AC-057 bất khả thi | `transform` | Kiểm lại: `MyHistoryEntry` (`app/(HM)/queries.ts:8-18`) có đúng chín trường, không có `per_question`; select (`:64-66`) cũng không lấy nó **và không lấy `created_at`** — mà hàm suy diễn cần **cả hai**. Kết quả mới: select thêm `per_question, created_at`; `EmbeddedRow` thêm hai trường; `MyHistoryEntry` nhận **đúng một** boolean `hasUnresolvedEssay` (UI-D11 — dữ liệu thô không băng qua biên component). Chi phí payload **chưa đo** → OQ-3. | `app/(HM)/queries.ts:8-18`, `:23-34`, `:60-93` |
...
**Ba giá trị lưu, sáu trạng thái render.** Bảng dưới là ánh xạ chuẩn giữa cái được lưu và RS-0…RS-6 của UI Spec; nó là hợp đồng mà `deriveEssayView()` phải thoả từng dòng.
...
- *Trường đầu ra so sánh:* toàn bộ chín trường sẵn có của `MyHistoryEntry` **cộng** `hasUnresolvedEssay === false`; và thứ tự sắp xếp (theo `submittedAt` giảm dần) không đổi.
...
| AC-048 | § Trần ký tự + § Cổng trần ký tự; **mục (3) tự di chuyển** (§ D-04) | EG-BE-028 |
...
- **OQ-3 — Chi phí payload của `per_question` trong `listMyHistory()` (thừa kế UI Spec O-3).** `readBounded` chặn ở `LIST_ROW_CEILING = 500` (`boundedRead.ts:74`), nhưng trần đó **chưa được đo** với `per_question` trong select. *Đầu vào cần:* một phép đo trên dev với số hàng bằng trần, ghi kích thước payload trước/sau. *Người chịu trách nhiệm:* kỹ sư, trước Work Plan. *Điều kiện leo thang:* nếu payload phình quá mức chấp nhận được, phương án thay thế là một RPC trả sẵn boolean — nhưng đó là **DDL**, tức TD-005 một lần nữa, nên **không được chọn mà không có số đo**.
...
- `docs/ui-spec/essay-auto-scoring-ui-spec.md` v1.1 — UI-D1…UI-D13, RS-0…RS-6, Copy Inventory, O-1 (đóng ở § Hợp đồng khoá jsonb), O-2 (đóng ở § MSA-2 + § Field Propagation Map), O-3 (chuyển thành OQ-3), O-8 (đã chốt; ảnh hưởng `unresolvedCount`).
