...
- **Một hệ quả sắc hơn, phải nói ra:** NFR Performance của PRD đặt mục tiêu **trung vị ≤ 60 giây** cho lượt thi ≤ 5 câu tự luận. Với TPM 8K và ~3K token/request, 5 câu ≈ 15K token ⇒ **tối thiểu ~2 phút**, nên mục tiêu ấy **không đạt được** trên free tier của tài khoản này — không phải vì cài đặt kém, mà vì trần của nhà cung cấp. Ghi ra thay vì để nó lặng lẽ trượt: xem **OQ-7**.
...
- **OQ-7 — Mục tiêu độ trễ của PRD không đạt được trên free tier của tài khoản này.** NFR Performance của PRD đặt **trung vị ≤ 60 giây** cho lượt thi ≤ 5 câu tự luận. Với TPM 8K và ~3K token/request, 5 câu ≈ 15K token ⇒ **tối thiểu ~2 phút**. Đây là trần của **nhà cung cấp**, không phải một khuyết tật cài đặt, nên không có cách nào trong tài liệu này chạm tới nó. *Đầu vào cần:* quyết định của kỹ sư — (a) nới mục tiêu của PRD cho khớp thực tế đo được, (b) chấp nhận và ghi là nợ đã biết, hoặc (c) trả tiền lên gói cao hơn. *Người chịu trách nhiệm:* kỹ sư (PRD là chủ sở hữu của con số ấy, không phải tài liệu này). *Điều kiện leo thang:* trước khi bật cờ trên prod, vì nó đổi lời hứa mà bốn bề mặt hiển thị đang ngụ ý. *Liên quan:* cận polling 120 giây của UI Spec dừng trước khi chấm xong trên lượt thi nhiều tự luận — suy giảm đúng cách qua `result.essay.pollStopped`, chủ sở hữu là UI Spec / Design Doc frontend (§ Hằng số).
