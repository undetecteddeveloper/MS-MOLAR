...
- [x] `SOURCE/features/exams/actions.ts` — `submitExam()` đọc cờ tính năng, truyền `options` vào `computeScore()`, và đăng ký `after()` **trước** `redirect()` (:192).
- [x] `SOURCE/features/exams/essayActions.ts` (mới) — Server Action chấm lại, uỷ quyền trước đo đếm (AC-072).
- [x] `SOURCE/features/exams/queries.ts` — `getResult()` thêm `created_at` vào select, gắn `essay?: EssayView` cho từng dòng và `essaySummary?: EssaySummary` cho cả lượt thi.
- [x] `SOURCE/features/history/queries.ts` — `listMyHistory()` thêm `per_question, created_at` vào select; `MyHistoryEntry` nhận **hai** trường boolean, mỗi trường mang **một** sự thật: `hasUnresolvedEssay` (còn câu đang chạy ⇒ **chốt chặn xuất PDF**, AC-058) và `hasIncompleteEssay` (có câu ở RS-6 ⇒ **điều kiện in** `result.essay.pdfIncomplete`, O-8). Hai chứ không một — xem **§ D-13**: gộp chúng lại chính là khuyết tật F-06, và nó ship ra hai tệp PDF khác nhau cho cùng một lượt thi.
...
- [x] `SOURCE/tests/e2e/service/essay-grade-write.service.e2e.test.ts` (**làn chạy được**, `npm run test:localdb`) — cases `EG-a…EG-e` với Postgres thật. **`test-rls.ts` KHÔNG nhận bản sao** (§ Integration Verification Points).
- [x] `SOURCE/app/(exams)/exams/[id]/attempt/[attemptId]/page.tsx` và `…/result/detail/page.tsx` — `export const maxDuration` (route segment; **không** khai được trong file `"use server"`).
...
- [ ] **Đo hiệu năng**: **Bắt buộc một phần.** Không phải cổng CI (không tất định), nhưng round-trip Singapore→Groq **phải được đo trong lúc cài đặt** trước khi chốt lại bốn hằng thời gian (C4, Scalability của PRD). Chỉ tiêu để đối chiếu: trung vị từ `exam_results.created_at` tới `essayGradedAt` cuối cùng **≤ 3 phút** cho lượt thi ≤ 5 câu tự luận (PRD v1.3, **OQ-7 đã đóng** — chỉ tiêu cũ “≤ 60 giây” **không đạt được** dưới trần TPM 8K).
...
- [x] **`MyHistoryEntry` KHÔNG mang `per_question`, và select của `listMyHistory()` không lấy nó.** Evidence: `features/history/queries.ts:8-18` (chín trường), `:64-66` (chuỗi select). Confirmed: **Yes** — Discrepancy D-03.
...
- [x] **`maxDuration` KHÔNG khai được trong file `"use server"`; phải đặt ở route segment gọi nó.** Evidence: `tutorActions.ts:16-21` (dẫn tài liệu Next.js 16 kèm trong `node_modules`); tiền lệ đã chạy: `app/(authoring)/upload/page.tsx:18` (`export const maxDuration = 300`). Confirmed: **Yes**.
...
- [x] `vitest run` (làn mặc định) — Enforces: đúng đắn unit/integration — Config: `SOURCE/vitest.config.ts` — Covers: `lib/**`, `app/**`, `components/**` — Status: `adopted` (cơ chế chứng minh chính, xem § Verification Strategy).
- [x] **`npm run test:localdb`** — Enforces: hành vi của Postgres THẬT cho hai hàm SQL mới (thứ tự mảng, ghi-lần-đầu-thắng, trần lượt tiêu lúc claim, grant) — Config: `SOURCE/vitest.localdb.config.ts` — Covers: `SOURCE/tests/e2e/service/**` — Status: `adopted`. **Đây là nhà của `EG-a…EG-e`.** Chạy **tại máy dev**, không phải trên CI (config ghi rõ: *"CI has no database"*), và kể từ 2026-08-29 nó là một trong **sáu** cổng verify trước commit.
- [x] `npm run test:fixture` — Config: `SOURCE/vitest.fixture.config.ts` — Status: `adopted` như một cổng chung của repo, **không** phải cơ chế của tính năng này. Ghi nhận trạng thái: **đang ĐỎ trên `main`** vì một khuyết tật có sẵn trong `subscription.fixture.e2e.test.ts`, **không liên quan** tính năng này (TECH-DEBT TD-030).
- [x] `npm run test:integration` — Config: `SOURCE/vitest.integration.config.ts` — Status: `noted` (không ca nào của tính năng này đáp xuống làn đó).
...
- [ ] `npx tsx supabase/test-rls.ts` — Status: `noted`, và **cố ý không phải nhà của `EG-a…EG-e`**. Lý do là cơ học chứ không phải thẩm mỹ: file này **không có npm script nào** (đã kiểm `package.json`) nên nó chỉ chạy khi có người gõ tay `npx tsx`, trong khi `npm run test:localdb` là một trong sáu cổng trước commit. Nó **vẫn** giữ nguyên vai trò cho các case đã ship — đáng kể nhất `S-b` (`:1314-1320`), thứ đã chứng minh JWT học sinh không `UPDATE` được `exam_results`.
...
- **Performance**: đường nộp bài không đổi (0 request đồng bộ). Trung vị từ `created_at` tới `essayGradedAt` cuối cùng **≤ 3 phút** cho lượt ≤ 5 câu tự luận (đo, không phải cổng CI). Chỉ tiêu này **nằm dưới `ESSAY_PASS_BUDGET_MS` (4 phút)** — một chỉ tiêu cao hơn trần đồng hồ của chính lượt chấm là không thể đạt theo cấu trúc.
...
| New | `SOURCE/features/exams/essayActions.ts` | `"use server"`. `retryEssayGrading(attemptId, questionId)` — typed-result, uỷ quyền trước đo đếm. |
...
| Existing (modified) | `SOURCE/features/exams/actions.ts` | `submitExam()` đọc cờ, truyền `options`, đăng ký `after()` trước `redirect()` (:192). |
| Existing (modified) | `SOURCE/features/exams/queries.ts` | `getResult()`: `created_at` vào select (:579-581) và vào `ResultRow` (:469-475); gắn `essay` cho từng dòng và `essaySummary` cho cả lượt (cạnh chỗ gắn `hasBeenWrongTwice` ở :606-610). |
| Existing (modified) | `SOURCE/features/history/queries.ts` | `listMyHistory()`: `per_question, created_at` vào select (:64-66); `MyHistoryEntry` (:8-18) nhận **hai** trường boolean (`hasUnresolvedEssay`, `hasIncompleteEssay` — § D-13); `EmbeddedRow` (:23-34) nhận hai trường. |
...
| Existing (modified) | `SOURCE/app/(exams)/exams/[id]/attempt/[attemptId]/result/page.tsx` (:56), `SOURCE/features/history/components/HistoryRow.tsx` (:23) | Hai chỗ dựng `AttemptPdfData`; mỗi chỗ điền `hasIncompleteEssay` từ nguồn đã suy sẵn của chính đường đọc mình (`ExamResult` / `MyHistoryEntry`). **Không** chỗ nào tự viết lại biểu thức RS-6. |
...
| New | `SOURCE/tests/e2e/service/essay-grade-write.service.e2e.test.ts` | Cases `EG-a…EG-e` với Postgres thật, chạy bằng `npm run test:localdb`. Khung test đã có trong cây; SVC-1 = `EG-c`+`EG-d`, SVC-2 = `EG-a`+`EG-b`+`EG-e`. |
| Existing (KHÔNG đổi) | `SOURCE/supabase/test-rls.ts` | **Không thêm Phần 10, không nhận bản sao nào của `EG-a…EG-e`** (§ D-16). `S-b` (:1314-1320) ở nguyên chỗ và tiếp tục phủ "JWT học sinh không `UPDATE` được `exam_results`". |
...
| Dấu "đang chấm" trên `/history` | `listMyHistory()` (`features/history/queries.ts:36-93`) | Thêm cột select + suy một boolean trong hàm map | **Trung bình** | Integration, EG-BE-026 |
...
| `features/exams/queries.ts:469-475`, `:554-671` | Integration point — `ResultRow` thiếu `created_at`; `:576-586` là select; `:606-610` là chỗ gắn trường suy ra. |
| `features/history/queries.ts:8-18`, `:23-34`, `:60-93` | Integration point — `MyHistoryEntry`, `EmbeddedRow`, select và hàm map. |
| `features/exams/actions.ts:55-193` | Integration point — `submitExam()`; `:83-85` nhánh idempotent, `:143-151` batch answers, `:154` gọi `computeScore`, `:162-166` ghi điểm, `:183-190` mastery best-effort, `:192` redirect. |
...
| `features/exams/components/QuestionRenderer.tsx:23`, `:165`, `:185-205` | Bằng chứng — trần đọc từ **một alias** của hằng; `short_answer` dùng `MAX_SHORT_ANSWER` **khác**; comment cũ ở :179-180. |
| `features/exams/tutorActions.ts:1-33`, `:106-139`, `:264-277`, `:300` | Pattern reference + bằng chứng — quy ước typed-result, `maxDuration` là cấu hình route segment, telemetry best-effort qua client học sinh, và `studentAnswer` **không** bị cắt. |
...
| `supabase/test-rls.ts:1244-1382` (Phần 4, S-a…S-e), `:1601-1671` (Phần 7, MM-a/MM-b) | Pattern reference — `S-b` (:1314-1320) **đã** chứng minh học sinh không `UPDATE` được `exam_results`, và **ở nguyên đó**; `MM-b` (:1652-1671) là khuôn phân biệt `42501` mà `EG-a`/`EG-b` chép theo. Năm ca mới **không** sống ở file này (§ D-16) — chúng chỉ mượn khuôn. |
...
| `lib/pdf/generateAttemptPdf.ts:11` (`AttemptPdfData`), `:30` (`generateAttemptPdfFile`) | Data contract — kiểm bằng grep toàn repo: kiểu này có **hai** chỗ dựng (`app/(exams)/exams/[id]/attempt/[attemptId]/result/page.tsx:56`, `features/history/components/HistoryRow.tsx:23`) và **sáu** chỗ chỉ truyền tiếp (`ResultActions.tsx:16`, `ActionButton.tsx:45`, `HistoryRowMenu.tsx:49`, `usePdfAction.ts:40`, cùng hai file test). Đây là bằng chứng cho § D-13: đặt trường ở kiểu này chạm cả hai lối xuất bằng một lần sửa. |
| `features/exams/components/__tests__/QuestionRenderer.test.tsx:112`, `:116`, `:119` | Coupled sites — `:119` ghim `maxLength` bằng literal `500` (chỗ ghép cặp của AC-048); `:112` ghim nguyên văn `player.essayNotScored` bản tiếng Anh (chỗ ghép cặp của AC-051, hỏng ở một thời điểm khác). Xem § D-14. |
...
| **D-11** | Chuỗi hiển thị `upload.essayStored` nói với **tác giả đề** rằng tự luận "chưa chấm tự động" | `out-of-scope` (kèm cảnh báo) | `vi.ts:271` / `en.ts:334` (`"Tự luận — đã lưu, chưa chấm tự động."`), render ở `features/authoring/components/QuestionEditor.tsx:15`. Nó trở thành **sai** khi cổng AC-067 qua. Ranh giới loại nó ra: D6 giữ **bề mặt của tác giả không đổi**, và bốn màn hình của UI Spec không gồm `(authoring)`. Nhưng nó không được im lặng — ghi thành **OQ-5** để kỹ sư chọn dứt điểm. | `vi.ts:271`, `en.ts:334`, `QuestionEditor.tsx:15` |
...
| **D-14** | **Chỗ ghép cặp thứ ba của trần ký tự mà AC-048 KHÔNG nêu tên** | `transform` | `SOURCE/features/exams/components/__tests__/QuestionRenderer.test.tsx` ghim **hai** thứ mà tính năng này dịch chuyển, và chúng thuộc **hai AC khác nhau, hỏng vào hai thời điểm khác nhau** — nên gộp chúng lại là cách chắc chắn để sửa nhầm một cái: **(a) `:119` — `expect(textarea?.maxLength).toBe(500)`** là chỗ ghép cặp của **AC-048**; nó đỏ **ngay** khi `LIMITS.MAX_ATTEMPT_ANSWER` lên 4000, nên nó phải di chuyển trong **cùng commit** với bước 12. Comment `:116` cũng viết cứng *"CHECK length <= 500"* và đi kèm. **(b) `:112` — chuỗi `"Essay — your working is saved with the attempt, not auto-scored yet."`** (đúng `en.ts:197` = `player.essayNotScored`) là chỗ ghép cặp của **AC-051**, **không** của AC-048; nó ở nguyên **xanh** suốt giai đoạn tính năng còn tắt, vì UI-D8 **giữ** khoá cũ và `QuestionRenderer` là client component nên cờ tới nó bằng prop, mặc định tắt. Nó chỉ thành chỗ ghép cặp khi một test bắt đầu chạy nhánh **bật**. **Lập luận này ĐÃ ĐƯỢC XÁC NHẬN, không còn là suy đoán** (v1.2): kỹ sư chốt FE-OQ-2 theo phương án (a), và Design Doc frontend §MSA-F2 pin đúng hình dạng mà lập luận này dựa vào — prop `essayGradingEnabled?: boolean` **tuỳ chọn, mặc định `false`**. `QuestionRenderer.test.tsx` dựng component **không** truyền prop đó, nên nó nhận mặc định `false`, nên nó render `player.essayNotScored`, nên chuỗi ghim ở `:112` vẫn đúng. Nếu prop từng được đổi thành **bắt buộc**, hoặc mặc định thành `true`, thì `:112` đỏ ngay và trở thành chỗ ghép cặp của AC-051 sớm hơn dự kiến. Danh sách chỗ ghép cặp của AC-048 vì thế là **bốn** ở tầng mã (schema.sql, `limits.ts`, `submitExam` slice, test này) cộng cổng `verify-schema` — mục (3) của AC-048 thì **tự di chuyển** (§ D-04). | `features/exams/components/__tests__/QuestionRenderer.test.tsx:112`, `:116`, `:119`; `lib/i18n/dictionaries/en.ts:197` |
| **D-15** | **v1.0 khẳng định `ESSAY_GRADING_ENABLED` "đọc ở đúng một chỗ" — SAI, và sai theo hai hướng độc lập** | `transform` | Hướng thứ nhất, **mâu thuẫn nội bộ ngay trong v1.0**: § Cờ tính năng viết "đọc ở đúng một chỗ (`submitExam()`)" trong khi chính đoạn đó, vài dòng dưới, lại viết *"`retryEssayGrading()` cũng kiểm cờ"*. Hai câu trong cùng một mục phủ định nhau. Hướng thứ hai, **mâu thuẫn với UI-D8**: chân trang ô tự luận phải chọn giữa hai khoá i18n theo cờ, mà `QuestionRenderer` là client component nên cờ **phải** được một Server Component đọc rồi truyền xuống — một chỗ đọc thứ ba. Kết quả mới: **ba** chỗ đọc phía server, phân thành **hai loại mục đích** (hai cổng hành vi, một cổng câu chữ), tất cả đọc **một** biến để chúng lật cùng lúc trong một lượt deploy. **Không** thêm `NEXT_PUBLIC_*` (UI-D7 cấm). *Ghi chú về nguồn: bản vá này được đặt hàng với con số "hai chỗ đọc"; con số đúng là **ba**, vì chỗ đọc trong `retryEssayGrading()` là của chính tài liệu này và người đặt hàng không có nó trong tầm nhìn. Viết "hai" sẽ đưa một lỗi mới vào đây để sửa một lỗi cũ.* | `features/exams/actions.ts` (`submitExam`), `features/exams/essayActions.ts` (`retryEssayGrading`), `app/(exams)/exams/[id]/attempt/[attemptId]/page.tsx` (segment); `ExamPlayer.tsx:29-41` và `QuestionRenderer.tsx:45-53` (đã kiểm: **chưa** có prop nào như vậy, nên prop mới phải tuỳ chọn) |
| **D-16** | **v1.0 giao `EG-a…EG-e` cho `test-rls.ts`, trong khi cây mã đã có một làn CHẠY ĐƯỢC dành đúng cho việc đó** | `transform` | Đây là **doc-vs-tree drift**, không phải một lựa chọn thiết kế: tài liệu này chưa từng nhắc `SOURCE/tests/e2e/service/**`, `vitest.localdb.config.ts` hay `npm run test:localdb`, dù cả ba đã được commit và comment đầu config mô tả mục đích gần đúng từng chữ mà § Verification Strategy của tài liệu này dùng. Kết quả mới (kỹ sư chốt 2026-08-29): năm ca chuyển sang `SOURCE/tests/e2e/service/essay-grade-write.service.e2e.test.ts`, chạy bằng `npm run test:localdb`; **nghĩa vụ và ID giữ nguyên văn, chỉ đổi nhà**. Ánh xạ khung test: SVC-1 = `EG-c`+`EG-d`, SVC-2 = `EG-a`+`EG-b`+`EG-e`. **Lý do làn chạy được thắng:** `test-rls.ts` **không có npm script** (đã kiểm `package.json`: có `verify:schema`, không có mục nào cho `test-rls`), nên nó chỉ chạy khi ai đó gõ tay `npx tsx`; còn `test:localdb` là một trong **sáu** cổng verify trước commit kể từ 2026-08-29 (nâng từ bốn — TD-030). TD-030 tồn tại chính vì `npm run test:fixture` bị phát hiện **đỏ trên `main`** mà không cổng nào thấy, đúng vì không có gì tự động chạy nó. **Một chứng cứ mà người review chạy lại được bằng một câu lệnh mạnh hơn một chứng cứ khớp câu chữ của tài liệu.** Hai hệ quả được ghi thành văn: `test-rls.ts` **không** nhận bản sao (hai nhà thì trôi lệch, và bản trôi lệch là bản không ai chạy), và `S-b` (`:1314-1320`) **ở nguyên chỗ cũ** vì nó đã ship và đã chạy. | `SOURCE/vitest.localdb.config.ts:1-21`; `SOURCE/package.json` (`test:localdb`, và **không** có script cho `test-rls`); `SOURCE/tests/e2e/service/essay-grade-write.service.e2e.test.ts:52-60`; `TECH-DEBT.md` TD-030 |
| **D-17** | **Hằng số dung lượng cỡ theo RPM, trong khi trần ràng buộc là TPM — và model khởi điểm không tồn tại trong tài khoản** | `transform` | Hai khuyết tật cùng gốc: v1.0 cỡ mọi thứ theo **giới hạn công bố của bên thứ ba** cho một model mà **tài khoản không có**. (a) `ESSAY_GRADER_MODEL` = `llama-3.3-70b-versatile` — **không có trong danh mục tài khoản**; thay bằng `qwen/qwen3.8-27b`, chọn vì **TPD 2M gấp mười lần** mọi ứng viên còn lại và TPD là trần ràng buộc trước tiên. (b) `GROQ_MAX_CONCURRENCY = 4` suy từ "30 RPM"; 30 RPM **đúng** nhưng **không bao giờ là trần ràng buộc** — **TPM 8K mới là**, và tài liệu chưa từng có con số đó. Ở ~3K token/request, đồng thời 4 bắn ~12K vào trần 8K, tức **vượt trần ở mọi lượt pass một cách có hệ thống**; và điều này **retry không cứu được** — F2/AC-065 dựng retry cho 429 *thỉnh thoảng*, không cho một cấu hình vượt trần mỗi lần chạy. Hạ xuống **2** (~6K/burst). (c) `GROQ_BUDGET_DAILY_LIMIT` chốt **600** request: TPD 2M ÷ ~3K ≈ 660, ràng buộc **trước** RPD 1K, nên "cho bằng RPD" là cấp phép quá tay; 600 khiến lời từ chối của ta nổ trước của nhà cung cấp. **Bài học phương pháp:** danh mục model của Groq là chuyện **theo từng tài khoản** — ba nguồn bên thứ ba đồng thuận vẫn không thay được một lượt đo trên chính tài khoản sẽ dùng. | Giới hạn đo được trên tài khoản của kỹ sư 2026-08-29 (§ Điểm phát Groq, § References); § Hằng số; § Trần ký tự căn cứ #3 |
...
Direct Impact:
  ...
  - SOURCE/features/exams/essayActions.ts (MỚI — Server Action chấm lại)
  - SOURCE/features/exams/actions.ts (submitExam đọc cờ, truyền options, đăng ký after() trước redirect :192)
  - SOURCE/features/exams/queries.ts (getResult: created_at vào select + ResultRow; gắn essay + essaySummary)
  - SOURCE/features/history/queries.ts (listMyHistory: per_question + created_at vào select; MyHistoryEntry +2 boolean — hasUnresolvedEssay, hasIncompleteEssay)
  - SOURCE/lib/pdf/generateAttemptPdf.ts (AttemptPdfData +1 trường hasIncompleteEssay — kiểu dùng chung của CẢ HAI lối xuất PDF)
  - SOURCE/app/(exams)/exams/[id]/attempt/[attemptId]/result/page.tsx và SOURCE/features/history/components/HistoryRow.tsx (hai chỗ dựng AttemptPdfData, mỗi chỗ điền trường mới từ đường đọc của mình)
  ...
  - SOURCE/tests/e2e/service/essay-grade-write.service.e2e.test.ts (MỚI — cases EG-a…EG-e, chạy bằng npm run test:localdb; test-rls.ts KHÔNG nhận bản sao, và S-b ở nguyên chỗ)
  - SOURCE/app/(exams)/exams/[id]/attempt/[attemptId]/page.tsx (maxDuration — route segment của submitExam)
  - SOURCE/app/(exams)/exams/[id]/attempt/[attemptId]/result/detail/page.tsx (maxDuration — route segment của retry)
...
flowchart TD
    subgraph SYNC["Đường ĐỒNG BỘ — không đổi về mặt quan sát được"]
      Player["Exam Player (client)"] -->|"Nộp bài"| Submit["submitExam()<br/>features/exams/actions.ts"]
    ...
    subgraph READ["Đường ĐỌC — nơi trạng thái cuối được SUY RA"]
      ...
      Retry["retryEssayGrading()<br/>features/exams/essayActions.ts<br/>UỶ QUYỀN TRƯỚC ĐO ĐẾM"] --> CL
...
| `GROQ_MAX_CONCURRENCY` | `lib/essay/gradeEssays.ts` | **`2`** | AC-036. **Cỡ theo TPM, không theo RPM** — sửa ở v1.5 (§ D-17). 30 RPM là con số **đúng** nhưng nó **chưa bao giờ là giới hạn ràng buộc**; **TPM 8K mới là**. Số học để người sau tính lại được khi giới hạn đổi: một bài 4000 ký tự tiếng Việt ≈ **1.200 token**, cộng đáp án tham chiếu, đề bài và rubric ⇒ một request ≈ **2.500–3.500 token**. Vậy **8K TPM ÷ ~3K ≈ 2–3 request mỗi phút**, không phải 30. Đồng thời 4 sẽ bắn ~12K token vào trần 8K — **vượt trần ở MỌI lượt pass, một cách có hệ thống**. Điều đó quan trọng hơn con số: F2 coi 429 là thất bại **được dự kiến** và AC-065 dựng retry trong cùng pass cho nó, nhưng **retry không cứu nổi một mức đồng thời vượt trần TPM mỗi lần chạy** — nó chỉ biến một khuyết tật cấu hình thành một vòng lặp chậm. Đồng thời **2** ⇒ ~6K mỗi burst, dưới 8K. |
...
**Số học của trường hợp xấu nhất, viết ra thay vì để suy:** 50 câu tự luận / đồng thời **2** = **25 nhịp**. Mỗi nhịp xấu nhất = 3 × 20 s deadline + ~3 s ngủ ≈ 63 s ⇒ ~26 phút, **vượt xa** trần 300 s của nền tảng. Đó **không** phải một khuyết tật cần sửa: `ESSAY_PASS_BUDGET_MS` cắt pass ở 4 phút, phần còn lại ở `pending` với **0 lượt đã tiêu**, hạn chờ đọc-lúc-render trình bày chúng thành "Chấm thất bại", và học sinh chấm lại từng câu.

**Tương tác thông lượng ↔ UX, GHI NHẬN chứ KHÔNG sửa ở đây (v1.5).** TPM chặn thông lượng bền vững ở **~2–3 câu mỗi PHÚT**. Hệ quả dây chuyền:

- Một lượt thi **5 câu** tự luận cần **~2–3 phút**, trong khi cận polling của UI Spec dừng cứng ở **120 giây**. Trên các lượt thi nhiều tự luận, **trang ngừng tự cập nhật trước khi chấm xong**.
- Nó **suy giảm đúng cách**, không phải hỏng: `result.essay.pollStopped` và nút "Cập nhật" thủ công đã tồn tại (UI Spec § EssayGradingPoller), và hạn chờ đọc-lúc-render 10 phút vẫn phủ trọn ca này. Câu chữ ở trạng thái đó **không** nói câu hỏi đã thất bại — đúng như AC-061 đòi.
- **Chủ sở hữu: UI Spec / Design Doc frontend.** Chỉnh lại hằng polling là một quyết định UX và **không thuộc tài liệu này**; không hằng số polling nào bị đụng ở đây.
- **Một hệ quả sắc hơn, đã được giải quyết ở thượng nguồn:** NFR Performance của PRD từng đặt mục tiêu **trung vị ≤ 60 giây** cho lượt thi ≤ 5 câu tự luận. Với TPM 8K và ~3K token/request, 5 câu ≈ 15K token ⇒ **tối thiểu ~2 phút**, nên mục tiêu ấy **không đạt được** — không phải vì cài đặt kém, mà vì trần của nhà cung cấp. **Đã đóng 2026-08-29 (OQ-7):** kỹ sư chọn **nới chỉ tiêu của PRD lên ≤ 3 phút** (PRD v1.3) thay vì ghi nợ hay rời free tier, vì **bốn bề mặt hiển thị đã ngụ ý lời hứa đó với học sinh** — một chỉ tiêu nhà cung cấp không thể đáp ứng về mặt vật lý không phải lỗi tài liệu mà ghi nợ sẽ chữa được. Kèm theo ở hạ nguồn: hai trần polling của UI Spec **18/120 giây → 30/240 giây** (UI Spec v1.4, frontend DD v1.2), vì trần 120 giây cũ sẽ hiện `pollStopped` trên một lượt chạy **bình thường**.
...
|---|---|---|
| `ExamResult` | `SOURCE/features/exams/queries.ts` | `getResult()` là chỗ duy nhất cầm `created_at` (nó **không** lộ `createdAt` ra ngoài — § Field Propagation Map giữ nguyên quyết định đó), nên nó phải tự gấp và phơi kết quả. |
| `MyHistoryEntry` | `SOURCE/features/history/queries.ts` | `/history` không có `EssaySummary`; đây là boolean **thứ hai**, cạnh `hasUnresolvedEssay`. |
| `AttemptPdfData` | `SOURCE/lib/pdf/generateAttemptPdf.ts:11` | **Kiểu chung của cả hai lối xuất PDF** — đã kiểm: nó được dựng ở `app/(exams)/exams/[id]/attempt/[attemptId]/result/page.tsx:56` **và** `features/history/components/HistoryRow.tsx:23`, rồi cả hai đi qua `components/history/usePdfAction.ts:40`. Đặt trường ở đây là chỗ duy nhất khiến hai lối xuất **không thể** mang hai sự thật khác nhau. |
...
```yaml
Contract: buildEssayPrompt(input: EssayPromptInput): string   # đổi tên ở v1.8 — xem dưới
Input:
...
#### `features/exams/essayActions.ts` (biên Server Action)
...
5. Áp `schema.sql` lên **dev trước**, chạy `npm run verify:schema`, rồi chạy `npm run test:localdb` (cases `EG-a…EG-e`). Thứ tự này là bắt buộc và chính config của làn service ghi lý do: chạy nó lên một database chưa nhận DDL sinh ra lỗi **trông y hệt** lỗi cài đặt nhưng không phải — thấy đỏ khi `verify:schema` chưa xanh thì sửa database, đừng sửa test.
...
3. **Nó nằm trong trần token mỗi phút của free tier ở mức đồng thời đã chọn.** *(Số liệu cập nhật ở v1.5 theo giới hạn ĐO ĐƯỢC của tài khoản — bản trước tính theo 12K TPM của `llama-3.3-70b-versatile`, một model **không có** trong danh mục tài khoản.)* Tiếng Việt tách token tệ hơn tiếng Anh (~2,5–3,5 ký tự/token), nên 4000 ký tự ≈ **1.150–1.600 token**. Cộng đáp án tham chiếu, đề bài và rubric, một request ≈ **2.500–3.500 token**. Ở `GROQ_MAX_CONCURRENCY = 2` mỗi burst ≈ **5.000–7.000 token**, **dưới** trần **8K TPM** của `qwen/qwen3.8-27b`. Trần **8000 ký tự vẫn KHÔNG lọt**: 2 × (~2.400 + ~1.200 + ~600) ≈ **8.400 token > 8K** — nên lập luận loại bỏ phương án "cứ nâng cho rộng" vẫn đứng, chỉ đổi con số.
...
Giá trị: **`qwen/qwen3.8-27b`** (kỹ sư chốt 2026-08-29, **đóng OQ-6**).

**OQ-6 đã nổ đúng như nó được viết ra.** Giá trị khởi điểm của v1.0 — `llama-3.3-70b-versatile` — **không có trong danh mục của tài khoản**. Danh mục đo được (tất cả ở 30 RPM):

| Model | RPD | TPM | TPD |
|---|---|---|---|
| `allam-2-7b` | 7K | 6K | 500K |
| `groq/compound`, `groq/compound-mini` | 250 | 70K | không giới hạn |
| `meta-llama/llama-prompt-guard-2-22m` / `-86m` | 14.4K | 15K | 500K |
| `openai/gpt-oss-120b`, `-20b`, `-safeguard-20b` | 1K | 8K | 200K |
| `qwen/qwen3.6-27b` | 1K | 8K | 200K |
| **`qwen/qwen3.8-27b`** | **1K** | **8K** | **2M** |

- **Vì sao `qwen/qwen3.8-27b`:** **TPD 2M của nó gấp MƯỜI LẦN mọi ứng viên còn lại**, và **TPD là trần ràng buộc trước tiên** ở đây (xem `GROQ_BUDGET_DAILY_LIMIT`). Qwen cũng mạnh về đa ngữ với tiếng Việt — ngôn ngữ của cả bài làm lẫn đáp án mẫu (A3).
- **`openai/gpt-oss-120b` bị loại** dù lớn hơn và nhiều khả năng suy luận theo rubric tốt hơn: TPD 200K chặn nó ở **~65 bài/ngày**. Ghi chú cũ vẫn đứng: có báo cáo cộng đồng rằng nó **bỏ qua** `response_format` dạng `json_schema` (§ References).
- **`groq/compound*` bị loại:** đó là **hệ thống agentic có tool use** — sai hình dạng cho một phán quyết JSON tất định — và RPD chỉ 250.
- **`allam-2-7b` bị loại:** hướng tiếng Ả Rập, và TPM 6K còn chật hơn.
- **Hai `meta-llama/llama-prompt-guard-*` bị loại:** chúng là **bộ phân loại tiêm chích**, không phải bộ chấm. (Chúng vẫn đáng quan tâm cho R9 — xem § Future Extensibility.)
- **Kỷ luật hằng số đã làm đúng việc của nó.** Rủi ro mà ADR-0006 và AC-032 nêu tên — danh mục free tier dịch chuyển — **đã xảy ra thật**, và vì tên model sống ở **một** hằng trong `lib/ai/models.ts` chứ không rải trong module phát, việc xử lý nó là **sửa một dòng**, không phải thiết kế lại. Nghĩa vụ AC-032 vẫn nguyên: **đổi giá trị hằng này bắt buộc chạy lại đánh giá đối kháng với provider thật (AC-070) và ghi lại kết quả có ngày tháng trước khi thay đổi ship** — bằng chứng chống tiêm chích là **theo từng model**.
- **Giới hạn còn lại, nói thẳng:** chưa ai chấm thử một bài tiếng Việt nào bằng model này. Lượt chạy AC-070 đầu tiên là phép đo đó.
...
|---|---|---|---|
| 1 | `submitExam()` (`features/exams/actions.ts`), ngay trước lượt gọi `computeScore()` | **Cổng hành vi** | Có phát năm khoá `essay*` hay không, và có đăng ký `after()` hay không |
| 2 | `retryEssayGrading()` (`features/exams/essayActions.ts`) | **Cổng hành vi** | Một lượt chấm lại có với tới được provider hay không khi tính năng đang tắt |
| 3 | **Segment trang làm bài** (`app/(exams)/exams/[id]/attempt/[attemptId]/page.tsx`) | **Cổng câu chữ** | **Chỉ** chọn giữa hai khoá i18n cho chân trang ô tự luận. Không chạm hành vi chấm |
...
Prop là **tuỳ chọn, mặc định `false`**, và tính tuỳ-chọn đó **gánh việc chứ không phải cho gọn**: `ExamPlayer` (`features/exams/components/ExamPlayer.tsx:29-41`) và `QuestionRenderer` (`:45-53`) hiện **không** khai prop nào như thế, nên một prop bắt buộc sẽ làm mọi chỗ dựng hiện có đỏ ở `tsc`. Tuỳ chọn + mặc định `false` giữ `features/exams/components/__tests__/ExamPlayer.test.tsx` **xanh mà không phải sửa**, và mặc định ấy trùng đúng với trạng thái ship (tắt).
...
5. **`verify-schema.ts` + làn service (`test:localdb`) — cổng, viết TRƯỚC khi dựa vào chúng.** Hai assertion grant, cổng trần ký tự (hai probe mã lỗi), cổng ghim trần lượt, và cases `EG-a…EG-e` trong `SOURCE/tests/e2e/service/essay-grade-write.service.e2e.test.ts`.
...
| Hai hàm SQL | **KHÔNG** — Postgres thật, qua `SOURCE/tests/e2e/service/essay-grade-write.service.e2e.test.ts` (`npm run test:localdb`) | Mock **không thể** chứng minh: thứ tự mảng được giữ, vị từ ghi-lần-đầu-thắng khớp 0 dòng, `42501` cho JWT học sinh, và `raise … check_violation`. Đây đúng là "Mock Limitations for Data Layer", và ở đây nó là một ranh giới bảo mật. |
...
- **Giới hạn của mock, ghi nhận:** một test đơn vị mock Supabase **không thể** bắt được ba thứ mà tính năng này phụ thuộc: (a) `jsonb_agg` không có `order by` xáo trộn mảng; (b) `WHERE … <> 'graded'` khớp 0 dòng thay vì ném; (c) `revoke`/`grant` thật sự đã được áp lên **cả hai** database. Cả ba chỉ Postgres thật mới chứng minh được: (a) và (b) nằm ở làn service (`npm run test:localdb`, cases `EG-c`/`EG-d`), (c) nằm ở `verify-schema.ts` cộng `EG-a`/`EG-b`.

### Integration Verification Points

- **`SOURCE/tests/e2e/service/essay-grade-write.service.e2e.test.ts`, cases `EG-a…EG-e`** — Postgres **thật**, chạy bằng **`npm run test:localdb`** (`vitest.localdb.config.ts`). Nghĩa vụ giữ **nguyên văn**; chỉ **nhà** của chúng đổi so với bản v1.0 (§ D-16). Ánh xạ do khung test đã chốt: **SVC-1 = `EG-c` + `EG-d`** (nửa settle), **SVC-2 = `EG-a` + `EG-b` + `EG-e`** (nửa claim + grant).
  - `EG-a` — JWT học sinh gọi `claim_essay_grading_attempt()` ⇒ `42501` (**không phải** một mã từ thân hàm). Phân biệt `42501` với `PGRST202` (chưa áp schema) và với mọi mã khác (`revoke` bị gỡ), đúng khuôn `MM-b` (`test-rls.ts:1652-1671`).
  ...
  - **`test-rls.ts` Phần 10 KHÔNG nhận bản sao thứ hai của năm ca này.** Hai nhà thì trôi lệch, và bản trôi lệch sẽ là bản **không ai chạy** — `test-rls.ts` không có npm script, `npm run test:localdb` thì có và là một trong sáu cổng trước commit.
  - **`S-b` (`test-rls.ts:1314-1320`) Ở NGUYÊN CHỖ CŨ.** Nó đã ship, đã chạy, và đã chứng minh JWT học sinh không `UPDATE` được `exam_results` trực tiếp. Nó **không** được gom về làn service cùng năm ca mới — ghi ra ở đây để một lượt "dọn cho gọn" sau này không gỡ một chứng cứ đang hoạt động ra khỏi chỗ nó đang hoạt động.
...
| 2 | **Đường ghi giữ mọi tính chất chứa của ADR-0010** | `npm run test:localdb` (cases `EG-a…EG-e`, Postgres thật) + `npm run verify:schema` trên **cả hai** database | Trước khi ship; lại sau mỗi lượt áp schema |
...
- **Khả năng đã hoãn, và điều kiện để mở lại:**
  - **`meta-llama/llama-prompt-guard-2-86m` như một tầng phân loại tiêm chích đứng TRƯỚC bộ chấm** — nó có trong danh mục tài khoản với **14.4K RPD**, và nó là bộ phân loại **chuyên dụng cho prompt injection**, tức là liên quan trực tiếp tới mô hình đe doạ R9 (bài làm đối kháng) mà PRD dựng cả § Security quanh nó. **Kỹ sư quyết định KHÔNG thêm bây giờ (2026-08-29)**, và lý do đáng giữ vì nó sẽ được hỏi lại: PRD đã phòng thủ bằng cách **validate nghiêm đầu ra của model** (AC-006/AC-041), nên một cú tiêm chích thành công **không dịch chuyển được một điểm số** — giá trị biên của một bộ phân loại thêm vào là nhỏ, trong khi chi phí thì không: nó **nhân đôi số request mỗi bài** (đụng thẳng vào chính TPM/TPD vừa buộc hạ đồng thời xuống 2), tạo **điểm phát thứ hai** với guard chokepoint và hạch toán ngân sách riêng của nó, và **mở lại một phạm vi đã được hoà giải qua năm tài liệu**. **Điều kiện mở lại:** khi R9 được xét lại — cụ thể là nếu một lượt chạy AC-070 cho thấy **band bị thổi phồng thật** (chứ không chỉ là rủi ro lý thuyết), hoặc nếu tập fixture đối kháng bắt đầu đỏ. Khi đó bộ phân loại này là ứng viên đầu tiên, và § Điểm phát Groq đã ghi sẵn giới hạn của nó.
...
| AC-036 | `GROQ_MAX_CONCURRENCY = 2` (§ Hằng số — cỡ theo TPM, § D-17) | — |
...
| AC-055 | § D-06 — **tám** chỗ ghép cặp (sửa ở v1.9), không phải hai | — |
...
> **Trạng thái chốt sổ 2026-08-30 (Final §16).** Bảy mục dưới đây được đọc lại sau khi Phase E chạy xong. **Sáu đóng, một còn mở.**
>
> | OQ | Trạng thái | Đóng bằng cái gì |
> |---|---|---|
> | **OQ-1** | **ĐÓNG 2026-08-30** (Task E5) | Đo thật, n = 18 lượt chấm. Round-trip **p50 671 ms / p95 1 289 ms** (min 503, max 1 289). Token: **873/request** trung bình (852 prompt + 22 completion; min/p50/max 814/874/922). **Không hằng thời gian nào dịch** — p95 cách `GROQ_CALL_DEADLINE_MS` (20 s) **15 lần**. Xem cảnh báo bên dưới: phép đo token này **lật lại lập luận** của OQ-4 và OQ-7 |
> | **OQ-2** | **ĐÓNG 2026-08-29** (Gate C) | `telemetry_log_event_type_check` — tên **và** định nghĩa **giống hệt nhau** trên cả hai database, đọc bằng truy vấn `pg_constraint` thật. Điều kiện leo thang ("hai project tên khác nhau") **không xảy ra** |
> | **OQ-3** | **ĐÓNG 2026-08-29** (Gate D) | Đo thật: **375 B/dòng** không có hai trường → **3 401 B/dòng** khi có (**≈9,1×**), tức ~183 KB → ~1 661 KB ở trần 500 dòng; dòng lớn nhất 5 385 B. **Quyết định của kỹ sư: ACCEPT.** Phương án RPC **không** được chọn, nên **Escalation 2 vẫn đóng** và số thay đổi schema áp tay giữ ở **hai** nhóm hàm/constraint |
> | **OQ-4** | **ĐÓNG 2026-08-29**, giá trị vẫn đúng, **lý do thì không** | `GROQ_BUDGET_DAILY_LIMIT = 600` vẫn an toàn, nhưng xem cảnh báo bên dưới |
> | **OQ-5** | **CÒN MỞ — chủ sở hữu: kỹ sư/sản phẩm** (Task E4) | Quyết định về `upload.essayStored`: (a) để nguyên, (b) sửa chuỗi cùng lượt bật cờ, (c) mở mục UI Spec cho `(authoring)`. **Không chặn ship**; nó chặn việc *coi bề mặt tác giả là đã đúng*. Chuỗi `upload.essayScored` đã tồn tại trong cả `en.ts` lẫn `vi.ts` và `QuestionEditor.tsx:410` đã chọn theo cờ, nên phương án (b) là **một dòng, đã lắp sẵn** — nhưng lựa chọn vẫn là của kỹ sư |
> | **OQ-6** | **ĐÓNG** — hai nửa, hai ngày | Nửa hằng số: 2026-08-29, `ESSAY_GRADER_MODEL = "qwen/qwen3.8-27b"`. **Nửa còn treo — chất lượng chấm tiếng Việt — nay cũng đóng**: lượt AC-070 đầu tiên chạy 2026-08-30 (Task E3) với provider thật, **7/7 EQUAL 0 RAISED** trên bộ đối kháng và **4/4 khớp** band người chấm mong đợi. Không đổi model ⇒ nghĩa vụ chạy lại của AC-032 **không** kích hoạt |
> | **OQ-7** | **ĐÓNG 2026-08-29**, kết luận vẫn đúng, **số học thì không** | Mục tiêu ≤ 3 phút giữ nguyên; xem cảnh báo bên dưới |
>
> **⚠️ CẢNH BÁO: phép đo token của OQ-1 lật lại lập luận đã ghi ở OQ-4 và OQ-7. Giá trị không đổi, nhưng LÝ DO thì sai, và một lý do sai là thứ phiên sau sẽ dựa vào để dịch con số.**
>
> - **OQ-4 nói TPD ràng buộc trước RPD.** Điều đó đúng ở ~3K token/request (2M ÷ 3K ≈ 660 < RPD 1K). Ở **873** token thật: 2M ÷ 873 ≈ **2 290 request/ngày**, nên **RPD 1 000 mới là cái ràng buộc trước**, không phải TPD. `600` vẫn nằm dưới **cả hai** nên giá trị không cần đổi — nhưng ai đọc lập luận cũ rồi nâng trần theo TPD sẽ nâng vượt RPD.
> - **OQ-7 nói 5 câu ≈ 15K token ⇒ tối thiểu ~1,9 phút.** Ở 873 token thật, 5 câu ≈ **4 365 token** ⇒ **~33 giây** ngân sách token, không phải ~1,9 phút. Mục tiêu ≤ 3 phút vì thế **rộng hơn nhiều** so với những gì tài liệu ghi.
> - **Và một trần chưa từng được ghi ở đâu trong tài liệu này: TPM 8 000.** Mọi lập luận đã ghi đều nói về trần **theo NGÀY**. Lượt E3 đầu tiên chết vì **HTTP 429 tokens-per-minute, limit 8 000** trên tier `on_demand`. Ở 873 token/request đó là **≈ 9,2 request/phút**, và **nó mới là cái ràng buộc một cụm burst**. Hệ quả cần quyết định: **50 câu tự luận ≈ 43 650 token ⇒ ≈ 5,5 phút**, so với `ESSAY_PASS_BUDGET_MS` = **4 phút** ⇒ một đề tự luận đầy **không thể xong trong một pass** trên tier này; phần đuôi settle thành chưa-ngã-ngũ và học sinh thấy nút chấm lại. `GROQ_MAX_CONCURRENCY = 2` **không cứu được** — ràng buộc là TPM, không phải song song. **Không hỏng** (chưa ngã ngũ ⇒ retry được, không bao giờ ra band sai) nhưng là hành vi thật đang chạy, và quyết định tier thuộc về kỹ sư.

- **OQ-1 — Hằng số dung lượng và thời gian: cái gì đã đo, cái gì còn là ước lượng.** **Đã đo (2026-08-29):** giới hạn free tier của tài khoản — 30 RPM, và với `qwen/qwen3.8-27b` là 1K RPD / 8K TPM / 2M TPD. **CÒN LÀ ƯỚC LƯỢNG, KHÔNG phải phép đo:** con số **~2.500–3.500 token mỗi request**, thứ mà **cả `GROQ_MAX_CONCURRENCY` (2) lẫn `GROQ_BUDGET_DAILY_LIMIT` (600) đều đứng lên trên**. Nói thẳng đúng tư thế tài liệu này đã dùng cho trần 4000 ký tự: đây là một phép suy từ tỉ lệ ký tự/token của tiếng Việt, không phải một lượt đếm token thật. *Thứ sẽ đo nó:* **lượt chấm thật đầu tiên**, ghi log số token **prompt + completion** mà nhà cung cấp trả về cho mỗi request. *Hệ quả nếu lệch:* nếu con số thật lệch đáng kể thì **cả hai hằng phải dịch theo** — TPM 8K ÷ token thật cho ra mức đồng thời an toàn, TPD 2M ÷ token thật cho ra trần ngày. **Vẫn chưa đo:** round-trip Singapore→Groq (C4), nên `GROQ_CALL_DEADLINE_MS` (20 s), `ESSAY_PASS_BUDGET_MS` (4 phút) và `ESSAY_PENDING_DEADLINE_MS` (10 phút) đều chọn bằng lập luận. *Đầu vào cần:* một phép đo trên dev — 10 lượt chấm thật, ghi p50/p95 độ trễ **và** token/request. *Người chịu trách nhiệm:* kỹ sư, trong lát V1. *Điều kiện leo thang:* p95 vượt 20 s ⇒ nâng `GROQ_CALL_DEADLINE_MS` **và** tính lại số học của `ESSAY_PASS_BUDGET_MS`; **hạn chờ đọc-lúc-render KHÔNG dịch theo** (AC-061) vì nó neo vào trần nền tảng chứ không vào độ trễ.
...
- **OQ-4 — ĐÃ ĐÓNG 2026-08-29 (v1.5): `GROQ_BUDGET_DAILY_LIMIT = 600` (đơn vị: **request**).** Suy ra từ giới hạn đo được của `qwen/qwen3.8-27b`: **TPD 2M ÷ ~3K token/request ≈ 660 request/ngày**, và con số đó **ràng buộc TRƯỚC** RPD 1K — nên cách đặt ngây thơ "cho bằng RPD" sẽ **cấp phép quá tay**. **600** chừa biên dưới **cả hai** trần, và quan trọng hơn: nó làm **lời từ chối của CHÍNH TA nổ trước lời từ chối của nhà cung cấp**, tức là học sinh nhận đường `project_budget_exhausted` sạch sẽ thay vì một cơn bão 429. Ở mức đặt chỗ xấu nhất 3 request/bài, đó là **~200 bài/ngày**; nếu chấm xong ngay lần đầu thì ~600 bài.
- **OQ-5 — Chuỗi `upload.essayStored` nói với tác giả đề rằng tự luận "chưa chấm tự động"** (§ D-11). Nó trở thành **sai** khi cổng AC-067 qua. D6 giữ bề mặt của tác giả không đổi và bốn màn hình của UI Spec không gồm `(authoring)`, nên tài liệu này **không** sửa nó. *Đầu vào cần:* quyết định của kỹ sư — (a) để nguyên và chấp nhận một câu sai trên màn tác giả, (b) sửa chuỗi cùng lượt bật cờ (một thay đổi i18n, không đụng cấu trúc), hay (c) mở một mục UI Spec cho `(authoring)`. *Người chịu trách nhiệm:* kỹ sư, trước khi bật cờ. *Điều kiện leo thang:* không chặn ship; chặn việc **coi bề mặt tác giả là đã đúng**.
- **OQ-6 — ĐÃ ĐÓNG 2026-08-29 (v1.5): `ESSAY_GRADER_MODEL = "qwen/qwen3.8-27b"`.** Rủi ro mà mục này ghi ra **đã xảy ra**: `llama-3.3-70b-versatile` **không có** trong danh mục của tài khoản. Danh mục đo được, lý do chọn, và lý do loại từng ứng viên nằm ở § Điểm phát Groq. Điều đáng giữ lại cho phiên sau: **kỷ luật "hằng số model hoán đổi được" là thứ biến sự cố này thành một lượt sửa một dòng** thay vì một lượt thiết kế lại — đúng công dụng mà ADR-0006 và AC-032 dựng nó ra. *Phần chưa đóng, chuyển tiếp:* chất lượng chấm tiếng Việt của model này **vẫn chưa được đo**; lượt chạy AC-070 đầu tiên (provider thật, bộ fixture đối kháng cộng một bộ nhỏ bài làm thật đã biết band mong đợi) là phép đo đó, và nó phải chạy **sau** cổng ZDR, **trước** khi bật cờ trên prod.
- **OQ-7 — ĐÃ ĐÓNG 2026-08-29 (v1.6): mục tiêu độ trễ của PRD nới từ ≤ 60 giây lên ≤ 3 phút cho lượt ≤ 5 câu.** *Chẩn đoán giữ nguyên:* với TPM 8K và ~3K token/request, 5 câu ≈ 15K token ⇒ **tối thiểu ~1,9 phút** chỉ riêng ngân sách token, trước khi tính một mili-giây độ trễ mạng hay model. Đây là trần của **nhà cung cấp**, không phải khuyết tật cài đặt — không có cách nào trong tài liệu này chạm tới nó. *Quyết định của kỹ sư:* phương án **(a) nới mục tiêu của PRD cho khớp thực tế đo được** — không phải (b) ghi nợ, không phải (c) trả tiền lên gói cao hơn. *Lý do được ghi lại:* **bốn bề mặt hiển thị đã ngụ ý lời hứa 60 giây với học sinh**, nên một chỉ tiêu mà nhà cung cấp **không thể đáp ứng về mặt vật lý** không phải một vấn đề tài liệu mà việc ghi nợ sẽ chữa được — nó là một lời hứa sẽ bị phá vỡ lặp đi lặp lại. *Ràng buộc đi kèm, mới:* chỉ tiêu **phải nằm dưới `ESSAY_PASS_BUDGET_MS` (4 phút)** — một chỉ tiêu cao hơn trần đồng hồ của chính lượt chấm là **không thể đạt theo cấu trúc**, vì người viết dừng trước khi chỉ tiêu có thể được chạm; 3 phút < 4 phút. Bất kỳ lần dịch nào của một trong hai số này phải kiểm số còn lại. *Ripple đã xử ở hạ nguồn, trong cùng lượt:* cận polling 120 giây của UI Spec dừng **trước** khi chấm xong trên lượt nhiều tự luận — với chỉ tiêu cũ đó là một **ngoại lệ**, với chỉ tiêu đúng thì nó là **kết cục mặc định**, tức dạy học sinh đọc một hệ thống đang chạy tốt thành hỏng; hai trần chuyển **18/120 giây → 30/240 giây** và đổi **mốc neo** sang `ESSAY_PASS_BUDGET_MS` (UI Spec v1.4, frontend DD v1.2). Chủ sở hữu vẫn là UI Spec / DD frontend; không hằng nào của tài liệu này bị đụng. *Còn mở, có chủ ý:* **~3K token/request vẫn là ước lượng, không phải đo đạc** — lượt chấm thật đầu tiên log token thật sẽ kéo **cả chỉ tiêu này lẫn `GROQ_MAX_CONCURRENCY` lẫn `GROQ_BUDGET_DAILY_LIMIT`** (OQ-1, Task E5).
...
- **Giới hạn free tier ĐO ĐƯỢC trên chính tài khoản Groq của kỹ sư (2026-08-29)** — thay thế các con số tra từ blog bên thứ ba mà v1.0 dùng, và là **nguồn chuẩn** cho `ESSAY_GRADER_MODEL`, `GROQ_MAX_CONCURRENCY` và `GROQ_BUDGET_DAILY_LIMIT`. Toàn bộ danh mục ở 30 RPM; `qwen/qwen3.8-27b` = 1K RPD / **8K TPM** / **2M TPD**. Bảng đầy đủ ở § Điểm phát Groq. **Bài học phương pháp, ghi lại:** `llama-3.3-70b-versatile` được ba nguồn bên thứ ba mô tả nhất quán là có trên free tier, và nó **không có trong tài khoản này** — danh mục của nhà cung cấp là chuyện **theo từng tài khoản**, nên chỉ đo trên chính tài khoản sẽ dùng mới là bằng chứng.
- [Groq Free Tier Limits 2026 — TokenMix](https://tokenmix.ai/blog/groq-free-tier-limits-2026) và [Groq API Pricing & Free Tier Rate Limits 2026 — Klymentiev](https://klymentiev.com/blog/groq-pricing) — **giữ lại làm bối cảnh lịch sử, KHÔNG còn là căn cứ cho hằng số nào.** Chúng là nguồn của con số 30 RPM (vẫn đúng) và của `llama-3.3-70b-versatile` (không áp dụng cho tài khoản này). Hình dạng "429 kèm header `retry-after`" từ hai nguồn này vẫn là căn cứ cho `GROQ_RETRY_MAX_WAIT_MS`.
...
|---|---|---|---|
| 2026-08-29 | 1.9 | **§ D-06: bảy chỗ ghép cặp → TÁM. Chỗ thứ tám là `SOURCE/lib/schema/__tests__/schemaFingerprint.test.ts`, và nó được tìm ra bằng THỰC THI chứ không bằng đọc danh sách.** Đó là test **duy nhất** trong repo thực sự **đọc** `schema.sql` và phân tích hai danh sách `error_code` trong đó; nới 6 → 9 làm đỏ ba khẳng định của nó (`:213`, `:231`, `:256`). **Ba chỗ mà D-06 CÓ nêu tên (`telemetry.test.ts :49/:265/:311`) đều XANH** — chúng so TypeScript với một bản chép tay và không bao giờ mở file SQL — nên bản liệt kê cũ **về nguyên tắc không thể bắt được** chỗ này. **Bằng chứng quyết định đã nằm sẵn trong repo từ trước tính năng này**: thông báo lỗi của chính khẳng định `:256` viết *“Sửa CẢ BA chỗ trong cùng một commit: hai danh sách SQL + `lib/tutor/telemetry.ts`”* — tức một cổng canh đã cưỡng chế luật một-commit trước khi tài liệu này được viết. **Hệ quả về lịch:** các literal telemetry phía TypeScript đã chuyển **vào commit của Task H5** (`2448179`) thay vì đợi Task B3.1; B3.1 thu lại còn phần nối call site ở `gradeEssays.ts`. Hai phương án bị loại, kèm chế độ hỏng có tên mà mỗi cái sẽ tái tạo: hoãn nới `error_code` sang B3.1 ⇒ sửa `schema.sql` lần hai, dịch vân tay lần hai và **áp tay lên cả hai database lần hai** — đúng hình dạng **TD-005** đã nổ bốn lần; chấp nhận cửa sổ đỏ ⇒ làn vitest **mặc định** đỏ suốt ~12 commit — đúng hình dạng **TD-030**. | Design Doc (Claude) |
| 2026-08-29 | 1.8 | **Thống nhất tên hàm dựng prompt: `buildEssayGradingPrompt` → `buildEssayPrompt`.** Tài liệu này là chỗ **duy nhất** trong cả chuỗi dùng tên dài: work plan (`:800`) và task file H3 (năm chỗ) đều viết `buildEssayPrompt`, và Task H3 đã cài đặt theo tên đó (commit `cc1ce75`). Sửa tài liệu cho khớp mã thay vì ngược lại, vì **bản thân cái tên không mang hợp đồng nào** — đổi mã theo tài liệu sẽ là một lượt sửa chạm vào hai module và hai file test để đổi lấy đúng một từ. **Phát hiện bởi chính executor của H3 và được nêu ra thay vì lặng lẽ chọn một bên** — đúng hình dạng doc-vs-code drift mà v1.5→v1.6 đã vấp một lần ở hằng dung lượng. Task B1.4 (consumer đầu tiên của hàm này) nay không còn hai tên để chọn. | Design Doc (Claude) |
