...
| 2026-08-16 | 1.1 | Implementation closed (Work Plan Phase 4 + Phase 5 manual passes + Final Phase Tasks 23-27). **One design gap found and fixed, in this document's own Accessibility Implementation scope:** UI Spec D5 requires the affordance button to be *removed* when the hint renders, and this document specified that removal without saying where focus goes. Measured on a real keyboard interaction (Phase 5 Task 19): the removed button was the element holding focus, so the browser dropped focus to `<body>` and the next Tab restarted at the top of the document — a keyboard user could never reach the hint they had just requested. Shipped fix: the hint panel is wrapped in a `tabIndex={-1}` container that takes focus on mount (`-1`, not `0`, so it receives focus programmatically without adding a permanent Tab stop), locked in by an assertion in `ExplainStepAffordance.test.tsx`'s D5 test. The wrapper is a plain `<div>` around `BentoCell` rather than a `ref` on `BentoCell` itself — that component is polymorphic (`as` = `div|li|section`), so declaring a `ref` prop there makes TypeScript intersect all three elements' prop types and no ref type satisfies all of them. **Confirmed as designed:** the flagged `explainStep()` argument-order risk held correct on the real round trip (`explainStep(attemptId, questionId)` read from the dev server log); the `busyRef` guard reduced two synchronous activations to exactly 1 Server Action call and 1 telemetry row; `SkillRecommendationCard`'s async-Server-Component test technique worked and its documented manual-only fallback was **not** needed. | Final Phase Task 27 (Claude) |
