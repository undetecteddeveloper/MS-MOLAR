...
| `TutorQuotaNote` | `useEntitlement()` at `components/billing/TutorQuotaNote.tsx:25` | nowhere at the time of writing; **now beside both `ExplainStepAffordance` call sites on the result-detail page** (UI-D17, frontend-owned) | n/a then; **Yes** now — the same `(layer2)` mount |
...
**No extra round trip, and no `React.cache()` (UI-D1).** Route groups are siblings under `SOURCE/app/`, not nested: a request resolves through exactly one of `(billing)`, `(layer2)`, `(layer4)`, so exactly **one** `readEntitlement()` call happens **on the render path** per request, no matter how many gated components render below it.

**The invariant is about the render path, and v1.11 says so explicitly because the unqualified form is false.** `readEntitlement()` has **five** production call sites, not three: the three layouts **plus** the two Server Action quota gates — `app/(layer2)/tutorActions.ts` and `app/(layer4)/actions.ts`, each calling it on the line above its `consumeQuota()` call. **Those two are sanctioned, not drift**, and the reason is structural rather than stylistic: a Server Action runs outside any React tree, so `useEntitlement()` has no context to read and there is no provider value to inherit. **The shipped guard that enforces ADR-0013 encodes exactly this split**: `app/(layer2)/__tests__/layout.test.tsx` walks the tree against two closed lists — a `RENDER_PATH_ALLOWED` list of the three layouts plus `readEntitlement.ts` itself, and a separate `SERVER_ACTION_ALLOWED` list naming `app/(layer2)/tutorActions.ts` and `app/(layer4)/actions.ts`, each entry carrying its own justification. A file only reaches the second list if its **first line of code** is the `"use server"` directive; anything else falls to the stricter render-path branch. The guard's stated reason is the one that matters here: **ADR-0013 forbids two *implementations* of a money-adjacent predicate, not two *call sites* of the single one.** The two alternatives it does forbid are the ones that would produce a second answer — deriving entitlement inside `consumeQuota()`, or trusting an entitlement the caller submits.
...
| ~~**E-02**~~ | **CLOSED at v1.11 — the proposed resolution shipped verbatim.** *(Statement of the original escalation, kept because it is what makes the closure checkable: "**`toCheckoutOrder()` changes a frontend-owned module.** `app/(billing)/queries.ts` is specified in `subscription-frontend-design.md` v1.1 with its own inline camelCase mapping, following four shipped precedents.")* | Was: two producers of one contract with two mappings is the I010 defect, but the second producer sat in another document's scope and this document could not edit it. **`design-sync` resolved it in the direction proposed** | `design-sync` — **discharged** | **Landed exactly as proposed.** `app/(billing)/queries.ts` imports `toCheckoutOrder` from `lib/billing/checkoutOrder` and `getMyOrder()` returns `toCheckoutOrder(row)`; the function kept its location, its signature and its `import "server-only"`, and `listMyOrders()` / `MyOrderRow` are untouched. **One mapper, two producers — the I010 defect is closed rather than accepted** |
