...
| **Version** | 1.12 |
| **Date** | 2026-08-20 |
| **Status** | Draft — **no blocking unresolved items *this document owns*.** B-01 was decided by the project owner on 2026-08-18 (see "Recorded Decisions"); FE-B-01 and FE-B-02 were closed in v1.3. **Two dependencies outside this document's ownership do gate its final step**, and v1.3's unqualified "no blocking unresolved items remain" was wrong to omit them: **UI Spec TBD-02** (legal content — `/terms` and `/refund-policy` still render `LegalContentPending`, and C-15 keeps the confirm control `aria-disabled` until it clears) and the manual `GEMINI_PAID_TIER_ENABLED` flip (R14/AC-048). Both are recorded under "What real money buys". **Three** items were **escalated** rather than decided here — **E-01** (a PRD amendment for AC-034's storage half), **E-02** (a cross-layer mapper change for `design-sync`, **CLOSED at v1.11**: the resolution it proposed shipped verbatim) and, new at v1.8, **E-03** (this document's own `quotaTracker` Non-scope-vs-analysis contradiction, withdrawn in favour of Non-scope, with a revision requested that **designs** the durable usage sink). None blocks the implementation this document designs; E-03 blocks one task outside it, work-plan Task 1.6, and BU-4 through it. **v1.6 is a documentation-hygiene pass** (plan Task 0.6): the stale "FE-B-01/FE-B-02 not yet acknowledged there" paragraph is corrected — the frontend Design Doc acknowledged both at its v1.4 — cross-document version pointers are refreshed, and this document's own v1.4 citation rule is applied to every remaining bare line number citing a concurrently-revised artifact. No design element moves and no contract changes. **v1.10 is the close-out pass** (plan Task 6.6). It adds § "Close-out sweep" — **57/57 ACs owned, 50 achieved, 7 deferred each with a named owner** (AC-040, AC-042, AC-043, AC-045, AC-048, AC-050, AC-055) — and § "BU-6 — state at close-out", which records **BU-6 as still open** with **Task 1.6 and BU-4** named as what it holds (chain: BU-6 → Task 1.6 → BU-4 → Task 6.8). It also repairs citations that rotted under real code movement — `telemetry.test.ts:261` → **`:311`** in nine places, `tutorActions.ts:51` → **`:55`**, `package.json:13`/`:12` → **`:16`/`:15`**, and `test-rls.ts` "Phần 8" → **`Phần 9`** — and corrects **OP-4**, the Consumer Parse Rule's "raw bytes" phrasing, which contradicted the Serialized Format cell on the same row. **The code was right in every one of these; only the prose was wrong.** No design element moves, no contract changes, and no acceptance criterion is added or removed. **v1.11 is the post-implementation verification fix pass, and every item in it is a *current-tense assertion that had become false*, not a moved line number.** Six corrections: `TutorQuotaNote` **is rendered** (both `ExplainStepAffordance` call sites on the result-detail page), not "rendered nowhere"; **S-05 and S-06 ship and both actions have shipped callers**, so the diagram's missing caller boxes are a scope statement rather than a state statement; **`readEntitlement()` has five production call sites, not three** — the three layouts plus the two Server Action quota gates, which are sanctioned by the shipped ADR-0013 guard and get their `Entitlement` by calling it directly, there being no React context in a Server Action, so the "exactly one per request" invariant is restated as the **render-path** invariant it always was and I2/I3 finally name their `Entitlement` producer; **escalation E-02 is CLOSED**, its proposed resolution having shipped verbatim (`getMyOrder()` returns `toCheckoutOrder(row)`); **`consumeQuota()`'s data-access boundary is documented** — it reads `subscriptions.period_anchor_at` and `user_profiles.created_at` on the period-anchor fallback and fails closed to `unavailable`, so Redis was never its only boundary and a Redis-only mock cannot exercise that branch; and **`lib/billing/quotaTelemetry.ts` is added to the Change Impact Map** with the constraint that forced it into its own module — both refusal sites are `"use server"` files and cannot export a shared constant. **The code is right in all six; only this document was wrong.** No design element moves, no contract changes, no acceptance criterion is added or removed, no history row is re-edited, and no file under `SOURCE/` is touched. **Two escalations remain open — E-01 and E-03**; E-02 is now closed. |
...
| | |
|---|---|
| **What changes** | 2 new tables (`payment_orders` carrying **four transfer-detail columns** added in v1.3 for FE-B-01) + 1 privileged SQL function; 1 new route handler (2nd in the app, first unauthenticated **write**); 1 payOS adapter; **1 existing entitlement stub body filled** (`readEntitlement.ts:34`) plus **2 new provider mounts** in `(exams)`/`(authoring)` layouts; **2** Redis counter families counting in **two different units** (v1.4, I004); 2 existing AI paths gated, both re-routed through one Gemini emit chokepoint; **1 existing CHECK constraint on `telemetry_log` altered in place** with 2 new error codes (v1.4, I005); **5** env vars registered in `checkEnv.ts`; 2 new `lib/billing/` modules (`pricing.ts`, `checkoutOrder.ts`). |
...
- The entitlement read path: fill `readEntitlement()`'s body; mount `EntitlementProvider` in `(exams)` and `(authoring)`.
...
| **A-3** | Sibling route groups resolve to **one** group layout per request, so exactly one `readEntitlement()` call happens per request. *(Load-bearing after v1.3 withdrew the false `getCurrentUserProfile()` precedent — D104.)* | Next.js App Router route groups: a folder in parentheses is excluded from the URL path, so `(billing)`, `(exams)`, `(authoring)` are siblings under `app/`, not nested, and a URL matches at most one — https://nextjs.org/docs/app/api-reference/file-conventions/route-groups. In-repo: `SOURCE/app/` contains seven parenthesised groups and five group layouts (`(history)`, `(billing)`, `(exams)`, `(analytics)`, `(authoring)`), all siblings; only `app/layout.tsx` is above them, and it holds no entitlement code | **Yes** |
...
| **`npm run test:integration`** → `vitest run --config vitest.integration.config.ts` (`package.json:12`), collecting `tests/integration/**` (**named here from v1.12**; through v1.11 the three manual lanes appeared in this document only inside close-out regression counts, never by script name, so a reader of this section could not run them) | The cases needing a **real dev Supabase**. Deliberately **outside `npm test` and outside CI** — "CI has no database" (§ Implementation Approach Phase 4); folded in, a missing credential would redden CI for the environment rather than for the code | **adopted** — manual, on dev |
| **`npm run test:fixture`** → `--config vitest.fixture.config.ts` (`package.json:14`), collecting `tests/e2e/fixture/**` with an **explicit `exclude` list** of the scripts there that carry no `test()` yet — one deleted line admits a file to the lane, and when the list empties the lane merges into `npm test` | Fixture-level end-to-end cases | **adopted** — manual, on dev |
| **`npm run test:localdb`** → `--config vitest.localdb.config.ts` (`package.json:13`), collecting `tests/e2e/service/**` | Service-level behaviour of **real Postgres** — the same class as the row above. **Has a blocking precondition stated in its own config header**: gate B must already be green on dev, or a failure here looks exactly like an implementation bug while being TD-005 drift | **adopted** — manual, on dev, **after** gate B |
...
Its rationale, in the code that enforces it today: *distinguishing the codes would leak that the server re-runs an eligibility check (`not_eligible`)* — `SOURCE/components/tutor/ExplainStepAffordance.tsx:96-99`. Concretely: the client-visible union stays exactly `"not_eligible" | "rate_limited" | "gemini_unavailable" | "server"` (`SOURCE/features/exams/tutorActions.ts:55`) and **all four keep rendering one message**. This is a **binding constraint, not an implementation preference** — a passing test does not license a weaker substitute, because what it protects is an absence of signal rather than a behaviour. **Its enforcer is plan Task 5.3** (`docs/plans/subscription-work-plan.md`, § Phase 5 — *"Gate the tutor path (I2), without reopening the eligibility-disclosure surface (UI-D3 / AC-041)"*), the change that adds `consumeQuota("tutor", …)` beside `guard("explainStep", …)` and is therefore the only change that could break it. **This documentation pass records the ownership and the constraint; it enforces nothing in code.**
...
| **AC-042** | **Code achieved; verification deferred.** The UI-D17 mount ships and the `(exams)` provider exists, so the note renders; the criterion's own proof is **FE-AC-26**, which a provider-wrapped unit test cannot discharge | **Engineer**, work-plan **Task 6.5** | One manual pass signed in as a user whose `tutor` quota is `known` |
...
|---|---|---|
| Privileged writes | `SOURCE/lib/supabase/service-role.ts` | **+3 narrow operations** (corrected v1.12, same undercount as the Change Impact Map's row): `recordPaymentSettlement` (`:451`) and `recordPaymentOrder` (`:512`) write; `readPaymentOrderForSettlement` (`:410`) **reads** — `settleOrder()` step 1, deliberately not owner-scoped because the webhook trigger has no session. All three keep the shape of `recordExamResult`/`recordSkillMastery` |
...
| Tutor gate | `SOURCE/features/exams/tutorActions.ts` | quota check added beside the existing `guard("explainStep", userId)` at `:175`, as `consumeQuota("tutor", userId, ent, 1)`; the refusal branch also writes the new telemetry code |
| Extraction gate | `SOURCE/features/authoring/actions.ts` | replaces the **DB-count** check `if ((count ?? 0) >= LIMITS.MAX_UPLOADS_PER_DAY)` at `:337`, which sits inside the `else` branch only; the per-user `guard("uploadExam", user.id)` at `:181` already covers both branches and stays. `consumeQuota("upload", userId, ent, metaCall ? 3 : 2)` ahead of the branch at `:268`; `metaCall`'s derivation (today `:417`) moves above it (v1.4, I004) |
...
| Layout wiring | `SOURCE/app/(billing)/layout.tsx:27` (existing call site) + **new** mounts in `SOURCE/app/(exams)/layout.tsx` and `SOURCE/app/(authoring)/layout.tsx` | see D005 below — today the provider is mounted in `(billing)` only, and every gated component renders outside it |
...
| `SOURCE/components/billing/TutorQuotaNote.tsx` | Shipped C-06 component (`:3`), reads `useEntitlement().tutor` at `:25`. **Rendered on the result-detail page** — imported and rendered beside **both** `ExplainStepAffordance` call sites in `app/(exams)/exams/[id]/attempt/[attemptId]/result/detail/page.tsx`, the UI-D17 mount the frontend Design Doc owns. *(Corrected at v1.11. Through v1.10 this cell read "**Rendered nowhere** — a repo-wide grep for `TutorQuotaNote` returns only its own definition", which was accurate when this analysis was written and stopped being accurate when the frontend mount landed; the same document's own AC-042 close-out row already said the note renders, so the two statements contradicted each other.)* | Not a backend deliverable, but it is a second consumer that will silently show `unknown` unless the provider covers wherever the frontend eventually mounts it. Recorded so the frontend Design Doc inherits the same provider-coverage constraint rather than rediscovering it. |
...
### MSA-3 — the `EntitlementProvider` mounts in `(exams)` / `(authoring)`
...
**Where the *mapping* lives, corrected at v1.11: in its own module, `SOURCE/lib/billing/quotaTelemetry.ts`, not beside either call.** Through v1.10 this section said the mapping sits beside the call in `tutorActions.ts`, and the § Change Impact Map's NEW-file list did not name the module at all. Both were wrong, and the reason the module exists is a **hard constraint this document never recorded**:

- **Both refusal sites are `"use server"` files.** `features/exams/tutorActions.ts` (tutor) and `features/authoring/actions.ts` (upload) each carry the Server Action directive, which requires **every export to be an async function**. Neither can export a shared constant, so neither can be the home of a table the other reads. The directive constrains **exports, not imports**, so a third, ordinary module is the only valid place to unify them.
- **The alternative it replaces is a silent divergence, not a duplication of typing effort.** With a literal copy on each side, `satisfies Record<…>` pins only the **key set** and the **value domain** — not the individual pairings. One side could carry `user_quota: "server"` and still compile while the other held the correct value, and the two tables would drift apart with nothing red.
- **It is not in `quota.ts` on purpose.** `consumeQuota()` answers in quota vocabulary and must not know about the `telemetry_log_error_code_check` CHECK vocabulary; the translation belongs to the layer holding the context, which is the same argument the paragraph above makes about the write.
- **`satisfies` pins both ends**: a future fourth refusal reason is a missing-key compile error, and a code outside `TELEMETRY_ERROR_CODES` is likewise — rather than a silent `null` in `error_code` discovered months later, after `buildTelemetryPayload()`'s runtime filter has swallowed it. `unavailable` deliberately has no code of its own: it is our own infrastructure failing, and a third literal would exceed R13's declared scope.
...
**The claim v1.3 made was false.** v1.3 said the post-create path and the cold-load path are "byte-identical **by construction**". They are not, because **construction is exactly what differs**. The database emits snake_case through PostgREST (`order_code`, `amount`, `pending_until`); `CheckoutOrder` is camelCase, renames `amount` to `amountVnd`, and types `pendingUntil` as an ISO 8601 **string**. Two independent mappings therefore exist — one inside `createOrder()`'s read-back, one in the frontend's `features/billing/queries.ts` (`subscription-frontend-design.md` § **Field Propagation Map (serialized boundaries)**, the `pendingUntil` row, puts it there — *"from `timestamptz`"* → *"ISO 8601 string prop"*) — and v1.3 pinned neither.
...
**Consequence to reconcile across layers — RECONCILED, and shipped as specified** *(v1.11)*. The frontend document's `queries.ts` was originally specified to do its own camelCase mapping, matching the four shipped route-group modules. Under this decision `getMyOrder()` still lives there and still returns `CheckoutOrder`, but its mapping step is `toCheckoutOrder(row)` rather than an inline object literal; `listMyOrders()`'s `MyOrderRow` is unaffected, being a different, list-shaped projection. **The shipped module does exactly that**: `features/billing/queries.ts` imports `toCheckoutOrder` and `getMyOrder()` returns it. This was flagged for `design-sync` as **E-02**, which is now **closed**. *(Through v1.10 this paragraph described the inline mapping as the current state and the change as pending agreement.)*
...
**Two things the diagram deliberately does not show.** `createOrder()` and `recheckOrder()` have **no caller box**: their consumer surfaces are S-06 and S-05, which are **frontend-scoped** and designed in `subscription-frontend-design.md`. **They are now built** *(v1.11)* — `app/(billing)/me/orders/page.tsx` and `app/(billing)/pricing/checkout/page.tsx` both ship, `PurchaseCta`'s handler calls `createOrder()`, and `RecheckOrderControl`'s handler calls `recheckOrder()` — **so the omission from the diagram is now a scope statement rather than a state statement**: the boxes are absent because the screens belong to the other document, not because they do not exist. The actions remain complete, testable Server Actions with the data contracts defined below. *(Through v1.10 this paragraph said the surfaces were "designed … but not built" and that the actions are "invoked in this phase only from tests and — for `recheckOrder()` — from the verification path described under 'What real money buys'"; both were true when written.)* *(Corrected in v1.3: v1.2 called them "deferred" and cited UI Spec `:38-39`; both are stale, see X-8. The version and the remaining line citation were refreshed again at v1.6, plan Task 0.6.)* And `ENT` feeds three route-group layouts, not one; see the Integration Point Map.
...
*(Table tense, stated at v1.11: the four rows below record the state **before** this decision, because that state is the argument for it. The `Provider above it` column is therefore the **pre-D005** answer, not the answer today — D005 shipped, and all three mounts exist. The column header read "Provider above it **today**" through v1.10, which made the problem statement read as a current-state claim.)*

| Consumer | Where it reads | Where it renders | Provider above it **before D005** |
|---|---|---|---|
| `ExplainStepAffordance` | `useEntitlement()` at `components/tutor/ExplainStepAffordance.tsx:52` | `app/(exams)/exams/[id]/attempt/[attemptId]/result/detail/page.tsx:17` (used at `:177`, `:230`) | **No** — `app/(exams)/layout.tsx` mounts `SiteHeader`/`BottomNav`/`SupportWidget` and no provider |
| Upload path (`extractAndAssemble` surface) | `app/(authoring)/upload/page.tsx` and `me/exams/*` | `app/(authoring)` | **No** — `app/(authoring)/layout.tsx` likewise has no provider |
| `TutorQuotaNote` | `useEntitlement()` at `components/billing/TutorQuotaNote.tsx:25` | nowhere at the time of writing; **now beside both `ExplainStepAffordance` call sites on the result-detail page** (UI-D17, frontend-owned) | n/a then; **Yes** now — the same `(exams)` mount |
| `PlanComparison` | `useEntitlement()` at `features/billing/components/pricing/PlanComparison.tsx:57` | `(billing)` | Yes |
...
**Selected fix: mount the provider in `(exams)/layout.tsx` and `(authoring)/layout.tsx`, mirroring `(billing)/layout.tsx:27,33` line for line.** Both layouts already `await getCurrentUserProfile()` (`(exams)/layout.tsx:18` — *corrected from `:19` in v1.3, D105* — and `(authoring)/layout.tsx:12`), so the user id the read needs is already in hand and the change is one `await readEntitlement(user?.id ?? null)` plus one wrapping element per file.

**No extra round trip, and no `React.cache()` (UI-D1).** Route groups are siblings under `SOURCE/app/`, not nested: a request resolves through exactly one of `(billing)`, `(exams)`, `(authoring)`, so exactly **one** `readEntitlement()` call happens **on the render path** per request, no matter how many gated components render below it.

**The invariant is about the render path, and v1.11 says so explicitly because the unqualified form is false.** `readEntitlement()` has **five** production call sites, not three: the three layouts **plus** the two Server Action quota gates — `features/exams/tutorActions.ts` and `features/authoring/actions.ts`, each calling it on the line above its `consumeQuota()` call. **Those two are sanctioned, not drift**, and the reason is structural rather than stylistic: a Server Action runs outside any React tree, so `useEntitlement()` has no context to read and there is no provider value to inherit. **The shipped guard that enforces ADR-0013 encodes exactly this split**: `app/(exams)/__tests__/layout.test.tsx` walks the tree against two closed lists — a `RENDER_PATH_ALLOWED` list of the three layouts plus `readEntitlement.ts` itself, and a separate `SERVER_ACTION_ALLOWED` list naming `features/exams/tutorActions.ts` and `features/authoring/actions.ts`, each entry carrying its own justification. A file only reaches the second list if its **first line of code** is the `"use server"` directive; anything else falls to the stricter render-path branch. The guard's stated reason is the one that matters here: **ADR-0013 forbids two *implementations* of a money-adjacent predicate, not two *call sites* of the single one.** The two alternatives it does forbid are the ones that would produce a second answer — deriving entitlement inside `consumeQuota()`, or trusting an entitlement the caller submits.

These two call sites also do not violate the round-trip argument this section makes: a Server Action invocation is a *separate* request from the render that produced the button, so it is one entitlement read for that request too, not a second read stacked on a render. Components below read React context, which is what `entitlement.tsx:11-16` already argues for at length — a per-component fetch would put a round trip behind every gated element, which is the thing UI-D1 and the PRD's non-functional section (`subscription-prd.md:368`) forbid. `React.cache()` is genuinely absent: the only occurrence of the string in `SOURCE` is the comment at `entitlement.tsx:11` saying so.

**The precedent this argument used to cite was false, and is replaced rather than repaired (corrected in v1.3, D104).** v1.1/v1.2 wrote: *"the same shape `getCurrentUserProfile()` already has: called once per route-group layout, three times in the codebase, never memoised."* Both numbers are wrong, and so is the invariant. `getCurrentUserProfile()` has **seven** call sites in `SOURCE`: five route-group layouts — `app/(billing)/layout.tsx:23`, `app/(history)/layout.tsx:13`, `app/(exams)/layout.tsx:18`, `app/(analytics)/layout.tsx:12`, `app/(authoring)/layout.tsx:12` — plus **`app/(analytics)/profile/page.tsx:37`**, a page that re-calls it *beneath its own route-group layout*, and **`app/page.tsx:27`**, a page with no route-group layout above it at all. So "once per route-group layout" is not a property this repository exhibits, and the file's own hot-path note (`lib/auth/getCurrentUser.ts:89-90`, "6 layout route-group + app/page.tsx") counts one layout more than exists.

**The conclusion is unaffected, because it never depended on that precedent.** "Exactly one `readEntitlement()` per request" follows from Next.js route-group resolution alone — sibling groups, one matching layout — not from how any other function is called. What the corrected precedent does add is a discipline, stated here because `profile/page.tsx:37` shows the repo does not enforce it by itself: **no page or component *on the render path* below these layouts may call `readEntitlement()`**; they read context. A second call there would not be wrong, only wasteful, and it is exactly the drift `profile/page.tsx` already exhibits for the user read.

**The discipline is scoped to the render path, and v1.11 states the scope because the unscoped form forbids something the design requires.** A **Server Action** is not on the render path and has no context to read, so the two quota gates — `tutorActions.ts` and `actions.ts` — call `readEntitlement()` directly and must. The rule as written through v1.10 would have condemned them, and an implementer obeying it literally had only two exits, both worse: derive entitlement inside `consumeQuota()`, or accept an entitlement the caller submits — the second inside a function whose whole premise is that nothing the caller claims is trusted. **A rule whose only compliant path does not exist in the file being judged is an over-broad rule, not a violation.** The shipped guard test encodes both lists, so the boundary is enforced rather than merely stated.
...
| Selected: mount in `(exams)` + `(authoring)` | 0 | no | 0 | 1 | Smallest change that makes the shipped hook return real data; reuses the existing user read in both layouts. |
...
**`consumeQuota()` touches Postgres as well as Redis, and v1.11 documents that boundary because no section did.** The signature above and § Test Boundaries both describe Redis as this function's only external dependency. It is not: on the **period-anchor fallback**, `consumeQuota()` reads the database directly.

| When | What it reads | Client | On failure |
|---|---|---|---|
| `ent[kind].state === "known"` | nothing — the period start is derived from the entitlement's own `resetsAt` | — | — |
| `ent[kind].state !== "known"` (the fallback) | `subscriptions.period_anchor_at` and `user_profiles.created_at` for the user, in one `Promise.all` through the **request-scoped** Supabase client | session client, under RLS | **Fails closed to `null`** — either query erroring, or a missing `created_at`, yields no period, and the caller path refuses rather than guessing a window |

**Why the second branch exists at all**: an `unknown` quota means the read path could not establish the user's period, and the enforcement path cannot skip a period boundary just because the read path degraded — without one, a counter key has no epoch and a user gets an unbounded window. So the write path re-derives the anchor from the two columns that define it.

**What this changes and what it does not.** It does not change the refusal contract: `unavailable` still means "we could not determine eligibility, therefore refuse" (AC-024), and the fallback failing simply lands there. It does change the honest description of the boundary — **the enforcement path has two external dependencies, Redis and Postgres**, and a test that mocks only Redis is not exercising the fallback branch at all.
...
What the code actually does (`SOURCE/features/authoring/actions.ts`):
...
Direct Impact:
  ...
  - SOURCE/app/(exams)/layout.tsx               # NEW: readEntitlement + EntitlementProvider mount
  - SOURCE/app/(authoring)/layout.tsx               # NEW: readEntitlement + EntitlementProvider mount
  - SOURCE/features/exams/tutorActions.ts          # consumeQuota beside guard() at :175
  - SOURCE/features/authoring/actions.ts               # consumeQuota before branch at :268; delete DB count at :331-343
  ...
  - SOURCE/lib/supabase/service-role.ts          # THREE operations, not two (corrected v1.12 — the same enumerated-list
                                                 #   defect fixed for quotaTelemetry.ts at v1.11):
                                                 #   +recordPaymentSettlement (:451)
                                                 #   +recordPaymentOrder (:512) — the creation insert: clients are revoked
                                                 #     from INSERT, so the row is written server-side
                                                 #   +readPaymentOrderForSettlement (:410) — settleOrder step 1, the
                                                 #     pre-verification read of the order row; described in prose as step 1
                                                 #     of § lib/billing/settleOrder.ts but never enumerated here, so the list
                                                 #     undercounted the privileged surface by one operation
  ...
  - NEW: SOURCE/lib/billing/quotaTelemetry.ts    # v1.11 — the OK-04 mapping (3 consumeQuota refusal reasons ->
                                                 #   telemetry_log.error_code), as ONE shared const. See below:
                                                 #   this file was missing from this list through v1.10.
...
Indirect Impact:
  ...
  - SOURCE/components/billing/TutorQuotaNote.tsx       # unchanged BY THIS DOCUMENT; rendered on the
                                                       # result-detail page by the frontend's UI-D17 mount
                                                       # (v1.11 — through v1.10 this read "still rendered
                                                       # nowhere", which the mount had already falsified)
  - SOURCE/features/billing/components/pricing/PlanComparison.tsx  # unchanged; :57 now sees a real plan
...
No Ripple Effect:
  ...
  - SOURCE/app/(auth)/*, (analytics)/*, (admin)/*, (history)/*   # no gated component renders there
```
...
|---|---|---|---|---|---|
| I1 | `(exams)/layout.tsx` / `(authoring)/layout.tsx` | mount `EntitlementProvider`, mirroring `(billing)/layout.tsx:27,33` | **High** — without it every gated component reads `FREE_FALLBACK` | In: `user?.id ?? null` (already resolved by `getCurrentUserProfile()`). Out: `Entitlement`, sync within the server render. On error: `readEntitlement` never throws to the layout; a Redis failure degrades the quota fields to `unknown` (UI-D2) and the layout still renders. | Render test per layout asserting a gated child sees `plan`/`tutor` from the provider, not `FREE_FALLBACK` |
| I2 | `tutorActions.ts:188` (`guard("explainStep", userId)`) | add `consumeQuota("tutor", …)` beside it | High — refuses before the Gemini call | In: `userId`, `Entitlement`. **The `Entitlement` producer, named at v1.11 because this row listed the input with no source**: the Server Action calls **`readEntitlement(userId)` directly**, on the line above `consumeQuota()`. It cannot read the provider — `useEntitlement()` is React context and a Server Action has no React tree to read it from. Out: `{ok:true}` or `{ok:false, reason}`. On error: `unavailable` ⇒ refuse (AC-024) | Unit test: `ok:false` ⇒ zero `callTutor` invocations |
| I3 | `actions.ts:181` (`guard("uploadExam", user.id)`) and the branch at `:268` | add `consumeQuota("upload", …)` ahead of the branch; delete `:331-343` | High — changes the counting basis for re-runs | as I2 — including the `Entitlement` producer: this action likewise calls **`readEntitlement(user.id)` directly**, immediately above its `consumeQuota()` call | Test both branches (`rerunExamId` set and unset) consume exactly one unit |
...
| I7 | **S-05 `/me/orders` / S-06 `/pricing/checkout`** (UI Spec § Screen List, both rows marked **Implement**; routes **frozen** by **UI-D11**; **frontend scope**, designed in `subscription-frontend-design.md`) | consume `createOrder()` / `recheckOrder()` and the `orders_select_own` RLS read | **High for the contract; the code has since landed** *(v1.11)* — both routes ship and both actions have shipped callers, so this is now a live integration rather than a deferred one. *(Through v1.10 this cell read "**High for the contract, deferred for the code** — no screen code in this phase".)* The contract obligations below are unchanged | In: nothing / `orderCode`. Out: the full **`CheckoutOrder`** (v1.3, TBD-07 closed) and `SettleResult` respectively. **Ownership: `recheckOrder()` answers `unknown_order` for an order the caller does not own, byte-identical to a nonexistent one, with zero provider calls** (FE-B-02). On error: discriminated `reason`, never an exception across the action boundary | This document ships the actions + their contract tests, including the FE-B-02 negative test; the **screens** are the frontend document's |
...
| I8 | `scripts/check-ai-key-bundle.mjs` (run via `npm run check:bundle`, `package.json:15`) | +payOS secret markers | Medium | In: build output. Out: non-zero exit if a secret literal appears client-side | **`SOURCE/lib/security/checkAiKeyBundleSecrets.test.ts`** (corrected v1.12). "Existing script's own gate" was **tautological on CI**: the *value* half of the scan only runs when `read(name)` resolves, and the `Check bundle` step of `ci.yml` carries no `env:` block while `.env.local` does not exist there, so all seven entries are `undefined` and the value branch runs **0 of 7** times — `markers` is the entire net. Nothing imported `SECRETS`, so deleting a marker (or a whole entry) kept every gate green: fewer markers can only make the scan *pass* more easily. The test pins the seven labels and each label's marker array **verbatim**, in the shape `publicPaths.test.ts` uses for `PUBLIC_PATHS`, so both omission and addition fail. It lives under `lib/security/` for a mechanical reason: `vitest.config.ts` collects only `lib/**`, `components/**`, `app/**` — `scripts/**` is outside every lane's glob |
...
|---|---|---|---|---|
| `createOrder()` → `{ qrCode, orderCode, expiresAt }` (TBD-07's declared shape) | `createOrder()` → the full **`CheckoutOrder`** (eight fields) | **Yes** — three fields are renamed at the boundary (`qrCode` → `qrPayload`, `expiresAt` → `pendingUntil`, and `amount` appears as `amountVnd`) and five are added | **No adapter was ever needed, and the reason has changed** *(v1.11)*. **When this row was written the action had no shipped caller**; it now has one — `PurchaseCta`'s handler awaits `createOrder()`, exactly as the frontend document specified, and `RecheckOrderControl`'s handler awaits `recheckOrder()`. **No adapter is required all the same**: the caller was written against the widened eight-field `CheckoutOrder` from the start, so there was never a narrow consumer to migrate. *(Through v1.10 this cell asserted in the present tense that the action has no shipped caller and that "the frontend document … has not been built".)* | None needed. Recorded so a reader does not look for a compatibility shim that would be dead code. The **frontend** document must adopt the new shape; flagged for `design-sync` |
...
| `consumeQuota` | **Redis *and* Postgres** *(corrected at v1.11 — this row named Redis alone through v1.10)*. Redis on every call; **Postgres only on the period-anchor fallback**, when `ent[kind].state !== "known"` and the two anchor columns must be re-read | Fail-closed on unavailability is the claim — **on both dependencies**. A Redis-only mock leaves the fallback branch unexercised, so the Postgres half of that claim is untested by construction; the branch needs a real-database lane or an explicit Supabase-client double |
...
**Which command runs which row (named from v1.12).** Every row above that says *"nothing — real Postgres"* sits outside `npm test`, because `vitest.config.ts` collects only `lib/**`, `components/**`, `app/**`. Through v1.11 this section never said what *does* run them, and the three lanes appeared in this document only inside close-out regression counts. They are:

| Command | Collects | Holds, for this feature |
|---|---|---|
| `npm run test:integration` (`package.json:12`) | `tests/integration/**` | `subscription.int.test.ts`, `telemetryDistinguishability.int.test.ts` — settlement idempotency, the ownership refusals, AC-047 |
| `npm run test:localdb` (`package.json:13`) | `tests/e2e/service/**` | `subscription.service.e2e.test.ts` — service-level behaviour against real Postgres. **Run gate B on dev first**; its config header states the precondition and the reason |
| `npm run test:fixture` (`package.json:14`) | `tests/e2e/fixture/**`, minus a named `exclude` list | `subscription.fixture.e2e.test.ts` |

All three are **manual, dev-only, and deliberately outside CI** — "CI has no database" (§ Implementation Approach Phase 4). A row above that names a mocked boundary is a `npm test` row; a row that refuses to mock one is a row from this table, and reading the first as though CI ran it is exactly the mistake this paragraph exists to prevent.
...
| **B — DB-side** | `npm run verify:schema` (`package.json:16` → `npx tsx supabase/verify-schema.ts`), run from `SOURCE/`, once per environment | The **real database**, through production credentials. **Nine** checks (`verify-schema.ts:11-23`), not eight — item **9** (`:20-23`) was added after implementation and is the money-object probe group: `payment_orders`, `subscriptions` and `record_payment_settlement`, one **refused** write per object, each table probe followed by a `service_role` post-check under a *different* predicate than the write used (`memo` / `period_anchor_at`), and, on the failure branch, a marker-scoped sweep so a diagnostic run cannot leave a live row behind. The two that matter for the DDL-drift claim are **item 6** — `on delete` of *every* foreign key read from the live catalog through the §16a RPC (`:444-457`) — and **item 7** — which build of `schema.sql` the DB is actually running, via the §17 fingerprint (`:17-18`) | The DDL being correct in git and **absent from a database** — precisely the TD-005 failure that occurred 2026-08-04 → 08-07, which gate A cannot see because the file is right | After each hand-apply, on **dev first, then prod** |
...
**Revisited after item 9 shipped.** Through v1.10 this document said gate B observed the money objects *only* through the fingerprint. That is still exactly true of **item 7**, and the sentence above stands as written about it — but it is no longer true of **gate B as a whole**. Item 9 asks the live database three behavioural questions about those objects and does not read the file to do it: an `authenticated` session's `INSERT` into `payment_orders` and into `subscriptions` must each be refused *by permission* (42501 or an RLS-policy violation, distinguished from a data-constraint refusal so that a missing `revoke` cannot hide behind `23502`/`23503`), and `record_payment_settlement` must be un-callable with a student JWT. A hand `grant insert` applied on the dashboard after the paste — the exact drift the fingerprint is blind to, because the file still matches itself byte for byte — now turns gate B red. What item 9 still does **not** cover is the *positive* half: that settlement writes the right values. That remains the real-Postgres test below, and no fingerprint or permission probe substitutes for it.

**One consequence for the sequence above.** The "gate B green on **prod**" half of the sequence is currently **blocked**, and not by anything about the DDL: `verify-schema.ts`'s `signInProbeUser()` *creates or password-resets* a probe account whose email and password are literals in committed source, so pointing this lane at production would provision a known-password confirmed account in the production auth tenant. The script now says so at the head of item 9. Gating that branch is an open decision recorded in the work plan's register alongside plan **Task 5.8**; until it is taken, gate B runs on **dev only**. The design instruction is unchanged — prod still must not receive a deploy against unverified DDL; what is open is the *mechanism* by which prod gets verified.
...
| ~~**E-02**~~ | **CLOSED at v1.11 — the proposed resolution shipped verbatim.** *(Statement of the original escalation, kept because it is what makes the closure checkable: "**`toCheckoutOrder()` changes a frontend-owned module.** `features/billing/queries.ts` is specified in `subscription-frontend-design.md` v1.1 with its own inline camelCase mapping, following four shipped precedents.")* | Was: two producers of one contract with two mappings is the I010 defect, but the second producer sat in another document's scope and this document could not edit it. **`design-sync` resolved it in the direction proposed** | `design-sync` — **discharged** | **Landed exactly as proposed.** `features/billing/queries.ts` imports `toCheckoutOrder` from `lib/billing/checkoutOrder` and `getMyOrder()` returns `toCheckoutOrder(row)`; the function kept its location, its signature and its `import "server-only"`, and `listMyOrders()` / `MyOrderRow` are untouched. **One mapper, two producers — the I010 defect is closed rather than accepted** |
...
| Webhook silence from a misconfigured key looks like normal quiet | Medium | Accepted (ADR-0014); R10 is the primary recovery path by design, so silence degrades rather than blocks. **R10's screen (S-05) has since shipped** *(v1.11)*: `/me/orders` renders the order list with a per-row re-check control, so the recovery path is now genuinely user-reachable and the webhook is **no longer the sole settlement trigger** — which is the state ADR-0014 assumed. *(Through v1.10 this cell said S-05 "is deferred and frontend-scoped", concluded that "until it ships the recovery path exists only as a callable Server Action", and instructed that the webhook "must therefore be treated as the sole *user-reachable* settlement trigger in this phase". That instruction is superseded by the screen landing; the acceptance of the risk is not, because a user still has to notice and act.)* I7 records the dependency |
...
|---|---|---|---|
| 2026-08-20 | 1.12 | **Closing round on the re-verification findings — four corrections, all of them *this document* being wrong about shipped code, plus one consequence of a same-round source change. No design element moves, no contract changes, no DDL changes, no acceptance criterion is added or removed, no history row is re-edited, and `SOURCE/lib/billing/types.ts` is untouched.** **(1) Gate B runs NINE checks, not eight.** § Verification Strategy said *"Eight checks (`verify-schema.ts:11-19`)"*; the header list now runs to item **9** at `:20-23`, the money-object probe group. The count and the citation are corrected and item 9 is described. **The adjacent fingerprint claim is revisited rather than deleted**: item 7 still proves only that the DB runs this build of the file — that sentence stands — but the wider reading, that gate B observes the money objects *only* through the fingerprint, is now false. Item 9 asks the live database three behavioural questions (`INSERT` into `payment_orders` and `subscriptions` must be refused **by permission**, distinguished from a data-constraint refusal; `record_payment_settlement` un-callable with a student JWT), so a hand `grant insert` applied on the dashboard after the paste — invisible to a byte-identical fingerprint — now turns gate B red. What item 9 does **not** cover is the positive half, that settlement writes the right values; that remains the real-Postgres test. **(2) `service-role.ts` gains THREE operations, not two.** Both enumerations were short by one: the Change Impact Map listed `+recordPaymentSettlement, +recordPaymentOrder`, and § Existing Codebase Analysis said *"+1 narrow operation"*. `readPaymentOrderForSettlement()` (`:410`) is absent from both while being described in prose as `settleOrder()` step 1. **This is the identical enumerated-list defect v1.11 fixed for `quotaTelemetry.ts`, and it is fixed the same way** — the operation is named in the list, with its line and the reason it is privileged (not owner-scoped, because the webhook trigger has no session). **(3) I8's coverage cell named a tautology.** *"Existing script's own gate"* cannot fail on CI: the `Check bundle` step of `ci.yml` carries no `env:` block and `.env.local` does not exist there, so `read(name)` is `undefined` for **all seven** entries and the value-matching branch runs **0 of 7** times. Nothing imported `SECRETS`, so deleting a marker kept every gate green — fewer markers only make the scan pass more easily. The real coverage, **`SOURCE/lib/security/checkAiKeyBundleSecrets.test.ts`**, is named: it pins the seven labels and each marker array verbatim, in the `publicPaths.test.ts` shape, catching both omission and addition. **(4) The three manual lanes are named by script.** `test:integration`, `test:fixture` and `test:localdb` appeared in this document **only** inside close-out regression counts — neither § Quality Assurance Mechanisms nor § Test Boundaries mentioned them, so a reader of either section could not run the rows that say *"nothing — real Postgres required"*. Both sections now name the commands, their `package.json` lines, what each collects, and that all three are manual and dev-only because "CI has no database"; `test:localdb`'s blocking precondition (gate B green on dev first, or a TD-005 drift reads as an implementation bug) is recorded with it. **(5) One consequence, not a correction: the "gate B green on prod" half of the sequence is now blocked.** `verify-schema.ts`'s `signInProbeUser()` creates-or-password-resets a probe account whose credentials are literals in committed source, so pointing that lane at production would provision a known-password confirmed account in the production auth tenant. The script now says so at the head of item 9, the design instruction is unchanged, and the gating decision is registered in the work plan beside plan Task 5.8. **Two escalations remain open — E-01 and E-03.** | Post-verification closing round (Claude) |
| 2026-08-20 | 1.11 | **Post-implementation verification fix pass — six current-tense assertions that had become false. Every one is a *state claim*, not a moved line number; the shipped code is correct in all six and only this document changed. No design element moves, no contract changes, no DDL changes, no acceptance criterion is added or removed, no source file is touched, and no history row is re-edited.** **(1) `TutorQuotaNote` is rendered.** § Existing Codebase Analysis said *"**Rendered nowhere** — a repo-wide grep for `TutorQuotaNote` returns only its own definition"*, and the NEW-file list said *"still rendered nowhere"*. It is imported and rendered beside **both** `ExplainStepAffordance` call sites on the result-detail page — the frontend's UI-D17 mount. **This document already contradicted itself**: its own AC-042 close-out row said the note renders. Also corrected: D005's consumer table carried a *"Provider above it **today**"* column stating the **pre-D005** answer, which read as a claim about the present. **(2) S-05 / S-06 ship and both actions have shipped callers.** § Architecture Overview said the two consumer surfaces are *"designed … but not built"*, I7 rated the code *"deferred"*, § Interface Change Impact said `createOrder()` has *"no shipped caller"*, and § Risks said S-05 *"is deferred"* so the webhook must be treated as the sole user-reachable settlement trigger. `PurchaseCta` calls `createOrder()`, `RecheckOrderControl` calls `recheckOrder()`, and both routes ship. **The risk acceptance stands; the instruction built on the false premise does not.** **(3) `readEntitlement()` has FIVE production call sites, not three, and the invariant is a render-path invariant.** *"Exactly one `readEntitlement()` call happens per request"* and *"no page or component below the layouts may call `readEntitlement()`"* are true **of the render path** and false as written: `features/exams/tutorActions.ts` and `features/authoring/actions.ts` each call it directly, above their `consumeQuota()` call. **Those two are sanctioned, and the shipped guard test encodes the split** — `app/(exams)/__tests__/layout.test.tsx` holds a `RENDER_PATH_ALLOWED` list (three layouts + the defining module) and a separate `SERVER_ACTION_ALLOWED` list naming exactly those two files, each with its own justification, admission gated on a real `"use server"` directive prologue. **ADR-0013 forbids two *implementations* of a money-adjacent predicate, not two *call sites* of the one implementation**; the two exits the unscoped rule would have forced — deriving entitlement inside `consumeQuota()`, or trusting a caller-supplied entitlement — are precisely what it forbids. **The invariant is restated rather than deleted**, and integration points **I2/I3**, which listed `Entitlement` as an input with **no producer**, now name it. **(4) Escalation E-02 is CLOSED.** It was still recorded as open, and § S-06's read path still described the frontend's inline camelCase mapping as current. The proposed resolution shipped verbatim: `features/billing/queries.ts` imports `toCheckoutOrder` and `getMyOrder()` returns it, keeping its location, signature and `import "server-only"`, with `listMyOrders()` untouched. **One mapper, two producers — I010 closed, not accepted.** **(5) `consumeQuota()` touches Postgres, and no section said so.** The signature block and § Test Boundaries both named Redis as its only boundary. On the **period-anchor fallback** — reached when `ent[kind].state !== "known"` — it reads `subscriptions.period_anchor_at` and `user_profiles.created_at` through the request-scoped client and **fails closed to `unavailable`**. A table of the two branches is added, and the Test Boundaries row is corrected with the consequence stated: **a Redis-only mock leaves that branch unexercised**. **(6) `lib/billing/quotaTelemetry.ts` was absent from the Change Impact Map**, and § R13 said the OK-04 mapping *"lives beside the call in `tutorActions.ts`"*. It lives in its own module **because both refusal sites are `"use server"` files**, and that directive constrains exports — neither can export a constant the other reads, so a third ordinary module is the only valid unification point. The constraint, and what the module replaces (two literal copies that `satisfies Record<…>` would let drift pair-by-pair while still compiling), are recorded. | Post-implementation verification fix cycle (Claude) |
