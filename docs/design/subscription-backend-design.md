...
- **UI Spec**: `docs/ui-spec/subscription-ui-spec.md` (**v1.2**, `:5`) — C-01 `EntitlementProvider`/`useEntitlement`, UI-D1 (server reads once per layout), UI-D2 (three-valued quota). *Corrected in v1.3 (X-8): this document cited **v1.1**, three of whose line numbers had moved.* v1.2 (`:16`) moves **S-05 and S-06 from Defer to Implement** and **UI-D11 (`:181`) freezes both routes under `SOURCE/app/(billing)/`** — `/me/orders` and `/pricing/checkout?order={orderCode}` (`:395-396`, `:400`), closing TBD-04 (`:1307`). The route *group* is the load-bearing half: `(billing)/layout.tsx:33` is the only `EntitlementProvider` mount in the repository. v1.2 also introduces **C-13's `CheckoutOrder`** (`:951-960`), which the Phase Inversion clause makes **normative for this document** — see "S-06's read path" below. The v1.1 revision row (`:17`) corrected C-05's precedence to **`hint-shown → blocked-quota → busy → error → idle`** (`:697`) and made blocked-quota a mount-time state with no `role="alert"`. **Effect on this document: none on the data contract, one on the read path.** The precedence is evaluated entirely inside `ExplainStepAffordance` from `useEntitlement().tutor` (`ExplainStepAffordance.tsx:52`); the backend's obligation is unchanged — supply `tutor` as `{state:"known", used, limit}` so `used >= limit` is decidable at mount, with no request in flight. That obligation is what makes D005's provider-mount extension (below) load-bearing rather than cosmetic: without a provider above that component, `tutor.state` is `unknown` at mount and the blocked-quota branch can never be reached.
- **Frontend Design Doc**: `docs/design/subscription-frontend-design.md` (**v1.1** — *corrected in v1.4 (I007): v1.3 cited it as v1.0 in five places, and its line numbers had moved by roughly 82 lines*) — designs S-05 and S-06 against UI Spec v1.2 and consumes this document's Server Actions and RLS read. It raised two blocking items **this document owns**, both closed here: **FE-B-01** (`:1036`, *"S-06 has no read path for four of `CheckoutOrder`'s eight fields"*) and **FE-B-02** (`:1060`, *"`recheckOrder()`'s ownership scoping is unstated"*). Its contradictions X-1, X-2, X-7 and X-8 sit in the table beginning at `:1076`; X-1 and X-2 are the same two defects seen from the other side and are closed by the same edits, and **X-7** (`revalidatePath` vs. two routes) is resolved inside the frontend document by `router.refresh()` and needs nothing from this one — recorded so it is not re-opened as a backend obligation.
  **Acknowledgement status, stated so `design-sync` is not misled**: FE-B-01 and FE-B-02 are closed **here**, in this document, and are **not yet acknowledged there** — `subscription-frontend-design.md` v1.1 still carries them as open blocking items in its Status line and its "Blocking Unresolved Items" section, and its `getMyOrder()` is still specified with its own inline camelCase mapping rather than the shared `toCheckoutOrder()` this revision introduces (I010). That document is under concurrent revision; reconciling the two is `design-sync`'s step, not this one's.
...
| `npm run verify:schema` (`package.json:13`, DB-side, per environment) | The DDL being right in git and absent from a database | **adopted** — gate B, dev then prod |
...
| `telemetry.test.ts:261`'s two-layer guard | That `TELEMETRY_ERROR_CODES` matches the CHECK constraint | **adopted**, unmodified, plus one added schema-parse case (AC-046) |
...
| **AC-026** | `:286` | **BE — reassigned in v1.4** | `createOrder()` step (3) writes exactly one row; `PREMIUM_PRICE_VND = 39000` | Real-Postgres: one row, unique `order_code`, `amount = 39000`, `status = 'pending'` |
| **AC-027** | `:287` | **BE — new element in v1.4** | `createOrder()` **step (0)**: owner-scoped reuse of a live `pending` row, original `pendingUntil`, zero provider calls | Third verification point, AC-027 case |
...
| AC-035 | `:309` | BE | `recheckOrder()` → `settleOrder()` → provider re-verify | Mocked-adapter ordering test |
| AC-036 | `:310` | BE | `not_paid_yet` branch, zero writes | Unit |
| AC-037 | `:311` | BE | `guard("recheckOrder", …)`, DB-cost family, `limit >= 15` | `rateLimit.test.ts` classification case |
...
| AC-041 | `:319` | **FE** | Quota message must not reuse `t("tutor.error")` | FE |
...
| **AC-046** | `:327` | **BE — new in v1.4** | `telemetry.ts:35` extended; existing two-layer guard kept | `telemetry.test.ts:261` unmodified + one added schema-parse case |
...
| AC-050 | `:340` | **FE** | Expiry reminder in `SiteHeader` | FE |
| AC-051 | `:343` | **FE** | AC-056's four Must components survive R16 | FE |
| AC-052 | `:266` | BE | A6 anchor: `user_profiles.created_at + 30d × k` | Unit with creation days 15, 29, 31 |
...
**Frontend-scope boundary (S-05 / S-06) — restated in v1.3 (X-8).** v1.2 of this document said UI Spec `:38-39` *defers* S-05 and S-06 and that `:250-255` marks their routes as "*proposed*, deliberately unfrozen". **Both halves are stale.** UI Spec **v1.2** moves both screens to **Implement** (`:395-396`) and **UI-D11** (`:181`) **freezes** both routes — `/me/orders` and `/pricing/checkout?order={orderCode}`, both filed under `SOURCE/app/(billing)/` because that layout holds the repository's only `EntitlementProvider` mount (`:400`, TBD-04 closed at `:1307`). The v1.1 line numbers this document quoted now point at unrelated text (`:38-39` is S-01/S-02; `:250-255` is UI-D15's status table).
...
| **Telemetry error codes** | `SOURCE/lib/tutor/telemetry.ts` | **v1.4, I005** — `TELEMETRY_ERROR_CODES` at `:35` gains `user_quota_exhausted` and `project_budget_exhausted`. The two-layer guard (`:37` type, `:78` runtime filter) is untouched; `telemetry.test.ts:49`'s transcription is updated and `:261` must pass unmodified |
...
**Four** blocks, inserted **before** `-- 17. Phiên bản schema` at `schema.sql:1597` (the fingerprint insert must remain the last statement in the file). Three are new objects; the fourth (added v1.4, I005) is an **in-place alter of an existing constraint** and is accompanied by an edit to the inline declaration it replaces.
...
| **AC-046** (`:327`) | *"Cho hằng `TELEMETRY_ERROR_CODES` … nó vẫn là **nguồn duy nhất** cho cả kiểu TypeScript lẫn bộ lọc lúc chạy — hai thứ đó không được phép trôi lệch (rào chắn hai lớp hiện có phải giữ nguyên)"* | `telemetry.ts:35` gains the two literals and nothing else changes about it: `TelemetryErrorCode` still derives from it at `:37`, and the runtime filter at `:78` still reads the same array. **The existing guard is kept, not replaced** — `telemetry.test.ts:49`'s hand-transcribed `SCHEMA_ERROR_CODES` is updated to the same six and `:261`'s equality assertion is untouched |
...
| **AC-027** | A `pending` order inside its 30-minute window is resumable — UI Spec `:414`'s "continue paying" link is a cold entry into S-06 |
| **C-13** | The eight-field `CheckoutOrder` shape, normative (UI Spec `:948-960`) |
| **UI Spec `:946`** | S-06 reads its order "through the same `orders_select_own` RLS read S-05 uses" |
...
| E — **subtractive**: narrow S-06 to the post-`createOrder()` entry only | **fails AC-027, UI Spec `:414`** | 0 | 0 | no | requires a **UI Spec revision first** (Phase Inversion) | Not this document's to take |
...
- **E** contradicts UI Spec `:414` and AC-027 and, per the Phase Inversion clause, would require the UI Spec to change first. That is an escalation, not a design choice available here.
...
export async function getPaymentStatus(orderCode: number): Promise<"pending" | "paid" | "cancelled" | "unknown">;
...
Direct Impact:
  ...
  - SOURCE/lib/tutor/__tests__/telemetry.test.ts # SCHEMA_ERROR_CODES hand transcription at :49 -> same six literals;
                                                 #   :261's equality assertion UNMODIFIED; +1 ADDED case parsing schema.sql
  - SOURCE/supabase/schema.sql                   # 2 tables + 1 function before :1597; §17 fingerprint literal.
...
| I2 | `tutorActions.ts:175` (`guard("explainStep", userId)`) | add `consumeQuota("tutor", …)` beside it | High — refuses before the Gemini call | In: `userId`, `Entitlement`. Out: `{ok:true}` or `{ok:false, reason}`. On error: `unavailable` ⇒ refuse (AC-024) | Unit test: `ok:false` ⇒ zero `callTutor` invocations |
...
| I7 | **S-05 `/me/orders` / S-06 `/pricing/checkout`** (UI Spec v1.2 `:395-396`, routes **frozen** by UI-D11 `:181`; **frontend scope**, designed in `subscription-frontend-design.md` v1.1) | consume `createOrder()` / `recheckOrder()` and the `orders_select_own` RLS read | **High for the contract, deferred for the code** — no screen code in this phase | In: nothing / `orderCode`. Out: the full **`CheckoutOrder`** (v1.3, TBD-07 closed) and `SettleResult` respectively. **Ownership: `recheckOrder()` answers `unknown_order` for an order the caller does not own, byte-identical to a nonexistent one, with zero provider calls** (FE-B-02). On error: discriminated `reason`, never an exception across the action boundary | This document ships the actions + their contract tests, including the FE-B-02 negative test; the **screens** are the frontend document's |
...
| I8 | `scripts/check-ai-key-bundle.mjs` (run via `npm run check:bundle`, `package.json:12`) | +payOS secret markers | Medium | In: build output. Out: non-zero exit if a secret literal appears client-side | Existing script's own gate |
...
| **I11** | **`lib/tutor/telemetry.ts`** (new in v1.4, I005) — `TELEMETRY_ERROR_CODES` at `:35` | Add two literals to the existing constant. **Nothing else in the file changes** | **Medium** — a type widening plus a runtime-filter widening, both from one constant | In: a `TelemetryEvent` whose `errorCode` may now be one of six. Out: a `TelemetryLogInsert`. **On error: unchanged** — `:78`'s filter still returns `null` for an unrecognised code rather than throwing, deliberately (`:25-31`: a telemetry write must never become a second failure point for a waiting student) | **AC-046's two-layer guard is the constraint, not the test**: `telemetry.test.ts:261` must pass **unmodified** with `SCHEMA_ERROR_CODES` at `:49` updated to the same six. Plus one added case parsing every `error_code in ( … )` occurrence out of `schema.sql` |
...
| **5** | **The two AI gates** (`tutorActions.ts:175`, `actions.ts` ahead of `:268`; delete `:331-343`) + the two telemetry codes | 4, and step 1's alter **on both databases** | **L1** — a Free user's sixth tutor call is refused with a distinguishable reason, and the row lands in `telemetry_log` |
...
| **UI Spec TBD-02 — the legal content** | Engineer / U3. `/terms` and `/refund-policy` render `LegalContentPending` placeholders today; UI Spec `:1027` makes AC-039 a **gate**, and C-15 keeps the confirm control `aria-disabled` while `legalContentReady === false` | **Open.** UI Spec `:1305` records it as *"STILL OPEN, and it is the only blocking item"*, and `:1314` confirms it is the only one remaining at approval time |
...
Items this document cannot resolve within its own authority. **Neither blocks implementation.**
