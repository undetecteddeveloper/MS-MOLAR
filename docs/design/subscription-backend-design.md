...
4. **The RLS policies** — the table is enabled for row level security and its policy set is stated. Work-plan Task 1.5's `test-rls.ts` Phần 8 covers `payment_orders`, `subscriptions` and `record_payment_settlement()` only, so a sink arriving without a stated policy set arrives with **no negative test at all**.
