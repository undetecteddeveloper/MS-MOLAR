...
| Vercel Hobby-plan function duration limit (relevant to `TUTOR_CALL_DEADLINE_MS`) | External dependency, verified in Phase 3 Task 13 — platform default 300s (fluid compute, enabled by default) accommodates the 30s deadline with no `maxDuration` export; one residual (dashboard-level "Default Max Duration") stays unreadable from the repository | See Assumed Behaviors / Risks. |
...
| 2026-08-16 | 1.1 | Implementation closed (Work Plan Phases 1-5 + Final Phase Tasks 23-27). **Discrepancies recorded, no design reversal:** (1) **U3 shipped at 0.90, not the 0.75 placeholder** — the dry-run over the real 47-question corpus put the correct/incorrect boundary exactly at 0.90 (36 questions ≥0.90 all human-verified correct; all 5 at 0.85 were wrong or out-of-taxonomy), so this is the retune U3 anticipated, not a drift; coverage fell 87.2% → 76.6%, still above PRD Success Criteria #4's 70% bar. U5 shipped at 0.7 as designed. (2) **The tutor's Gemini key allows 20 requests per DAY project-wide** (`GenerateRequestsPerDayPerProjectPerModel-FreeTier`, read from the 429 body 2026-08-16) — this document's Assumed Behaviors treat Gemini failure as transient-and-retryable, which is true of 503s but NOT of this: it is a hard daily budget shared with UGC extraction, and `RATE_LIMITS.explainStep` (20/hour per user) lets one user drain it in an hour. Open decision, deliberately not resolved here. (3) Tutor round-trip latency measured 7.3s-23.0s against `TUTOR_CALL_DEADLINE_MS = 30_000` — the margin is 7s, not the 10× the maxDuration reasoning assumed. Everything else in this document was verified as designed against real dev Postgres and a real browser session; see `docs/plans/tasks/engine1-adaptive-ai-work-plan-phase5-completion.md` and `-phase6-completion.md`. | Final Phase Task 27 (Claude) |
| 2026-08-16 | 1.2 | **Corrects item (2) of v1.1, which is now out of date.** v1.1 records `RATE_LIMITS.explainStep` as `20/hour per user` and the daily-budget question as an "open decision, deliberately not resolved here." Commit `e8d91a4`, landed the same day but after v1.1 was written, changed it to `{ limit: 3, windowMs: 24h }`. **The unit was the fix, not the number** — an hourly window cannot bound a per-day supplier quota at any `limit` (3/hour is still 72/day for one account), so this document's implicit assumption that a rate limit's window is an arbitrary tuning knob is wrong wherever the limit exists to protect a *third-party* quota: there the window must match the provider's own unit. `rateLimit.test.ts` now encodes this as a structural invariant, partitioning `RATE_LIMITS` into DB-cost actions (cap generous, ≥15) and supplier-capped actions (`windowMs === 24h`, `limit <= 20`), with a third test asserting the two lists partition the table exactly — an unclassified new action fails the suite. **Still open, and correctly out of this document's scope:** the aggregate axis. The guard is per user; nothing counts the project's total, so 7 distinct users at 3 each still exceed 20, and an exhausted budget still reaches students as `error_code='server'`, indistinguishable from an outage. Owner decision, unchanged: that axis belongs to the Subscription feature, where the ceiling becomes a per-plan entitlement rather than one constant shared by everyone (`docs/prd/subscription-prd.md` R5/R7 already cites this interim `3` as its starting point). Items (1) and (3) of v1.1 stand as written. | Engine 1 close-out (Claude) |
