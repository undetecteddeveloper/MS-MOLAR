...
- [x] Build `SupportWidget` and its five UI Spec-defined children, mounted at the five UI-D1 mount points (`(exams)`, `(analytics)`, `(authoring)`, `(history)` layouts, and `app/page.tsx`) — reflected in Existing Codebase Analysis (mount point verification) and Change Impact Map.
...
- [x] **All four non-`(admin)`, non-`(auth)` route-group layouts (`(exams)`, `(analytics)`, `(authoring)`, `(history)`) already call `getCurrentUserProfile()` and render `<BottomNav />`, and `app/page.tsx` renders `<BottomNav />` directly.** Evidence: `(exams)/layout.tsx:11-32` read directly (confirms the pattern); `(analytics)/layout.tsx`, `(authoring)/layout.tsx`, `(history)/layout.tsx` confirmed to exist by Glob but not individually re-read line-by-line in this session — this document relies on the UI Spec's own stronger claim ("verified by reading all four," UI Spec UI-D1 rationale) for their structural identity, and independently confirms `app/page.tsx:5,8` imports `BottomNav` and `getCurrentUserProfile`. Confirmed: **Yes** for `(exams)` and `page.tsx` (directly read); **Inherited, not independently re-verified** for `(analytics)`/`(authoring)`/`(history)` — see the matching Risks row.
...
- [x] Client component boundary marked `"use client"` only at the smallest interactive scope; Server Components by default `[explicit]` — Source: `(exams)/layout.tsx` (Server Component) rendering `<SiteHeader user={user} />`/`<BottomNav />` (client leaves); `ReportExam.tsx`/`ModerationRow.tsx` (`"use client"` at the leaf, not the route).
...
| Design System | `SOURCE/components/ui/button.tsx`; `SOURCE/components/layout/{PageContainer,PageHeader}.tsx`; `SOURCE/features/authoring/components/StatusBadge.tsx` (pattern precedent) | Read directly in this session (see Existing Codebase Analysis) |
...
- [ ] **Given** the current pathname matches the `(exams)` attempt route pattern, **when** the page renders, **then** `SupportWidget` returns `null` regardless of auth state. (AC-005)
...
| `BottomNav`, five layouts' `getCurrentUserProfile()`/mount structure | Verified existing in codebase (directly for `(exams)` and `page.tsx`; existence-confirmed by Glob, structure inherited from UI Spec's own claim for `(analytics)`/`(authoring)`/`(history)`) | `BottomNav.tsx`, `(exams)/layout.tsx`, `page.tsx` (read directly); `(analytics)/layout.tsx`, `(authoring)/layout.tsx`, `(history)/layout.tsx` (Glob-confirmed to exist) |
...
|---|---|
| `SOURCE/features/exams/components/ReportExam.tsx` (full file) | Dialog shell, focus/Escape/scrim handling, plain-`useState`-driven async submit, error-message rendering pattern `SupportWidgetDialog` follows |
| `SOURCE/features/admin/components/ModerationRow.tsx` (full file) | `useActionState` + hidden-`FormData`-input pattern `TicketStatusControl`/`InternalNoteForm` follow to bridge a two-argument backend action into a form-action shape |
| `SOURCE/features/admin/actions.ts` (full file) | Confirms `moderateExamAction` itself already has the `(prevState, formData)` shape (unlike this feature's two new admin actions) — the precedent this document departs from, with the departure documented in Fact Disposition |
| `SOURCE/features/authoring/components/StatusBadge.tsx` (full file) | `CONFIG` map + glyph + `cn()` shape `TicketStatusBadge` mirrors with its own independent type/config (UI Spec I002) |
...
| `SOURCE/app/(history)/history/loading.tsx`, `error.tsx` (full files) | The route-boundary pattern (`PageContainer` skeleton; `role="alert"` + `reset()`-wired retry) `/admin/tickets`' new `loading.tsx`/`error.tsx` follow |
...
| `SOURCE/app/(exams)/layout.tsx`, `SOURCE/app/page.tsx:1-40` | The two UI-D1 mount points read directly in this session; confirms `getCurrentUserProfile()` call site and `<BottomNav />` render position |
...
// From "@/features/admin/ticketActions" (backend-owned)
...
Output:
  Guarantees: returns null (no DOM node) when user is null (AC-003) OR when usePathname() matches the
              (exams) attempt-route pattern /^\/exams\/[^/]+\/attempt\/[^/]+$/ (AC-005) — both checks run
...
### `TicketStatusBadge` (new, `SOURCE/features/admin/components/tickets/TicketStatusBadge.tsx`, `"use client"`)
...
Direct Impact:
  ...
  - NEW SOURCE/features/admin/components/tickets/TicketQueueList.tsx, TicketQueueRow.tsx, TicketStatusBadge.tsx,
  ...
  - SOURCE/app/(exams)/layout.tsx, app/(analytics)/layout.tsx, app/(authoring)/layout.tsx, app/(history)/layout.tsx,
...
flowchart TB
  subgraph MountPoints["Mount points (existing layouts, additive sibling only)"]
    L2["app/(exams)/layout.tsx"]
    L3["app/(analytics)/layout.tsx"]
    L4["app/(authoring)/layout.tsx"]
    HM["app/(history)/layout.tsx"]
    HOME["app/page.tsx"]
  end
...
- Precedents read directly in this session: `SOURCE/features/exams/components/ReportExam.tsx`; `SOURCE/features/admin/components/ModerationRow.tsx, SOURCE/features/admin/actions.ts, SOURCE/app/(admin)/admin/page.tsx`; `SOURCE/features/authoring/components/StatusBadge.tsx`; `SOURCE/components/layout/BottomNav.tsx`; `SOURCE/components/ui/{button.tsx,SuccessToast.tsx}`; `SOURCE/components/layout/{PageContainer.tsx,PageHeader.tsx}`; `SOURCE/app/(history)/history/{loading.tsx,error.tsx}`; `SOURCE/lib/i18n/{client.tsx,server.ts,translate.ts,dictionaries/vi.ts}`; `SOURCE/lib/auth/{admin.ts,getCurrentUser.ts}`; `SOURCE/lib/ugc/limits.ts`; `SOURCE/app/(exams)/layout.tsx`; `SOURCE/app/page.tsx`.
