...
| `ui:09` | C-13's `CheckoutOrder` is normative for the backend | **preserve** | Consumed as normative. The read-path gap it exposed was FE-B-01, **closed in backend v1.4** by the four persisted transfer columns | UI Spec `:948-963` |
...
**Owner**: backend design phase (the same owner as TBD-07). **Status: closed**, together with TBD-07, in backend Design Doc v1.4 `:572-694`.
...
**Escalation condition — DISCHARGED.** It read: "S-06 implementation (slice S2) does not start until one of the three is recorded." Option 1 **is** recorded, in backend Design Doc v1.4 `:572-694`, so **the condition is met and S2 is startable**. It is not to be re-read as an open gate.
...
**Owner**: backend design phase. **Status: closed** in backend Design Doc v1.4 `:695-700`.
