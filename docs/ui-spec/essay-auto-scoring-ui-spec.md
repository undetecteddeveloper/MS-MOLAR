...
| **O-2** | UI-D9 giả định payload xuống client mang **một boolean** `retryAvailable`, suy ra ở server từ bộ đếm lượt. Cần backend xác nhận: (a) boolean này được suy ra ở đâu (`getResult()` hay hàm suy diễn dùng chung), và (b) AC-044 có được đọc là cho phép boolean này không. Nếu kỹ sư muốn **con số** hiện ra thay vì boolean, quyết định đó phải kèm câu chữ giải quyết được vấn đề "số tụt mà học sinh không làm gì" (ADR-0018 Decision 4) | Kỹ sư + Design Doc backend | Trước Design Doc frontend "Accepted" |
...
| **O-5** | UI-D8 giữ **hai** khoá chân trang player vì AC-067 tạo ra một khoảng thời gian mà câu cũ vẫn đúng. Phương án gọn hơn: Work Plan ràng buộc commit đổi chữ chỉ đáp xuống **sau** khi cổng ZDR đã qua, khi đó một khoá là đủ. Cần kỹ sư chọn dứt điểm — hai khoá là mặc định của tài liệu này | Kỹ sư (Work Plan) | Trước khi task copy được lên lịch |
...
| **O-8** | Có nên chặn xuất PDF ở **RS-6** (hết lượt, vĩnh viễn thất bại) hay không. Tài liệu này quyết định **không chặn**, vì chặn ở đó là chặn vĩnh viễn và AC-058 tự nêu tên `failed past its retry cap` trong danh sách mở khoá. Ghi thành open item vì hệ quả cần được nhìn thấy một cách có ý thức: **PDF ấy sẽ mang một con điểm thiếu phần tự luận, mãi mãi** — cần xác nhận rằng chú thích trong PDF (hoặc việc không có chú thích) là chấp nhận được | Kỹ sư / chủ sản phẩm | Trước Design Doc frontend "Accepted" |

*Mọi TBD ở trên đều có owner và mốc. **O-1** và **O-2** chặn Design Doc frontend; các mục còn lại không chặn.*
