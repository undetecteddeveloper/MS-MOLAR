...
|---|---|---|
| `ESSAY_POLL_FAST_INTERVAL_MS` | **5 000** | 12 tick đầu phủ 60 giây. Không phải vì 60 giây là mục tiêu độ trễ — **mục tiêu đó đã đổi thành ≤ 3 phút** (OQ-7, PRD v1.3) — mà vì câu **đầu tiên** trong một lượt vẫn đáp xuống sớm: với đồng thời 2, hai bài đầu tiên đi ngay trong burst đầu. Nhịp dày ở đây mua **độ nạp của kết quả đầu tiên**, không phải của toàn lượt |
| `ESSAY_POLL_FAST_TICKS` | **12** | 12 × 5s = 60s |
| `ESSAY_POLL_SLOW_INTERVAL_MS` | **10 000** | Qua 60 giây, các band còn lại đáp xuống **rải ra theo trần TPM** chứ không dồn cục; giữ nhịp dày ở đây là bắt **thiết bị yếu nhất** trả tiền cho những lượt refresh hầu hết không thấy gì mới |
| `ESSAY_POLL_MAX_REFRESHES` | **30** | 12 nhanh + **18 chậm** (v1.4; trước là 18 = 12 + 6). Trần **số lượt** — mỗi lượt là một RSC payload đầy đủ của trang kết quả. **Giá phải trả, nói thẳng:** 30 payload thay vì 18 trên thiết bị yếu nhất. Đánh đổi có chủ ý và lý do nằm ở ô dưới |
| `ESSAY_POLL_MAX_ELAPSED_MS` | **240 000** | Trần **thời gian**, khai **độc lập** với trần số lượt. Hai trần vì một lượt `router.refresh()` chậm làm hai đại lượng lệch nhau; cái nào chạm trước thì dừng. **240 000 = `ESSAY_PASS_BUDGET_MS`** (v1.4) — xem ô ngay dưới bảng |

**Hai con số này KHÔNG phải hạn chờ của AC-026, và không được suy ra từ nó** (AC-061). Chúng là **giới hạn tài nguyên phía client**; hạn chờ là **quy tắc trình bày phía server** áp cho **mọi** lượt đọc, kể cả một lượt mở trang nguội nhiều ngày sau khi không có poller nào chạy. Đổi một cái **không** kéo theo đổi cái kia. Hai con số ở trên hợp lệ ngay cả khi hạn chờ ngắn hơn 240 giây hoặc dài hơn nhiều lần.

**Vì sao hai trần chuyển từ 18 / 120 giây sang 30 / 240 giây (v1.4, 2026-08-29).** Bản v1.0 lấy hai trần này từ **mục tiêu độ trễ của PRD** (median ≤ 60s), cộng một biên gấp đôi. Mục tiêu ấy **đã đổi thành ≤ 3 phút** (OQ-7, PRD v1.3) vì nó chưa bao giờ đạt được: trần **TPM 8K** của nhà cung cấp khiến một lượt 5 câu mất tối thiểu ~2 phút. Giữ nguyên trần 120 giây sau khi mục tiêu đã dịch có nghĩa là **học sinh sẽ thấy `result.essay.pollStopped` trên một lượt chạy HOÀN TOÀN BÌNH THƯỜNG** — hệ thống đang làm đúng việc của nó nhưng giao diện báo là đã thôi cập nhật. Trạng thái dừng là một **ngoại lệ**, không phải kết cục mặc định; để nó thành mặc định là dạy học sinh đọc một hệ thống đang chạy tốt thành hỏng.

**Mốc neo mới tốt hơn mốc neo cũ.** 240 000 ms **không** lấy từ mục tiêu độ trễ mới (làm thế là lặp lại đúng sai lầm cũ: neo một trần tài nguyên vào một con số **kỳ vọng**). Nó bằng **`ESSAY_PASS_BUDGET_MS` = 240 000** — trần đồng hồ của chính **lượt chấm**. Sau mốc đó không còn band nào có thể đáp xuống từ pass đó nữa, nên mọi lượt refresh tiếp theo là **chắc chắn vô ích**. Poller giờ dừng đúng lúc người viết dừng — một mệnh đề **kiểm được**, thay vì một ước lượng.

**Một lệch nhỏ, ghi ra chứ không giấu:** đồng hồ của poller bắt đầu khi **trang kết quả mount**, còn đồng hồ của pass bắt đầu khi **submit**. Trang mount sau submit vài giây (một lượt redirect + RSC), nên poller dừng **muộn hơn** pass vài giây — đúng chiều an toàn: nó không bao giờ dừng **trước** khi người viết xong. Hai con số vẫn là hai đại lượng khác nhau; **không** được viết mã suy cái này ra từ cái kia.

**Tab ẩn.** Một tick xảy ra khi `document.visibilityState === "hidden"` **không** gọi `router.refresh()` và **không** tiêu một lượt trong ngân sách **30** lượt; đồng hồ **240** giây vẫn chạy. Lý do: một tab chạy nền là đúng ca mà một lượt tải lại RSC đầy đủ **không mua được gì** (không ai đang nhìn), trong khi trần thời gian vẫn bảo đảm vòng lặp kết thúc.
...
| AC-021 | Khi chạm 30 lượt **hoặc** 240 giây | — | Dừng hẳn vòng lặp | Default → Partial (hoặc → Empty nếu đã xong) | — |
...
| **O-3** | ~~Chi phí payload của `per_question` + `created_at` trong `listMyHistory()`.~~ **ĐÃ ĐÓNG 2026-08-29 (Gate D), bằng số đo chứ không bằng ước lượng:** **375 B/dòng** không có hai trường → **3 401 B/dòng** khi có, tức **≈9,1×**, và ~183 KB → **~1 661 KB** ở trần 500 dòng; dòng lớn nhất đo được **5 385 B**. **Quyết định của kỹ sư: ACCEPT.** Lý do ghi kèm: 500 là một trần **cao hơn dữ liệu hôm nay ba bậc độ lớn** (prod có tổng cộng 9 dòng kết quả), nên chạm tới nó là điều kiện kích hoạt **phân trang**, không phải điều kiện để đặt một con số lớn hơn. **Phương án RPC KHÔNG được chọn**, nên **ADR-0018 Escalation 2 vẫn đóng** và số thay đổi schema áp tay **không** bị nâng từ hai lên ba | Kỹ sư — **đã đóng** | ✅ |
| **O-4** | Trạng thái "Đã chấm" hiện được đánh dấu bằng **độ đậm + `--foreground`** vì `globals.css` **không có** `--success`/`--warning`. Nếu sản phẩm muốn một màu tích cực thật, việc đó là **thêm token `--success` + đóng TBD-04** của `short-answer-scoring-ui-spec.md` (nơi `#4F7942` đang là hex cứng lặp lại nhiều lần), **không** phải chép hex vào tính năng này. Tài liệu này cố ý **không** phát minh một hex. **XÁC NHẬN CÒN MỞ 2026-08-30 (Final §16) — cố ý, không phải bỏ sót.** Không chặn gì: `EssayLifecycleBadge.test.tsx` đã ghim rằng file **không có literal hex nào** và **không mượn `#4F7942`**, nên hàng rào mà O-4 tồn tại để giữ đang được cưỡng chế bằng test chứ không bằng trí nhớ | Kỹ sư / chủ sản phẩm | Không chặn ship; quyết định trước khi ai đó chép `#4F7942` lần nữa |
...
| **O-6** | ~~Bốn hằng polling chốt từ mục tiêu độ trễ chứ không từ đo đạc.~~ **ĐÃ ĐÓNG HOÀN TOÀN 2026-08-30 (Task E5).** Nửa đầu đóng 2026-08-29 (v1.4): hai **trần** dịch 18 → 30 lượt và 120 → 240 giây, và **đổi mốc neo** sang `ESSAY_PASS_BUDGET_MS`. Nửa còn lại — vòng round-trip Singapore→Groq (C4) — **nay đã đo**: **p50 671 ms / p95 1 289 ms** trên n = 18 lượt chấm thật (min 503 ms, max 1 289 ms). Một bộ 3 mẫu đo end-to-end qua `submitExam()` cho ~2,5–5 s; **chênh lệch là orchestrator + claim + ghi DB, không phải nhà cung cấp**. **Không hằng nào dịch:** p95 cách `GROQ_CALL_DEADLINE_MS` (20 s) **15 lần**, nên điều kiện leo thang không thoả; hai **nhịp** (5s×12 → 10s) giữ nguyên vì chúng chốt trải nghiệm chứ không chốt tài nguyên; và hai **trần** neo vào `ESSAY_PASS_BUDGET_MS`, thứ **không** dịch — nên một phép đo độ trễ tự nó không dịch chúng. **Hạn chờ đọc-lúc-render vẫn là một con số khác và vẫn không dịch theo** (AC-061): nó neo vào trần thời lượng của nền tảng, không vào độ trễ | Kỹ sư — **đã đóng** | ✅ |
...
|------|---------|---------|--------|
| 2026-08-30 | 1.5 | **Final §16 — đóng O-3 và O-6 bằng số đo, xác nhận O-4 còn mở, và sửa một lỗi phiên bản của CHÍNH file này.** **O-3 đóng** bằng phép đo Gate D (375 → 3 401 B/dòng, ≈9,1×, ~1 661 KB ở trần; kỹ sư ACCEPT) — phương án RPC không được chọn nên **Escalation 2 vẫn đóng**. **O-6 đóng nốt nửa còn lại** bằng Task E5 (p50 671 ms / p95 1 289 ms, n = 18): không hằng nào dịch, vì p95 cách `GROQ_CALL_DEADLINE_MS` 15 lần và hai trần neo vào `ESSAY_PASS_BUDGET_MS` chứ không vào độ trễ. **O-4 xác nhận CÒN MỞ có chủ ý** với chủ sở hữu của nó. **Lỗi phiên bản đã sửa:** bảng header ghi **Version 1.3** trong khi § Update History của chính file này đã có một hàng **1.4** từ 2026-08-29 — tức header tụt lại sau bản ghi có thẩm quyền. Đây **cùng một lớp lỗi** mà Design Doc frontend đã điểm mặt ở header của nó, và nó đã lan: bản sửa citation 2026-08-30 của frontend DD đọc header cũ rồi kết luận "UI Spec thật là v1.3", trong khi bản ghi có thẩm quyền nói 1.4. Header nay là **1.5** và ghi chú ở frontend DD đã được đính chính. | UI Spec (Claude) |
...
| 2026-08-29 | 1.4 | **Hai trần polling dịch theo OQ-7, và đổi mốc neo (kỹ sư chốt 2026-08-29).** `ESSAY_POLL_MAX_REFRESHES` **18 → 30** (12 nhanh + **18** chậm) và `ESSAY_POLL_MAX_ELAPSED_MS` **120 000 → 240 000**. Lý do không phải “chờ lâu hơn cho chắc”: PRD v1.3 đã nới mục tiêu độ trễ từ ≤ 60 giây lên **≤ 3 phút** vì trần **TPM 8K** của nhà cung cấp khiến một lượt 5 câu mất tối thiểu ~2 phút, nên trần 120 giây cũ sẽ hiện `result.essay.pollStopped` trên một lượt chạy **bình thường** — biến một trạng thái ngoại lệ thành kết cục mặc định và dạy học sinh đọc hệ thống đang chạy tốt thành hỏng. **Thay đổi có giá trị lâu dài hơn con số là MỐC NEO:** 240 000 ms **không** suy từ mục tiêu độ trễ mới (làm thế là lặp lại sai lầm cũ: neo trần tài nguyên vào một con số kỳ vọng) mà bằng **`ESSAY_PASS_BUDGET_MS`** — sau mốc đó không band nào còn có thể đáp xuống từ pass đó, nên mọi refresh tiếp theo là **chắc chắn vô ích**; poller dừng đúng lúc người viết dừng, một mệnh đề kiểm được. **Giá phải trả, ghi ra chứ không giấu:** tối đa 30 RSC payload đầy đủ thay vì 18 trên thiết bị yếu nhất. **Lệch nhỏ được ghi:** đồng hồ poller bắt đầu lúc trang mount, đồng hồ pass bắt đầu lúc submit — poller dừng muộn hơn vài giây, đúng chiều an toàn. **AC-061 giữ nguyên**: hạn chờ đọc-lúc-render vẫn là 10 phút, vẫn neo vào trần thời lượng của nền tảng, và **không** được suy ra từ hai trần trên. Cập nhật kèm: bảng hằng số, đoạn “tab ẩn”, dòng AC-021 trong bảng trạng thái, và O-6 (đóng một nửa). |
