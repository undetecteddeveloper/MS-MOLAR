...
| Existing component precedent | `SOURCE/features/exams/components/ReportExam.tsx` (dialog pattern — PRD-cited, D8); `SOURCE/features/admin/components/ModerationRow.tsx`, `actions.ts` (admin row/action pattern — PRD-cited, R7); `SOURCE/features/authoring/components/StatusBadge.tsx` (non-color-only status label); `SOURCE/components/ui/{button,SuccessToast}.tsx`; `SOURCE/components/layout/{PageContainer,PageHeader,BottomNav}.tsx` | repo `feat/support-system` |
...
| Design System | `SOURCE/components/ui/{button,SuccessToast}.tsx`; `SOURCE/components/layout/{PageContainer,PageHeader}.tsx`; `SOURCE/features/authoring/components/StatusBadge.tsx` (pattern precedent for the new sibling component `TicketStatusBadge`, see Existing Component Reuse Map) | `SuccessToast` is evaluated and **not** reused for the submit acknowledgement — see Decisions Record UI-D8 |
...
|-----------|------------|-------------|-----------------|
| S-01 | Support Widget — Trigger | Floating circular affordance, rendered wherever the widget is mounted (UI-D1) | Logged-in student, on any mounted route, not on the `(exams)` attempt route |
...
|--------|------------|---------|-----------------|
| (page load) | S-01 | Route renders in a mounted layout | `user !== null` (AC-003) AND pathname is not the `(exams)` attempt route (AC-005) |
...
Root Layout (SOURCE/app/layout.tsx)
  +-- app/(exams)/layout.tsx, app/(analytics)/layout.tsx, app/(authoring)/layout.tsx, app/(history)/layout.tsx, app/page.tsx
...
Absent states (not rows in the matrix, because the component renders nothing — no DOM node — rather than a disabled/dim display): logged-out visitor (AC-003); on the `(exams)` attempt route (AC-005, UI-D1).
...
| AC-005 | Given the current route is the `(exams)` attempt route | (page render) | Component returns `null` regardless of auth state | Absent | — |
...
| Widget dialog scrim + shell | Reuse (pattern, new file) | `SOURCE/features/exams/components/ReportExam.tsx` | Structure, Escape/scrim-click-to-close, and minimal focus trap copied as a pattern (UI-D7); not imported across route groups — new component `SupportWidgetDialog` |
...
| Admin status label | New (sibling component, pattern reuse — resolves review finding I002) | `SOURCE/features/authoring/components/StatusBadge.tsx` (pattern precedent only; **not modified**) | New component `TicketStatusBadge`, with its own independent `Status` union (`new`/`in_progress`/`resolved`) and `CONFIG` map — never merged into `StatusBadge`'s own `Status`/`CONFIG`, because the two domains are unrelated (UGC content lifecycle vs. ticket workflow) and the originally proposed glyphs collided exactly with glyphs `StatusBadge` already assigns to unrelated statuses. Follows `StatusBadge`'s glyph+text, no-color-alone visual contract verbatim (`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium`, `aria-hidden` glyph + text label). Glyphs chosen to be visually distinct from all five already in `StatusBadge.CONFIG` (`◌` processing, `◑` review, `○` draft, `●` published, `▲` failed): `✉` new, `▶` in_progress, `✓` resolved (colors confirmed at implementation time, TBD-08). |
| Admin row shell + form wiring | Reuse (pattern) | `SOURCE/features/admin/components/ModerationRow.tsx`, `actions.ts` | `useActionState` form-action pattern reused for `TicketStatusControl` and `InternalNoteForm` |
