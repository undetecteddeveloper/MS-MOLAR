...
| Version | Date | Change |
|---|---|---|
| 1.11 | 2026-08-21 | **A rendering repair on this document's own v1.7 revision row — no claim moves.** That row carried two **unescaped pipes** inside its inline-code union types (`variant: "row" \| "primary"`), giving it **5 cells against a 3-column header**. GFM parses table cells *before* inline code, so backticks do not protect a pipe: cells 4 and 5 were **silently discarded in every rendered view** — roughly 1,960 characters, including the **`Amended`: Props become `{ orderCode: number; variant: "row" \| "primary"; status: string }`** clause, the entire **"typed `string`, deliberately, not a four-literal union"** rationale, and the "No i18n key is added" clause. The failure was worse than absence: the rendered cell terminated mid-signature at `{ orderCode: number; variant: "row"`, so the **visible last word of the v1.7 row was the superseded two-prop signature**, and the amendment that repairs it was exactly what was eaten — a reader of the rendered authoritative spec was shown precisely the contradicted version. The break is **pre-existing**, introduced at `e5b91e7` (confirmed by `git log -S`), and the normative Props line outside the table was never affected. **Escaping a pipe changes rendering, not asserted content**, so this does not re-edit a history row: not one word of the v1.7 row's claim moves. No screen is re-specified, no component is renumbered, no decision or acceptance criterion changes, and no file under `SOURCE/` is touched. |
...
`globals.css` wins on any conflict with `.claude/MEMORY.md` (`.claude/MEMORY.md`, *"khi hai chỗ lệch nhau thì **`globals.css` thắng**"*). `PROJECT_OVERVIEW.md §2` was deleted 2026-08-06 and must not be cited; several in-repo comments still reference it (e.g. `SOURCE/app/page.tsx:15`) and those references are stale.
...
**Decision.** `SOURCE/app/(billing)/` containing `pricing/page.tsx`, `terms/page.tsx`, `refund-policy/page.tsx`, with a `layout.tsx` that mirrors `SOURCE/app/(exams)/layout.tsx` exactly.
...
| `features/authoring/components/ExamRow.tsx:68` — a second private `DD/MM/YYYY HH:mm` | Same defect, plus it is the duplication the Rule of Three says to stop at three |
...
**Decision.** `SOURCE/app/(billing)/me/orders/` and `SOURCE/app/(billing)/pricing/checkout/` each gain `loading.tsx` and `error.tsx`, modelled line-for-line on `app/(analytics)/profile/{loading,error}.tsx`.
...
```
SOURCE/app/(billing)/layout.tsx                  [NEW — mirrors app/(exams)/layout.tsx]
├── SkipLink                                      [reuse]
...
└── rendered by app/(exams)/…/result/detail/page.tsx, beside both affordance call sites
```
...
**File**: `SOURCE/features/billing/components/pricing/PlanComparison.tsx`
...
**File**: `SOURCE/features/billing/components/pricing/PurchaseCta.tsx` (`"use client"`)
...
**File**: `SOURCE/features/billing/components/orders/OrderList.tsx` — **server component** (no `"use client"`).
...
**File**: `SOURCE/features/billing/components/orders/OrderRow.tsx` — server component; it renders two client children (C-09, C-10).
...
**File**: `SOURCE/features/billing/components/orders/PlanSummary.tsx` — `"use client"` (it reads `useEntitlement()`; the provider is above it because of UI-D11's route-group choice).
...
**File**: `SOURCE/features/billing/components/checkout/VietQrCode.tsx` — **server component**; the SVG is produced during the server render (UI-D14).
...
**File**: `SOURCE/features/billing/components/checkout/TransferDetails.tsx` — server component.
...
**File**: `SOURCE/features/billing/components/checkout/PaymentConfirm.tsx` — `"use client"`.
...
Every page must reserve space for the fixed `BottomNav` via `.pb-bottom-nav` — supplied by the `(billing)` layout, matching `(exams)/layout.tsx:28`.
...
| **TBD-08** | **Migrate the four legacy date call sites to `lib/format/datetime.ts`.** `lib/history/format.ts:19`, `features/authoring/components/ExamRow.tsx:68`, `admin/tickets/InternalNotesPanel.tsx:72`, `admin/tickets/TicketQueueRow.tsx:46`. The first two read the *runtime's* timezone (UTC on Vercel for anything rendered server-side); the last two read the *server's* locale. **Deliberately not bundled into this feature**: changing what a shipped screen prints is a visible change with no AC behind it, and it would put `/history`, `/me/exams` and `/admin/tickets` at regression risk inside a payment change. | engineer | next maintenance pass; not before the subscription feature ships |
...
|---|---|---|---|
| 2026-08-21 | 1.11 | **A rendering repair on this document's own v1.7 revision row — no claim moves.** That row carried two **unescaped pipes** inside its inline-code union types (`variant: "row" \| "primary"`), giving it **5 cells against a 3-column header**. GFM parses table cells *before* inline code, so backticks do not protect a pipe: cells 4 and 5 were **silently discarded in every rendered view** — roughly 1,960 characters, including the **`Amended`: Props become `{ orderCode: number; variant: "row" \| "primary"; status: string }`** clause, the entire **"typed `string`, deliberately, not a four-literal union"** rationale, and the "No i18n key is added" clause. The failure was worse than absence: the rendered cell terminated mid-signature at `{ orderCode: number; variant: "row"`, so the **visible last word of the v1.7 row was the superseded two-prop signature**, and the amendment that repairs it was exactly what was eaten — a reader of the rendered authoritative spec was shown precisely the contradicted version. The break is **pre-existing**, introduced at `e5b91e7` (confirmed by `git log -S`), and the normative Props line outside the table was never affected. **Escaping a pipe changes rendering, not asserted content**, so this does not re-edit a history row: not one word of the v1.7 row's claim moves. No screen is re-specified, no component is renumbered, no decision or acceptance criterion changes, and no file under `SOURCE/` is touched. | UI Spec verification (Claude) |
