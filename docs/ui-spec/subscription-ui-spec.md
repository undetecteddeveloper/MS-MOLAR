...
| **Backend Design Doc** | `docs/design/subscription-backend-design.md` (v1.2) — owns `createOrder()` / `recheckOrder()`, the `payment_orders` table and the `orders_select_own` RLS read that S-05 and S-06 consume. |
...
**Native `disabled` is forbidden**, in both the terminal-status case and the busy case (`ExplainStepAffordance.tsx:11-14`; test-enforced at `ExplainStepAffordance.test.tsx:299-300` by asserting `hasAttribute("disabled") === false` **and** `.disabled === false`). The point is sharpest here: a user whose order says "cancelled" needs to reach the control to read *why* re-checking will not help.
...
The screen's data spine. It reads **one** order by the `?order=` search param through the same `orders_select_own` RLS read S-05 uses, so a foreign or non-existent `orderCode` returns no row and lands in Empty — the client cannot see another user's order, and the screen does not need its own permission logic.
...
| State | Default | Loading | Empty | Error | Partial |
|---|---|---|---|---|---|
| Display | `pending` order: QR (C-12) + text block (C-14) + deadline + legal links + confirm control (C-15) | Route-level `loading.tsx` (UI-D18) at `size="small"` | **No `?order=`, unknown `orderCode`, or an order belonging to someone else** — one shared state: "no active payment in progress", with a link back to `/pricing`. All three are indistinguishable **on purpose**: distinguishing "not yours" from "not found" would confirm the existence of another user's order | Route-level `error.tsx` (UI-D18) | **Order already `paid` / `expired` / `cancelled`**: the QR and the transfer block are **not** rendered — showing transfer instructions for a settled or dead order invites a second payment. The status, the `orderCode` and a link to `/me/orders` are rendered instead |
...
**Props**: `{ orderCode: number; legalContentReady: boolean }`.
...
| **TBD-07** | **`createOrder()`'s return shape is missing three AC-028 fields.** The backend Design Doc v1.2 declares `{ qrCode, orderCode, expiresAt }` (Integration Point I7). S-06 needs `amountVnd`, `accountNumber`, `accountName` and `memo` as well — three of them are AC-028's mandatory **text** equivalent. Per the Phase Inversion clause, C-13's `CheckoutOrder` type is normative and the backend document adopts it. **Required input**: a backend Design Doc revision confirming these fields cross the Server Action boundary. **S-06 must not re-derive any of them** — an amount recomputed from the price constant would diverge from an in-flight order after a price change, the same failure `settleOrder()` step 3 refuses on the server side. | backend design phase | before S-06 implementation starts |
