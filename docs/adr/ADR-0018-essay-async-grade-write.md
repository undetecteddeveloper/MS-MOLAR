...
| `claim_attempt_answer_key()` already establishes the **claim-before-you-act** shape: it closes the attempt *first*, then returns the answer key, so "got the key" and "can still answer" are mutually exclusive at the DB level. | `schema.sql` §10b; `features/exams/actions.ts` step 3 |
