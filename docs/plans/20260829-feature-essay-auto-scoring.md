...
- [ ] A7 — Until A6 carries a real date, `ESSAY_GRADING_ENABLED` is **absent in both Vercel scopes** (Production and Preview), and Phase E is not started. A local dev `true` is governed by **A5b**, not by this item — it is permitted after A5b, against seeded data only.
...
- [ ] **Task B1.5 — GREEN: `computeScore()` options + branch split, then `submitExam()` + `maxDuration` — TWO COMMITS WITH AN EXPLICIT BOUNDARY (I-2 closed 2026-08-29; boundary fixed by I004)**
  ...
  - Files: **commit 1** `SOURCE/lib/scoring/computeScore.ts`, `SOURCE/lib/scoring/__tests__/computeScore.test.ts`; **commit 2** `SOURCE/app/(layer2)/actions.ts`, `SOURCE/app/(layer2)/exams/[id]/attempt/[attemptId]/page.tsx`
...
- [ ] **Task B2.1 — `getResult()`: `created_at` in the select, plus the three derived fields**
  ...
  - Files: `SOURCE/app/(layer2)/queries.ts`, `SOURCE/types/result.ts` (add `essay?: EssayView` to `PerQuestionResult`, following the `hasBeenWrongTwice` precedent at `:19-24`)
...
- [ ] **Task F-C2 — `EssayGradingPoller` + deterministic RTL test**
  ...
  - Two-phase cadence and two **independent** caps, **all six values declared as named constants** so the component never hand-types a literal (I016): `ESSAY_POLL_FAST_INTERVAL_MS` = 5 000, `ESSAY_POLL_FAST_TICKS` = 12 (covering the first 60 seconds, the window where a result is genuinely expected), `ESSAY_POLL_SLOW_INTERVAL_MS` = 10 000, `ESSAY_POLL_MAX_REFRESHES` = **18**, `ESSAY_POLL_MAX_ELAPSED_MS` = **120 000**. The last two were the only ones the UI Spec left as bare numbers; naming them is what keeps the test file and the component from drifting, since the tests assert against exactly those two bounds. A tick while `document.visibilityState === "hidden"` is **skipped and costs no budget**, but the 120-second clock keeps running. On stopping, show `result.essay.pollStopped` and a real `<button>` "Cập nhật" that performs one `router.refresh()` and **reloads both budgets**. These four constants are **not** the read-time deadline and are not derived from it (AC-061); the deadline is anchored to the platform's duration ceiling, not to a latency estimate. They are owned by **O-6/OQ-1** until measured.
