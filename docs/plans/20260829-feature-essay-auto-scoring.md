...
- **Backend — new**: `SOURCE/lib/scoring/essayLifecycle.ts`, `SOURCE/lib/billing/budgetDay.ts`, `SOURCE/lib/essay/{groqClient,prompt,parseGrade,budget,gradeEssays}.ts`, `SOURCE/app/(layer2)/essayActions.ts`
- **Backend — modified**: `SOURCE/lib/scoring/computeScore.ts`, `SOURCE/lib/billing/quota.ts`, `SOURCE/lib/supabase/service-role.ts`, `SOURCE/lib/ai/models.ts`, `SOURCE/lib/ugc/limits.ts`, `SOURCE/lib/tutor/{prompt,telemetry}.ts`, `SOURCE/lib/env/checkEnv.ts`, `SOURCE/lib/schema/schemaFingerprint.ts`, `SOURCE/lib/pdf/generateAttemptPdf.ts`, `SOURCE/app/(layer2)/{actions,queries}.ts`, `SOURCE/app/(HM)/queries.ts`, `SOURCE/scripts/check-ai-key-bundle.mjs`, `SOURCE/supabase/{schema.sql,verify-schema.ts,test-rls.ts}`, `SOURCE/types/result.ts`, route segments `SOURCE/app/(layer2)/exams/[id]/attempt/[attemptId]/page.tsx` and `…/result/detail/page.tsx`
- **Frontend — new**: `SOURCE/components/essay/EssayLifecycleBadge.tsx`, `SOURCE/app/(layer2)/_components/{EssayScoreLine,EssayReviewBlock,EssayRegradeControl,EssayGradingPoller}.tsx` + five co-located test files
- **Frontend — modified**: `SOURCE/components/history/{usePdfAction.ts,ActionButton.tsx,HistoryRowMenu.tsx}`, `SOURCE/app/(HM)/history/_components/HistoryRow.tsx`, `SOURCE/app/(layer2)/_components/{ResultActions,QuestionRenderer,ExamPlayer}.tsx`, `SOURCE/components/pdf/AttemptPdfTemplate.tsx`, `SOURCE/lib/i18n/dictionaries/{en,vi}.ts`, `SOURCE/app/(layer2)/exams/[id]/attempt/[attemptId]/result/page.tsx`
- **Coupled test files that move in the same commit as their production change**: `SOURCE/lib/security/checkAiKeyBundleSecrets.test.ts`, `SOURCE/lib/tutor/__tests__/telemetry.test.ts`, `SOURCE/app/(layer2)/_components/__tests__/QuestionRenderer.test.tsx`, `SOURCE/components/history/{ActionButton.test.tsx,HistoryRowMenu.test.tsx}`, `SOURCE/lib/scoring/__tests__/computeScore.test.ts`
- **Test skeletons already committed, converted by this plan**: `SOURCE/app/(layer2)/__tests__/essayGrading.int.test.ts`, `SOURCE/tests/e2e/fixture/essay-auto-scoring.fixture.e2e.test.ts`, `SOURCE/tests/e2e/service/essay-grade-write.service.e2e.test.ts`
...
- Design Doc (backend): `docs/design/essay-auto-scoring-backend-design.md` **v1.3** — EG-BE-001…036, OQ-1…OQ-6
...
Grading ships **disabled**. `ESSAY_GRADING_ENABLED` absent ⇒ off. No real student essay may reach Groq until every line below is ticked and the console check is dated.
...
- [ ] A6 — **Dated console check recorded here.** Provider default (no training, no inference-request storage) is *not* ZDR: provider docs state input/output may be logged temporarily for reliability/abuse work, retained up to 30 days. The users are minors.
  ...
  - Screenshot / evidence location: `____________________`
- [ ] A7 — Until A6 carries a real date, `ESSAY_GRADING_ENABLED` is **absent** in every environment, and Phase E is not started.
...
- [ ] B7 — `npm run verify:schema` green against **dev** and against **prod**.
...
- [ ] H2 — `LIMITS.MAX_ATTEMPT_ANSWER` 500 → 4000 **and** `SOURCE/app/(layer2)/_components/__tests__/QuestionRenderer.test.tsx:119` (`expect(textarea?.maxLength).toBe(500)`) plus the hard-coded `"CHECK length <= 500"` comment at `:116`. This site is **AC-048**-coupled and goes red the instant the constant moves.
...
- [ ] H5 — Telemetry literals move together across all seven sites: SQL inline `event_type` (`schema.sql:1383`), SQL inline `error_code` (`:1390-1399`), the `error_code` drop/add pair (`:1818-1821`), the **new** `event_type` drop/add pair, `TelemetryEventType` (`lib/tutor/telemetry.ts:40`), `TELEMETRY_ERROR_CODES` (`:35`), and the three test pins (`telemetry.test.ts:49`, `:265`, `:311`).
...
| 2 | The write path keeps every ADR-0010 containment property | `test-rls.ts` Part 10 / `tests/e2e/service/**` against real Postgres, plus `verify:schema` on **both** databases | before ship; again after every schema application |
...
| `npx vitest run` | unit/integration correctness — **primary correctness-proof mechanism** | `SOURCE/vitest.config.ts` (`lib/**`, `components/**`, `app/**`) | all new unit/component tests, `SOURCE/app/(layer2)/__tests__/essayGrading.int.test.ts` |
...
| `npm run verify:schema` | grants on both new functions; the character ceiling read back from a real DB; the schema fingerprint; the `ESSAY_MAX_ATTEMPTS` pin | `SOURCE/supabase/verify-schema.ts` | **must be extended before it is relied on** — today it asserts nothing about the ceiling |
| `npx tsx supabase/test-rls.ts` | a student JWT can call neither new function and cannot `UPDATE exam_results` | `SOURCE/supabase/test-rls.ts` Part 10, cases `EG-a…EG-e` | `public.exam_results`, both new functions |
...
| docs/design/essay-auto-scoring-backend-design.md | Điểm phát Groq | `SOURCE/lib/ai/models.ts` — `ESSAY_GRADER_MODEL = "llama-3.3-70b-versatile"` | impl-target | H4 | covered | Must live in a module **without** `server-only` (the 2026-07-17 incident); AC-032 binds a re-run of AC-070 to any change of this value |
...
| docs/design/essay-auto-scoring-backend-design.md | Integration Verification Points | `test-rls.ts` Part 10, cases `EG-a…EG-e` | verification | H6, H8 | covered | See Open Item **I-1** on placement (`test-rls.ts` vs `tests/e2e/service/**`) |
...
| docs/design/essay-auto-scoring-backend-design.md | Agreement Checklist Scope | `SOURCE/lib/supabase/service-role.ts` — two new named operations (11 → 13) | impl-target | B1.5 | covered | **TD-029 event**: these are operations 12 and 13; a 14th forces the revisit |
...
| docs/design/essay-auto-scoring-backend-design.md | Ba chỗ đọc phía server / D-15 | `export const maxDuration` on the player route segment (cannot be declared in a `"use server"` file) | impl-target | B1.5 | covered | Precedent `app/(layer4)/upload/page.tsx:18` |
...
| docs/design/essay-auto-scoring-backend-design.md | Agreement Checklist Scope | `SOURCE/app/(layer2)/essayActions.ts` (new) — `retryEssayGrading(attemptId, questionId)`, typed result, no throw, no redirect, authorise before meter | impl-target | B3.2 | covered | EG-BE-022; Gate G |
...
| UI Spec (§ Component: EssayGradingPoller) | derived-display | `ESSAY_POLL_FAST_INTERVAL_MS` **5 000**, `ESSAY_POLL_FAST_TICKS` **12**, `ESSAY_POLL_SLOW_INTERVAL_MS` **10 000**, hard caps **18 refreshes / 120 seconds** — independent of the 10-minute read-time deadline (AC-061) | F-C2 |
...
| docs/adr/ADR-0010-score-write-trust-boundary.md | Decision | dependency_direction | Privileged operations are exposed as named operations from `lib/supabase/service-role.ts`; `serviceRoleClient()` stays private; enforcement lives in SQL, not at the call site; `import "server-only"`; the bundle scan stands | B1.5, H5, H6 |
| docs/adr/ADR-0010-score-write-trust-boundary.md | Consequences (kill criterion, already fired → TD-029) | placement | Adding operations 12 and 13 to `service-role.ts` proceeds by engineer decision; a **fourteenth** operation, or a **third** in-place mutation of `exam_results`, forces the revisit | B1.5, Final Phase |
| docs/adr/ADR-0011-mastery-write-trust-boundary.md | Decision | placement | A second privileged operation is a **separate function**, not a mode parameter on the first — which is why claim and settle are two functions | H5, B1.5 |
...
| `LIMITS.MAX_ATTEMPT_ANSWER` (TypeScript) → `attempt_answers_answer_check` (Postgres) | `SOURCE/lib/ugc/limits.ts:17` and `submitExam()`'s slice at `actions.ts:146` | `public.attempt_answers.answer` CHECK | Integer ceiling, duplicated in two places (git and each database) | `verify:schema` probes behaviourally and discriminates by SQLSTATE (`23514` check violation vs `23503` foreign-key violation), because no CHECK-constraint read path exists | `npm run verify:schema` is **red** when the two ceilings differ, on both databases; code ceiling above DB ceiling means Postgres rejects an entire submission | H5, H6, H7, B3.3 |
...
graph TD
    ...
    B11["B1.1 RED: computeScore output comparison (EVP)"]
    B12["B1.2 groqClient + chokepoint scan + negative control"]
    B13["B1.3 budget.ts"]
    B14["B1.4 gradeEssays.ts (claim→budget→provider→settle)"]
    B15["B1.5 GREEN: computeScore + service-role x2 + submitExam + maxDuration"]
...
- [ ] **Task G0.1 — AC-067 Zero Data Retention gate (HUMAN ONLY)**
  ...
  - Work: complete Gate A items A1–A7 above, including the **dated console check** written into this file.
  - Verification method: the date, the engineer's name and the evidence location are physically present in Gate A6 of this document. No test in the repo can check this; that is precisely why it is a gate with an owner.
  - Dependencies: none.
  - **Blocks**: Phase E only. It does **not** block implementation — the feature is built and merged in its disabled state, which is its shipping state.
  - Completion: Implementation Complete = Gate A fully ticked with a real date; Quality Complete = N/A; Integration Complete = `ESSAY_GRADING_ENABLED` verified absent in every environment until then.
...
### Phase H: Foundation — horizontal slice (Estimated commits: 8)
...
- [ ] **Task H6 — `verify-schema.ts` and `test-rls.ts` assertions, written BEFORE anything is relied on**
  - Implementation:
    - `SOURCE/supabase/verify-schema.ts`: two grant assertions for the new functions (template: the `record_exam_result` assertions at `:373-388`), distinguishing `42501` from an incidental failure; the **character-ceiling gate** as a two-probe behavioural check discriminating by SQLSTATE (there is **no** CHECK-constraint read path — `schema_foreign_keys()` filters `contype = 'f'` at `:1233`, and adding a `schema_check_constraints()` function would be a **fourth** DDL, which is exactly what TD-005 warns against); and the `ESSAY_MAX_ATTEMPTS` pin gate that regex-extracts the cap literal from the claim function's body in `schema.sql` and compares it to the TypeScript constant, failing with a message naming both values.
    - `SOURCE/supabase/test-rls.ts` Part 10, cases `EG-a…EG-e`: `EG-a`/`EG-b` — a student JWT calling each function returns `42501`, discriminated from `PGRST202` (schema never applied) and from every other code (the revoke is missing or partial), following the `MM-b` template at `:1652-1671`; `EG-c` — `service_role` claims successfully, `essayAttempts` increments by exactly 1, and the `questionId` sequence is unchanged when grading the **second** of three essays; `EG-d` — two settles for one pair: first returns `true`, second returns `false`, the stored band equals the first and `essayState` is still `graded`; `EG-e` — claim on a non-`submitted` attempt returns `claimed = false, reason = 'not_submitted'`, and settle on that attempt raises `check_violation`. Direct `UPDATE exam_results` by a student JWT is **already** covered by the shipped `S-b` case at `:1314-1320` — do not duplicate it.
  ...
  - Proof Obligations: EG-BE-013 (student JWT refused on both functions); EG-BE-028 (`LIMITS.MAX_ATTEMPT_ANSWER` equals the DB ceiling on **both** databases, and `verify:schema` is **red** when they differ).
  - Files: `SOURCE/supabase/verify-schema.ts`, `SOURCE/supabase/test-rls.ts`
...
- [ ] **Task H7 — Phase 3.5: apply the DDL to dev and prod (HUMAN-CONFIRMED)**
  ...
  - Work, in order and without shortcuts:
    ...
    6. Run `npm run verify:schema` against dev and against prod.
  ...
  - **Blocks**: Tasks H8, B1.5, B3.1, B3.3.
  - Completion: Implementation Complete = DDL live on both projects; Quality Complete = `verify:schema` green on both; Integration Complete = both functions callable as `service_role` and refused (`42501`) with a student JWT.
...
- [ ] **Task H8 — Convert SVC-1 and SVC-2 from `it.todo` to executing tests (real Postgres)**
  ...
  - See **Open Item I-1** — the backend DD assigns these proofs to `test-rls.ts` Part 10; the skeleton sits in the runnable lane. Fill **one** home, not both.
...
- [ ] `npm run verify:schema` green against dev **and** prod, including the new ceiling gate and the `ESSAY_MAX_ATTEMPTS` pin
...
### Phase B1: Automatic Grading Path — vertical slice V1 (Estimated commits: 6)
...
- [ ] **Task B1.1 — RED: `computeScore()` output-comparison tests (EARLY VERIFICATION POINT)**
  ...
  - Completion: Implementation Complete = cases written and failing for the right reason; Quality Complete = lint/type-check pass on the test file; Integration Complete = N/A (test-file-only commit, no production code touched).
...
- [ ] **Task B1.4 — `lib/essay/gradeEssays.ts` — pass orchestration in the mandated order**
  - Implementation: create `SOURCE/lib/essay/gradeEssays.ts` with `import "server-only"`, exporting `gradeEssaysForAttempt(...)`. Per essay question with ground truth, in **this order and no other** (Gate G, AC-072): **claim → reserve budget → call provider → settle**. Concurrency capped at `GROQ_MAX_CONCURRENCY = 4`; wall-clock capped at `ESSAY_PASS_BUDGET_MS = 240_000` (4 minutes), stopping proactively **before** the platform's 300 s fluid-compute ceiling rather than being cut off. Every exit is swallowed and logged.
  ...
  - Dependencies: Task B1.2, Task B1.3.
...
- [ ] **Task B1.5 — GREEN: `computeScore()` options + branch split + two `service-role` operations + `submitExam()` + `maxDuration` (one commit)**
  - Implementation:
    - `SOURCE/lib/scoring/computeScore.ts`: add the optional third parameter `options` defaulting to `{ essayGrading: false }`; split the `if (!isScored(q))` early return in the `.map()` callback at `:99-101` so a gradeable essay emits the five keys via `newEssayEntry()`; extract `hasEssayGroundTruth()` and share it with `isScored()`'s `:40` expression. **`isScored()` keeps its behaviour — essay still returns `false`.** Fix the **reason** in the two comment blocks at `:17-18` and `:35`: the truth is no longer "essay is never scored" but "the band is written **outside** `computeScore`, and the row deliberately stays `scored:false`". The function stays **pure**: no I/O, no `process.env`, no async (AC-013) — the flag is **passed in**, never read inside.
    - `SOURCE/lib/supabase/service-role.ts`: add `claimEssayGradingAttempt(attemptId, questionId)` and `recordEssayGrade(attemptId, questionId, state, earned, max, lowConfidence)`, shaped after `recordSkillMastery()` (`:95-104`). Operations 11 → **13**. Add the `TD-029` note at this exact line: a **fourteenth** operation, or a **third** in-place mutation of `exam_results`, forces the revisit. `serviceRoleClient()` stays private. The six-parameter signature exceeds the 0–2 recommendation deliberately — ADR-0018 Decision 1 fixes it verbatim, and wrapping it in an object would misalign the `.rpc()` call from the SQL `p_*` parameter names, adding a mapping neither sibling in the file has.
    - `SOURCE/app/(layer2)/actions.ts`: `submitExam()` reads `ESSAY_GRADING_ENABLED` (only a trimmed `"true"` means on), passes it into `computeScore()` as an option, and registers `after(() => gradeEssaysForAttempt(...))` **before** `redirect()` at `:192`. Registering after `redirect()` means, in Next, never registering at all — grading would silently never run once the flag was turned on, and nothing in the flag-off state could reveal it. Precedent and the rule in writing: `lib/support/actions.ts:122-127`. The Supabase client already built in `submitExam()` is **captured in the closure before** `after()` is registered, so the telemetry path carries a JWT-bearing instance (R-05 — the alternative assumption is unverified and the design does not depend on it).
    ...
    - Fix the two `computeScore.ts` comment sites from the eleven-site D-09 list here; the remaining eight are Task B4.1.
  - **Hard sequencing rule**: the backend DD puts steps 7 and 9 in the **same change set**. Step 7 alone emits `pending` keys with nothing to grade them, so every essay would read "Đang chấm" and then "Chấm thất bại" ten minutes later — a screen that lies twice, which is exactly what UI-D7 exists to prevent. See **Open Item I-2** for the tension between this and single-commit granularity.
  - Proof Obligations: Task B1.1's cases turn GREEN; EG-BE-001…004; EG-BE-030 (no existing scoring output moves); EG-BE-032 (`submitExam()` emits **0** synchronous provider requests, and registration precedes `redirect()`); EG-BE-033 (any pass failure leaves `submitExam()`'s observable outcome unchanged — the `exam_results` row is still written, `record_skill_mastery()` is still called, the redirect still happens).
  - Files: `SOURCE/lib/scoring/computeScore.ts`, `SOURCE/lib/supabase/service-role.ts`, `SOURCE/app/(layer2)/actions.ts`, `SOURCE/app/(layer2)/exams/[id]/attempt/[attemptId]/page.tsx`
  - Dependencies: Task B1.1, Task B1.4, Task H7 (the SQL functions must exist on dev before the operations can be exercised).
  - Completion: Implementation Complete = all four files in one commit; Quality Complete = six verify gates green, `computeScore` suite green with zero regressions; Integration Complete = **L1** — on dev with the flag on, submitting an attempt with three essays produces bands, and `per_question` read back with SQL matches the W1 shape.
...
- [ ] Manual dev run (**L1**): an attempt with three essays submitted with the flag on produces three bands; `per_question` read back by SQL matches W1; the `questionId` sequence is unchanged
...
- [ ] **Task B2.1 — `getResult()`: `created_at` in the select, plus the three derived fields**
  ...
  - Files: `SOURCE/app/(layer2)/queries.ts`, `SOURCE/types/result.ts` (add `essay?: EssayView` to `PerQuestionResult`, following the `hasBeenWrongTwice` precedent at `:19-24`; also fix the stale `scored` comment at `:14-17` — reason only, no value change)
  - Dependencies: Task B1.5.
...
- [ ] **Task B2.2 — `listMyHistory()`: two required booleans (GATED on Gate D)**
  ...
  - Files: `SOURCE/app/(HM)/queries.ts`
...
- [ ] **Task B2.3 — `AttemptPdfData` gains `hasIncompleteEssay` and both construction sites fill it**
  ...
  - Files: `SOURCE/lib/pdf/generateAttemptPdf.ts`, `SOURCE/app/(layer2)/exams/[id]/attempt/[attemptId]/result/page.tsx`, `SOURCE/app/(HM)/history/_components/HistoryRow.tsx`
...
- [ ] **Task B3.1 — Telemetry: one event type, three error codes, all seven coupled sites**
  ...
  - Dependencies: Task H7 (the widened CHECKs must be live, or every grading telemetry write fails silently).
...
- [ ] **Task B3.2 — `app/(layer2)/essayActions.ts` — the retry Server Action + `maxDuration`**
  - Implementation: create `SOURCE/app/(layer2)/essayActions.ts` with `"use server"`, exporting `retryEssayGrading(attemptId, questionId)`. Typed result, **no throw, no redirect** — the caller is an affordance inside an already-rendered page (precedent: `tutorActions.ts:8-12`, citing `rateExam()`). It lives in its own file, not in `actions.ts`, following the recorded rule at `tutorActions.ts:1-6` ("everything guarding the door sits in one file you can read in one pass").
  ...
  - Files: `SOURCE/app/(layer2)/essayActions.ts` (new), `SOURCE/app/(layer2)/exams/[id]/attempt/[attemptId]/result/detail/page.tsx`, `SOURCE/app/(layer2)/__tests__/essayActions.test.ts` (new)
  - Dependencies: Task B3.1, Task B2.1, Task B1.4.
  - Completion: Implementation Complete = action + segment config; Quality Complete = six verify gates green; Integration Complete = **L1** on dev, pressing retry on a `failed` question yields a band.

- [ ] **Task B3.3 — Tutor prompt cap + character ceiling raise + the AC-048 coupled test site (ONE commit)**
  - **Entry condition**: Task H7 complete — the widened CHECK is live on **both** databases and `npm run verify:schema` is green on both. This is the R-f condition and it is not negotiable: shipping the code ceiling above the database ceiling makes Postgres reject an **entire** submission.
  ...
  - Files: `SOURCE/lib/tutor/prompt.ts`, `SOURCE/lib/ugc/limits.ts`, `SOURCE/app/(layer2)/_components/__tests__/QuestionRenderer.test.tsx`
  - Dependencies: Task H7 (**hard**), Task B3.1.
  - Completion: Implementation Complete = three files in one commit; Quality Complete = six verify gates **plus** `npm run verify:schema` green on both databases; Integration Complete = the raised ceiling is enforced identically in code and in both databases, and Gemini's token cost has not moved.
...
- [ ] The tutor cap landed before or with the ceiling raise; `verify:schema` green on both databases afterwards
- [ ] `QuestionRenderer.test.tsx:119` moved in the same commit as the constant; `:112` untouched
- [ ] **L1**: retry on a `failed` question on dev returns a band; a refusal returns exactly one specific reason
...
- [ ] **Task B4.1 — The remaining eight D-09 sites**
  - Implementation: correct the reason text at `SOURCE/types/result.ts:14-17` (if not already done in B2.1), `SOURCE/lib/scoring/__tests__/computeScore.test.ts:4` (header) and `:131` (describe title), `SOURCE/lib/tutor/__tests__/prompt.test.ts:238` and `:251`, `SOURCE/lib/scoring/__tests__/wrongTwice.test.ts:112` and `:132`, `SOURCE/app/(layer2)/tutorActions.ts:269-272`, and `SOURCE/app/(layer2)/exams/[id]/attempt/[attemptId]/result/detail/page.tsx:6`. AC-051 names four sites; there are **eleven** — two landed in B1.5 (`computeScore.ts:17-18`, `:35`) and one in B3.3 (`prompt.ts:36`).
  ...
  - Files: the eight listed above.
  - Dependencies: Task B1.5.
  - Completion: Implementation Complete = eight reasons corrected, zero values or behaviours changed; Quality Complete = six verify gates green (a behaviour change here would show as a test failure); Integration Complete = N/A.
...
- [ ] **Task F-A3 — `EssayScoreLine` + insertion into `result/page.tsx` (FRONTEND EARLY VERIFICATION POINT)**
  ...
  - Files: `SOURCE/app/(layer2)/_components/EssayScoreLine.tsx` (new), `SOURCE/app/(layer2)/_components/__tests__/EssayScoreLine.test.tsx` (new), `SOURCE/app/(layer2)/exams/[id]/attempt/[attemptId]/result/page.tsx`
...
- [ ] **Task F-B1 — `EssayReviewBlock` + the essay sub-branch inside `notScored`**
  ...
  - Files: `SOURCE/app/(layer2)/_components/EssayReviewBlock.tsx` (new), `SOURCE/app/(layer2)/_components/__tests__/EssayReviewBlock.test.tsx` (new), `SOURCE/app/(layer2)/exams/[id]/attempt/[attemptId]/result/detail/page.tsx`
...
- [ ] **Task F-B2 — `usePdfAction` guard + `ActionButton` + `ResultActions` + 13 coupled test sites (one commit)**
  ...
  - Files: `SOURCE/components/history/usePdfAction.ts`, `SOURCE/components/history/ActionButton.tsx`, `SOURCE/app/(layer2)/_components/ResultActions.tsx`, `SOURCE/components/history/ActionButton.test.tsx`
  - Dependencies: Task F-A3.
...
- [ ] **Task F-B3 — `HistoryRowMenu` + `HistoryRow` badge + the PDF annotation line + 2 coupled test sites (one commit)**
  ...
  - Files: `SOURCE/components/history/HistoryRowMenu.tsx`, `SOURCE/app/(HM)/history/_components/HistoryRow.tsx`, `SOURCE/components/pdf/AttemptPdfTemplate.tsx`, `SOURCE/lib/pdf/generateAttemptPdf.ts`, `SOURCE/components/history/HistoryRowMenu.test.tsx`
...
- [ ] **Task F-C1 — `EssayRegradeControl` + RTL test**
  - Implementation: create `SOURCE/app/(layer2)/_components/EssayRegradeControl.tsx` as a `"use client"` component, rendered only on S-02, inside `EssayReviewBlock`, at RS-4/RS-5/RS-6. Seven-step handler, copied from `RecheckOrderControl`'s documented shape: (1) `if (exhausted) return;` **before** the busy latch — at RS-6 there is nothing to send: no action call, no busy phase, no outcome node; (2) `if (busyRef.current) return;` **before** any `setState` and **before** any `await`; (3) set the busy flag, **then** `setState` → `aria-busy` boolean, `aria-disabled` string, the `sr-only` reason element changes text; (4) `await retryEssayGrading(...)`; (5) store the outcome → a `role="alert"` node **appears** (inserted on outcome, not a pre-inserted region — this **is** a user-initiated action, the exact opposite of the poller's `polite` region); (6) `router.refresh()` — **never** a local state patch, because the server decides the band (first-write-wins) and a local patch would let `EssayScoreLine` above say one thing while the question card says another; (7) release the latch in `finally`.
  ...
  - Files: `SOURCE/app/(layer2)/_components/EssayRegradeControl.tsx` (new), `SOURCE/app/(layer2)/_components/__tests__/EssayRegradeControl.test.tsx` (new)
...
- [ ] **Task F-C2 — `EssayGradingPoller` + deterministic RTL test**
  - Implementation: create `SOURCE/app/(layer2)/_components/EssayGradingPoller.tsx` as a `"use client"` component, mounted on **both** result pages. This is **entirely new code** — a repo-wide grep found `(layer2)` has **0** `router.refresh()` calls, **0** `visibilityState` uses, and the only `setInterval` in the codebase at `(layer1)/_components/HomeCarousel.tsx:88`.
  ...
  - Two-phase cadence and two **independent** caps: `ESSAY_POLL_FAST_INTERVAL_MS` 5 000 × `ESSAY_POLL_FAST_TICKS` 12 (covering the first 60 seconds, the window where a result is genuinely expected), then `ESSAY_POLL_SLOW_INTERVAL_MS` 10 000; hard caps at **18 refreshes** and **120 seconds**. A tick while `document.visibilityState === "hidden"` is **skipped and costs no budget**, but the 120-second clock keeps running. On stopping, show `result.essay.pollStopped` and a real `<button>` "Cập nhật" that performs one `router.refresh()` and **reloads both budgets**. These four constants are **not** the read-time deadline and are not derived from it (AC-061); the deadline is anchored to the platform's duration ceiling, not to a latency estimate. They are owned by **O-6/OQ-1** until measured.
  ...
  - Proof Obligations — six poller cases: **P-1** `pendingCount = 1`, advance 5 000 ms ⇒ `refresh` called exactly 1×. **P-2** advance 12 × 5 000 then one more ⇒ the 13th happens after **10 000** ms, not 5 000 (phase change). **P-3** advance until `refresh` has been called **18** times, then further ⇒ no 19th, and the "Cập nhật" button is found by role. **P-4** hidden, advance 10 × 5 000, visible, advance 5 000 ⇒ `refresh` called exactly **1×** (hidden ticks spend no budget). **P-5** hidden, advance 130 000 ⇒ **0** refreshes **and** the stopped block appears — the only case that catches an inverted check order, where a permanently hidden tab loops forever because it never spends the count budget so the count cap is never reached. **P-6** `pendingCount = 0` from the start ⇒ **0** timers scheduled (advance 200 000 ⇒ 0 refreshes) and the `aria-live` region is still present.
  ...
  - Files: `SOURCE/app/(layer2)/_components/EssayGradingPoller.tsx` (new), `SOURCE/app/(layer2)/_components/__tests__/EssayGradingPoller.test.tsx` (new), plus mount points in `result/page.tsx` and `result/detail/page.tsx`
  - Dependencies: Task F-A3.
  - Completion: Implementation Complete = component + mounts + nine cases; Quality Complete = six verify gates green, zero `waitFor` in the poller describe; Integration Complete = **L1** on dev, a page with one pending essay updates itself within ≤ 10 seconds of the band landing.
...
- [ ] **Task F-C3 — Convert FE2E-1 and FE2E-3**
  ...
  - Completion: Implementation Complete = FE2E-1 and FE2E-3 executing; Quality Complete = `npm run test:fixture` shows **only** the two recorded TD-030 failures; Integration Complete = the shipped state and the two-door guard are automated claims.
...
- [ ] **Task F-C4 — Convert FE2E-2 (reserved-slot journey)**
  ...
  - Determinism: fake timers only, no `waitFor` in this describe, all clock movement through `vi.advanceTimersByTime` inside `act()`.
  - Files: `SOURCE/tests/e2e/fixture/essay-auto-scoring.fixture.e2e.test.ts`
  - Dependencies: Task F-C2, Task F-B2.
...
- [ ] **Task F-D1 — Flag-selected footnote key + prop chain + segment read**
  ...
  - Files: `SOURCE/app/(layer2)/exams/[id]/attempt/[attemptId]/page.tsx`, `SOURCE/app/(layer2)/_components/ExamPlayer.tsx`, `SOURCE/app/(layer2)/_components/QuestionRenderer.tsx`, `SOURCE/app/(layer2)/_components/__tests__/QuestionRenderer.test.tsx`
...
- [ ] **Task E2 — OQ-4: decide the operational value of `GROQ_BUDGET_DAILY_LIMIT`**
  - Owner: **engineer**, before enabling the flag. The design fixed the **variable name** and the fail-closed behaviour; it did **not** fix the number.
  - Input needed: the desired daily service level. Arithmetic to start from: the published free-tier limit for `llama-3.3-70b-versatile` is 1 000 requests/day, and each question reserves 3, so a limit of 300 ≈ 100 questions/day at worst case.
  - Escalation condition: if the arithmetic yields a number **below the essay count of one full exam** (50), a single attempt cannot be fully graded in one day — and that must be known in advance rather than discovered afterwards.
  - Record the chosen value and the reasoning here: `____________________`
...
- [ ] **Task E3 — OQ-6 + AC-070: first real-provider evaluation of `ESSAY_GRADER_MODEL`**
  ...
  - `llama-3.3-70b-versatile` was chosen for multilingual capability and because it has the most specifically published free-tier limits — **not** on any assessment of grading quality. **Nobody has graded a Vietnamese essay with it.** It is a swappable starting point.
...
- [ ] **Task E5 — OQ-1 / O-6: measure the round trip and confirm or adjust four time constants**
  ...
  - Measure: 10 real gradings, recording p50/p95 from request emission to response receipt. The Singapore→Groq round trip is **unmeasured** (C4); `GROQ_CALL_DEADLINE_MS` (20 s), `GROQ_MAX_CONCURRENCY` (4), `ESSAY_PASS_BUDGET_MS` (4 min) and `ESSAY_PENDING_DEADLINE_MS` (10 min) were all chosen by argument.
  ...
  - Also revisit the four polling constants (O-6) if typical grading latency far exceeds 60 s — and note again that the polling bound and the read-time deadline are **different numbers** and neither is derived from the other.
...
- [ ] **Run the two additional gates**: `npm run check:bundle`; `npm run verify:schema` against **both** databases.
...
- [ ] **Document updates**:
  ...
  - [ ] `docs/design/essay-auto-scoring-frontend-design.md` — mark FE-OQ-3, FE-OQ-4, FE-OQ-5 resolved; correct its header, which cites the UI Spec as **v1.2** and the backend Design Doc as **v1.2** while the current versions are **v1.3**.
...
- [ ] **Delete this work plan only after explicit user approval** (work plans are excluded by `.gitignore`; the completion record lives in the project's own progress store).
...
| **O-6** | UI Spec | The four polling constants (5 s × 12, 10 s × 6, caps of 18 refreshes and 120 s) come from the PRD's latency **target**, not from measurement | engineer | **Phase E, Task E5**, together with OQ-1 | If typical grading latency far exceeds 60 s, the two-phase cadence should shift — but the read-time deadline remains a **different number** and does not shift with it (AC-061) |
...
- **I-1 — Where do the two SQL proofs live?** The backend Design Doc assigns them to `SOURCE/supabase/test-rls.ts` § Part 10, cases `EG-a…EG-e`, and never mentions `SOURCE/tests/e2e/service/**`, `vitest.localdb.config.ts` or `npm run test:localdb` — although that lane exists, is committed, and its own config comment describes its purpose in the same words the Design Doc uses. `test-rls.ts` has no npm script and is run by hand with `npx tsx`. The skeleton was placed in the runnable lane because that is the one a CI-adjacent command executes and a reviewer can re-run. **The obligations are identical either way**: SVC-1 = EG-c + EG-d, SVC-2 = EG-a + EG-b + EG-e. **Fill one home, not both** — two homes drift and the duplicate becomes the one nobody runs. If the Design Doc's placement wins, move the obligations into `test-rls.ts` Part 10 **verbatim** and delete the skeleton file. *Owner: engineer, before Task H8.*

- **I-2 — Must backend steps 7 and 9 literally be one commit?** The Design Doc puts them in "the same change set" because step 7 alone emits `pending` keys with nothing to grade them. But the flag defaults **off**, and with it off step 7 emits no keys at all — so the stated hazard cannot occur in the shipping state. Task B1.5 follows the Design Doc literally (one commit spanning `computeScore.ts`, `service-role.ts`, `actions.ts` and a route segment), which strains single-commit granularity. The alternative is two commits with a written rule that neither is deployed with the flag on until both have landed. *Owner: engineer, before Task B1.5. Either answer is defensible; guessing silently is not.*
...
- **What "done" does not mean**: merging everything in this plan leaves the feature **disabled**. Grading real student writing begins only after Phase E, and Phase E begins only after Gate A6 carries a real date. Until then, `ESSAY_GRADING_ENABLED` is absent, `computeScore()` emits no keys, `after()` is never registered, and zero Groq requests is a structural consequence rather than something anyone has to remember.
