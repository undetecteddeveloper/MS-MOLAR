...
| docs/design/essay-auto-scoring-backend-design.md | Integration Verification Points | cases `EG-a…EG-e`, in `tests/e2e/service/**` | verification | H6, H8 | covered | **I-1 closed 2026-08-29**: runnable lane wins over `test-rls.ts`; backend DD amended |
...
- [ ] **Task B1.5 — GREEN: `computeScore()` options + branch split + two `service-role` operations + `submitExam()` + `maxDuration` — SPLIT INTO TWO COMMITS (I-2, closed 2026-08-29)**
  - **Deployment rule, to be repeated in BOTH commit messages: neither commit is deployed with `ESSAY_GRADING_ENABLED` on until both have landed.** The Design Doc's one-commit hazard (step 7 emitting `pending` keys with nothing to grade them) cannot occur while the flag is off, and the flag is off until Gate A passes.
  - Implementation:
