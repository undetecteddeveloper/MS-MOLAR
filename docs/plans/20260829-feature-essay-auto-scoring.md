...
| **AC-020** | a client component under SOURCE/app/(layer2)/_components/ polls while at least one essay in the attempt is pending,… | F-C2 *(confirmed E6)* | `corrected` | `EssayGradingPoller` — mount condition is `essaySummary !== undefined`, **not** `pendingCount > 0` (defect F-05) |
...
| **AC-053** | on the result-detail page (SOURCE/app/(layer2)/exams/[id]/attempt/[attemptId]/result/detail/page.tsx, label rendered… | F-B1 *(confirmed E6)* | `corrected` | The `notAutoScored` label is suppressed by **lifecycle state**, inside the existing `notScored` branch at `result/detail/page.tsx:75` |
