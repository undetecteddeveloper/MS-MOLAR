...
#### Tasks
- [x] **P6-T1**: Edit `SOURCE/app/(exams)/exams/[id]/page.tsx:30-32,127` — accept `searchParams`, read `from`, pass `source={from}` to `StartAttemptButton`. Edit `SOURCE/features/exams/components/StartAttemptButton.tsx:17-18` — add `source?: string` prop; `startAttempt.bind(null, examId, source)` replaces `bind(null, examId)`. Target: SOURCE/app/(exams)/exams/[id]/page.tsx, SOURCE/features/exams/components/StartAttemptButton.tsx. AC: AC-039, AC-040, AC-041. Design ref: backend DD § The attempt-source write path (chain); frontend DD § Field Propagation Map. Connection Map: consumer half of "`?from=` querystring" row, producer half of "StartAttemptButton→startAttempt" row. Proof Obligation: `?from=` is read as `searchParams: Promise<{from?:string}>`, untyped, no client-side validation — normalisation happens only inside `toAttemptSource` (Phase 1) at the server-action boundary. Verify: gates 1-4; manual smoke — start an attempt from a shelf card, read the `exam_attempts.source` column back on dev. **Code + automated roundtrip tests done, gates 1-4 green; manual dev smoke test DONE 2026-09-19** (engineer signed the shared Playwright session in): started an attempt from `/exams/exam-toan-10?from=hot` -> `exam_attempts.source='hot'` on dev; started one from `/exams/exam-hoa-10` (no `from`) -> `source='none'`. See task file Investigation Notes.

#### Phase Completion Criteria (L1)
- [ ] Starting an attempt from a `?from=hot` card writes `source='hot'` on dev; a flat-grid/home-block start writes `source='none'` — **pending manual dev smoke test** (see P6-T1 task file)
- [x] Gates 1-4 green — `tsc`/`eslint`/`build`/`check:bundle` clean; `vitest run` green for this phase's files (1 pre-existing, unrelated failure in `lib/security/rateLimit.test.ts`, same one noted at Phase 5)
...
#### Phase Completion Criteria (L1)
- [x] Home right column shows "Đề nổi nhất" + top-3 hot exams for a signed-in visitor with data — proven by the real-composition automated test AND confirmed by a real signed-in browser render on dev 2026-09-19: region "Đề nổi nhất" with exactly 3 exams (ly-10, hoa-10, KIỂM TRA CUỐI HỌC KÌ 2), same order as the top 3 of the `/exams` hot shelf; old "Đề mới đăng" block absent
...
- [x] **P8-T2**: Extend `SOURCE/supabase/test-rls.ts` with Phần 10 (cases HS-a through HS-g) per backend DD § Test Boundaries and Placement. Target: SOURCE/supabase/test-rls.ts. AC: AC-027 (all sub-cases). Design ref: backend DD "test-rls.ts Phần 10" (verbatim case list). Proof Obligation (verbatim): HS-a positive control (A sees 0 of B's `exam_attempts` rows post-DDL); HS-b cross-user proof (`total_count>=1` for an exam only B submitted); HS-c leak proof (returned row's key set is EXACTLY the 4 declared columns); HS-d unpublished-exam exclusion; HS-e anon→42501, authenticated→array; HS-f banned-author exclusion + reappearance after unban; HS-g `source='hacked'`→23514, writing on B's `user_id`→refused by `attempts_insert_own`. Verify: `npx tsx supabase/test-rls.ts` run manually against dev, all 7 cases pass. **Done**: all 7 cases pass live on dev (verified). Full-suite process exit code is non-zero due to a pre-existing, unrelated Phần 2 Rating-fixture defect (stale `INITIAL_RATING_SCORES`/`UPDATED_RATING_SCORES` outside the `[1,5]` range after an earlier, unrelated rating-scale migration) — root-caused and documented in the task file's Investigation Notes, not caused by this task (proven via `git diff` isolation + sequential execution order), out of this task's scope to fix.

- [x] **P8-T3**: Create `SOURCE/tests/e2e/service/examHotCountsFixtures.ts` (companion fixtures module — does not exist yet), following the exact shape of `examSearchFixtures.ts`: re-export `HAS_LIVE_DB`/`adminClient`/`anonClient` from `essayGradeWriteFixtures.ts`; a `SLOT` prefix convention; `setUp(admin, slot)` seeding 2 real users (A, B) plus published/unpublished/banned-author exam rows and `exam_attempts` rows straddling the hour-snapped window boundary; `tearDown(admin, fixture, slot)` idempotent and prefix-scoped; `serviceClient()`/`anonClient()` for grant-boundary probes. Target: SOURCE/tests/e2e/service/examHotCountsFixtures.ts (new). AC: supports AC-018, AC-025, AC-027, AC-028. Design ref: backend DD § Test Boundaries and Placement row for this file; the skeleton's own "FIXTURES MODULE" comment. Verify: gates 1-3 (module compiles); consumed by P8-T4. **Done**: gates 1-2, 4 green (build passes; `test:localdb`/gate 3+6 deferred to P8-T4, which exercises this module). Idempotency proof and a functional sanity run of the real `exam_hot_counts()` RPC against the seeded fixture both passed live on dev (ref `hynwleaxtbtjzkvpjsug`), dev confirmed clean of residual rows afterward — see task file Investigation Notes.

- [x] **P8-T4**: Fill in `SOURCE/tests/e2e/service/exam-hot-counts.service.e2e.test.ts` (already committed as `it.todo`, 6 obligations) — wrap the top-level `describe` in `describe.skipIf(!HAS_LIVE_DB)` (importing `HAS_LIVE_DB` from P8-T3), replace each `it.todo` with a real `it`. Target: SOURCE/tests/e2e/service/exam-hot-counts.service.e2e.test.ts. AC: AC-018, AC-025, AC-027, AC-028. Design ref: backend DD § Test Boundaries and Placement (service e2e row); skeleton's own Proof Obligation list. Proof Obligation (from skeleton, verbatim, 6 obligations): (a) HS-b cross-user proof via real JWT; (b) HS-c key-set-exact proof; (c) `in_progress`/unpublished/banned-author exclusion + banned-author reappearance after unban; (d) hour-snapped window boundary — attempt 1s before `date_trunc('hour', p_since_recent)` excluded, 1s after included; (e) `p_max_rows` clamp asserted against IMPORTED `LIST_ROW_CEILING`/`POSTGREST_MAX_ROWS`, not hand-copied literals; (f) anon→42501, service_role→array. Verify: `npm run test:localdb` green (gate 6, exercising this feature's real-Postgres surface for the first time). **Done**: all 6 obligations pass live on dev (ref `hynwleaxtbtjzkvpjsug`) — 5.78s for this file, 25/25 across all 4 service e2e files for the full `test:localdb` lane. Obligation (e) seeds `LIST_ROW_CEILING + 20` extra candidate exams directly in the test (fixtures module untouched, Target Files scope) under the same `HC_PREFIX` slot prefix so P8-T3's existing `tearDown` auto-cleans them; observed clamp = `Math.min(LIST_ROW_CEILING + 1, POSTGREST_MAX_ROWS)` = 501, computed from the imported constants (`@/lib/supabase/boundedRead`, not `lib/adaptive/constants.ts` as the task file's Investigation Targets had misstated — corrected in that task file's Investigation Notes). Dev confirmed clean of residual `hc-svc-hc1-*` rows post-run. Gates 1-2, 4 (tsc/eslint/build), 6 (test:localdb) all green.

#### Phase Completion Criteria (L2 on live dev)
- [x] `test-rls.ts` Phần 10 all 7 cases (HS-a..HS-g) pass on dev
- [x] `verify:schema` green, including the 2 new `exam_hot_counts` probes
- [x] `test:localdb` green, including all 6 obligations of `exam-hot-counts.service.e2e.test.ts`
- [x] Gates 1-6 all green for the first time with every feature test file present
...
#### Tasks
- [x] **FQA-T1**: Run the full Design-to-Plan Traceability table coverage check — confirm every row's covering task(s) landed; confirm 0 unjustified `gap` rows remain. **Done**: all 66 rows confirmed `covered` by a landed task with its specific DD artifact verified present in the committed code (spot-checked via `git show`/`grep`/file-existence beyond trusting each task file's own narrative); 0 unjustified gaps. Two non-table-row findings recorded for visibility (see task file Investigation Notes): (1) P4-T1's task file and this plan's own Phase 4 Completion Criteria sub-checklist have stale unchecked boxes even though the commit (`930e350`) landed correctly as one commit — documentation hygiene only, no functional gap; (2) P6-T1/P7-T1's manual dev smoke tests are still pending (already tracked here as `[🔄]` at Phase 6/7 Completion Criteria, not hidden) — this blocks genuine completion of FQA-T10, which depends on real written data from those smoke tests; needs an engineer to sign in to the shared Playwright CLI session as the test account and run both before FQA-T10 runs.
- [x] **FQA-T2**: Run all 6 verify gates from `SOURCE/`, in order, real exit codes. Confirm the only red case (if any) is the pre-existing `lib/security/rateLimit.test.ts` baseline (`33 <= 20`) — confirm its failure count is unchanged from the Phase 0 baseline (still exactly 1, same assertion). Anything beyond that is a real regression and blocks sign-off. **AC-045**: run `git diff main -- SOURCE/package.json` (or the equivalent against the branch's base) and confirm 0 added dependencies (`dependencies` and `devDependencies` both byte-identical to before this feature); confirm the three new shelf icons (`Target`, `Flame`, `Compass`) are imported from `lucide-react` in `ExamShelf.tsx` and nowhere introduce a second icon package. **Done**: gates 1/2/4/5/6 exit 0 (`tsc`, `eslint`, `build`, `test:fixture` 10/10, `test:localdb` 25/25); gate 3 (`vitest run`) exits 1 with exactly 1 failing case — `lib/security/rateLimit.test.ts`, same case name and same `33 <= 20` assertion as the documented Phase 0 baseline, no new regressions. `git diff main -- SOURCE/package.json` empty (0 added deps); `package-lock.json` diff also empty. `ExamShelf.tsx:2` imports `Target`/`Flame`/`Compass` from `lucide-react` in one statement; repo-wide grep found 0 second icon package imports. See task file Investigation Notes for full gate table.
- [x] **FQA-T3**: Confirm Success Criteria #5 — `rating.int.test.ts:317-440` passes unmodified (diff check against the pre-feature baseline). **Done**: baseline = `f3de797e6846cf6bfdc46f8ad036b7828e29fea8` (parent of `5267fd9`, this feature's first commit). `git diff --stat` against that baseline for `rating.int.test.ts` shows 195 insertions, 0 deletions/modifications; `git diff -U0` shows a single hunk, pure append after original line 746 (`@@ -746,0 +747,195 @@`) — confirmed by a direct `git diff --no-index` between the baseline blob and the current working-tree file, showing lines 1-746 (which fully contains `:317-440`/`:319-457`) byte-identical. `npx vitest run features/exams/__tests__/rating.int.test.ts` — 31/31 tests passed. See task file `docs/plans/tasks/exam-shelves-task-FQA-T3.md` Investigation Notes for the full command/output record.
- [x] **FQA-T4**: Security review — confirm ADR-0021's Security Considerations are implemented: `exam_attempts` RLS unweakened (0 policy changes); RPC granted to `authenticated`+`service_role` only; `?from=` normalised before reaching SQL; window boundaries hour-snapped server-side. Done: security-reviewer sign-off `approved_with_notes`, 0 required fixes; all 4 properties confirmed with proof pointers; 2 non-blocking notes (RPC rate-limit hardening, documented k-anonymity trade-off) — see task file.
- [x] **FQA-T5 (manual, not automatable)** — **DONE 2026-09-19**: 16/16 data points, CLS = 0 everywhere (`/exams` cold open, shelf horizontal scroll, `/exams?sort=hot`, `/` signed-in; at 360/768/1024/1280). Probe validated with a positive control (injected 200px block -> CLS 0.1563). Superseded deferral text follows: Playwright CLI interaction audit (`npm run pw`, run from inside `SOURCE/`) — measure CLS at 360/768/1024/1280 on `/exams` (0 params) cold open AND on a horizontal shelf swipe, plus `/exams?sort=hot` and `/` signed-in. Target: CLS = 0 at all four widths, both triggers (Success Criteria #3). **DEFERRED to engineer** (2026-09-19): 0/16 data points measured — all 4 scenarios need an authenticated session (`/exams` is auth-gated); no test-account password is available to this session and auto mode denies fresh sign-in. Same blocker class as P6-T1/P7-T1's deferred smoke tests, per the engineer's standing decision to run these themselves. Everything else validated as workable (npm run pw runs, resize + CLS measurement technique confirmed) — only the auth step blocks it.
- [x] **FQA-T6 (manual, not automatable)** — **DONE 2026-09-19 except live TalkBack** (needs a physical device, left to the engineer): measured on the rendered dev page at 1280 and 360 — Tab order = DOM order (Cần luyện -> Nổi nhất), every card link reachable, visible focus ring on each, ribbon `aria-hidden` and not focusable, card link names exclude ribbon text; contrast: ribbon 11.04:1 (`rgb(20,41,28)` on `rgb(255,214,92)`), card meta 8.98:1, shelf subtitles 10.13:1. Superseded deferral text follows: Accessibility pass — keyboard + TalkBack at 360 and 1280: every card reachable in DOM order, shelf headings announced, ribbon absent from the accessible name; confirm the already-measured contrast values hold in shipped markup (ribbon 11.0:1, subtitle 9.0:1). Target: 0 defects (Success Criteria #8). **PARTIALLY DEFERRED to engineer** (2026-09-19): same auth blocker as FQA-T5, re-verified this session — `/exams` redirects to `/?auth=signin`, no test-account password available. The live keyboard/TalkBack/contrast pass (0/3 checks) could not run. However, the DOM/ARIA-structure sub-claims (tab order, heading structure, ribbon-exclusion mechanism) ARE substantiated via 4 existing fixture-e2e/component tests (`ExamRibbon.test.tsx`, `ExamShelf.test.tsx`, `ExamCard.snapshot.test.tsx`, `exam-shelves.fixture.e2e.test.ts` — 39 tests, all passing, all re-run this session) — see task file Investigation Notes. Contrast has 0 live substitute. Not a clean defer like FQA-T5: part of the claim is proven, part remains blocked.
- [x] **FQA-T7 (manual, not automatable, LAST GATE)** — **Done** (2026-09-19, engineer-confirmed, applied via Composio `SUPABASE_BETA_RUN_SQL_QUERY` on `MS-MOLAR-prod` `pebjdlbgbmizgfpuptjl`): all 7 statements applied one per call, each verified at catalogue level; prod `schema_version.fingerprint` = `340bab74ca57` = dev's. See task file Investigation Notes. Original scope: Production schema fingerprint comparison — at deploy time, apply the migration's 7 statements to prod ONE STATEMENT PER CALL over the MCP/Composio SQL path (backend DD § Migration Procedure step 9), gated on the engineer's EXPLICIT confirmation before any statement touching `exam_attempts` is sent. After each statement, verify at catalogue level (`pg_proc.proacl`, `pg_constraint`, `pg_indexes`, `information_schema.columns` — never `information_schema.routine_privileges`, which reads empty under the read-only prod user and looks exactly like a missing grant). Read prod's `schema_version.fingerprint` and confirm it equals dev's (Success Criteria #7). This task is explicitly OUT OF SCOPE for implementation — it happens at deploy time, separately, and is the last thing before the feature is called done.
- [x] **FQA-T8**: Coverage check — confirm new/changed files meet the project's coverage configuration (diagnostic signal, not a target). **Done** (2026-09-19): no coverage tool is configured/installed in this repo (`vitest.config.ts` has no `test.coverage` block; `@vitest/coverage-v8`/`-istanbul` absent from `node_modules` and `package.json` — installing one would add a dependency, conflicting with this same feature's AC-045). Substituted a manual structural review against the plan's Design-to-Plan Traceability table plus the 3 critical-path categories the task's own Success Criteria name: ladder boundary conditions (`examShelves.test.ts` — exact 4/5/6 `HOT_SHELF_MIN_CARDS` boundary, `SHELF_MAX_CARDS` cut, rung order, tie-breaks), normalisation functions (`attemptSource.test.ts`, `browseParams.test.ts` — exhaustive valid/invalid/edge-case inputs), and composition budget checks (`shelves.int.test.ts` 4-call budget + deferred-gate ordering + RPC arg shape, `rating.int.test.ts`'s `?sort=hot`/`?sort=garbage` 3rd candidate, `hotCounts.test.ts`). `npx vitest run`: 2207 passed / 1 pre-existing unrelated failure (documented baseline, unchanged) / 10 skipped (all in an unrelated feature's manual-gated file, 0 in exam-shelves). `npm run test:fixture`: 10/10 passed. No genuine gap found; no test added (none needed). See task file `docs/plans/tasks/exam-shelves-task-FQA-T8.md` Investigation Notes for full detail.
- [x] **FQA-T9**: Documentation — confirm `docs/project-context/external-resources.md` still reflects this feature's entries; confirm PRD Undetermined Items U1-U4 remain closed as recorded in PRD v1.2 (no action expected, confirm only). **Done** (2026-09-19): cross-checked all 10 project-tier label citations across both Design Docs' § External Resources Used tables against `external-resources.md` — all present, none stale/missing (0 edits made; the one apparent mismatch, frontend DD citing "API Schema Source" while the project-tier entry says `not applicable`, is consistent — the project-tier "not applicable" reflects the code-first API surface, and the DD's citation points at that same code-first surface, `shelves.ts`). Confirmed the file's "drift-only correction" note matches the UI Spec's Revision History record that TBD-03 (stale `external-resources.md`) was fixed in a separate pass, per this plan's own Notes section. U1-U4 confirmed still closed via backend DD's Agreement Checklist row (`exam-shelves-backend-design.md:44`) and verified against shipped code (`HOT_SHELF_MIN_CARDS=5`, three site-wide rungs, AC-019–023 subtitle copy) — no new information from FQA-T1–T8 bears on any of the four. See task file `docs/plans/tasks/exam-shelves-task-FQA-T9.md` Investigation Notes.
- [x] **FQA-T10**: Confirm AC-042 — run the backend Design Doc's metric SQL query (§ The SQL objects, "Metric query") against the shipped `exam_attempts.source` column on dev; confirm it groups by `source` and returns a `share_pct` per value in one `select`, with 0 application-log reading required. **Done** (2026-09-19): ran the query verbatim against dev (`hynwleaxtbtjzkvpjsug`) — 0 errors, 0 rows (every existing attempt predates the `2026-09-18` ship-date filter; P6-T1/P7-T1 smoke tests that would write post-ship rows are still deferred to the engineer, as already tracked at FQA-T1/FQA-T5/FQA-T6). Supplementary run of the same query shape with a widened date bound confirmed the grouping + `share_pct` arithmetic against dev's real 185 pre-ship rows (all `source = 'none'`, the column default): 1 row, `share_pct = 100.0`, computed correctly in one `select`. Column read-back (`information_schema.columns`) confirms `source` is `not null default 'none'::text` on dev, matching the DD exactly. AC-042 confirmed structurally correct and self-sufficient against the DB alone. See task file `docs/plans/tasks/exam-shelves-task-FQA-T10.md` Investigation Notes.
...
- [x] 6 verify gates green by real exit code; the pre-existing `rateLimit.test.ts` baseline failure unchanged
