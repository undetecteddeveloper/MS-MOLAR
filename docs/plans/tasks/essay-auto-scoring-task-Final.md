...
### 16. Document updates — done 2026-08-30

**All five documents updated. Two of them were wrong in ways the checklist itself did not predict, and one of those errors had already propagated.**

- [x] **`TECH-DEBT.md`** (at the **repo root**, not `docs/`) — **TD-029 confirmed accurate**: `grep -c "^export async function"` on `service-role.ts` returns **13**, exactly the number the entry predicted, so operations 12 and 13 landed and no fourteenth slipped in alongside them; both revisit triggers still state correctly (a **14th** operation; a **3rd** in-place `exam_results` mutation, ADR-0018 having spent the first two on claim and settle). **TD-005 updated** — it did **not** reflect this feature's DDL at all, and the "**two** hand-applied schema changes" figure carried by the upstream documents **undercounts by one**. The three groups are now named: the two SQL functions; the `telemetry_log` CHECK pair (`event_type` 2→3, `error_code` 6→9); and the **character-ceiling change** on `attempt_answers_answer_check` (500→4000, R11), which is the one the count forgot. **TD-030 untouched and still open** — confirmed by the diff being additions only.
- [x] **Backend Design Doc** — OQ-1…OQ-7 closed out with their outcomes: **six closed, one open** (OQ-5, `upload.essayStored`, engineer/product). **And a warning added that matters more than the statuses:** Task E5's token measurement **falsifies the recorded reasoning** of OQ-4 and OQ-7 while leaving both values correct. OQ-4 argues TPD binds before RPD — true at the estimated ~3K tokens/request, **false** at the measured **873** (2M ÷ 873 ≈ 2 290/day, so **RPD 1 000 binds first**). OQ-7's arithmetic says 5 essays ≈ 15K tokens ⇒ ~1.9 minutes; at 873 it is ≈ 4 365 tokens ⇒ **~33 seconds**. A value that is right for a reason that is wrong is exactly what a later session moves in the wrong direction, which is why this is recorded rather than left implicit.
- [x] **Frontend Design Doc** — **FE-OQ-3 closed** on the Gate D measurement (375 → 3 401 B/row, ≈9.1×, ~1 661 KB at the 500-row ceiling; engineer ACCEPT; the RPC path **not** taken, so **Escalation 2 stays closed** and the hand-applied schema count is not raised from two to three). **FE-OQ-4 and FE-OQ-5 left open with their owner** — both need a real browser, and jsdom cannot answer either; the RTL case proves only the necessary condition.
- [x] **UI Spec** — **O-3 closed** with the payload measurement, **O-6 closed** with the latency measurement (p50 **671 ms** / p95 **1 289 ms**, n = 18; **no constant moves**, because p95 sits **15×** under `GROQ_CALL_DEADLINE_MS` and the two caps are anchored to `ESSAY_PASS_BUDGET_MS`, which did not move). **O-4 left open with its owner**, confirmed deliberate rather than overlooked — and non-blocking in a checkable way: `EssayLifecycleBadge.test.tsx` pins that the file carries **no hex literal** and does **not** borrow `#4F7942`, so the fence O-4 exists to hold is enforced by a test rather than by memory.
- [x] **ADR-0018** — the new fingerprint **`9979c9deea52`** recorded in Consequences, replacing the pre-feature `29931beeb950`. Not a prediction: pinned at both declaration sites and **read back by real query** from **both** projects (Gate B6), re-confirmed on prod in B3.3. The "Known unknowns" cell **still holds** after E3's first real-provider run (7/7 EQUAL, 4/4 matched); what that run **added** is a ceiling the ADR never recorded — every budget argument in it is stated per **day**, but the constraint that binds a burst is **TPM 8 000**, ≈ **9.2 requests/minute** at 873 tokens.

#### Two corrections to the checklist's own premises
