...
## Investigation Notes
_(Record here: the shuffle seeds used and the result of each; the outcome of running each case alone with `-t`; the SQLSTATE observed for the student-JWT case and how `PGRST202` was distinguished.)_
...
| 6 | `npm run test:localdb` | | **this task's primary gate** — must be green against dev |
