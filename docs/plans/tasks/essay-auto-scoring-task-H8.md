...
- `exams` has no `created_by`. **Correction, 2026-08-30:** this line originally also said "and no `status`", which is **wrong** — `schema.sql:233` defines `status text not null default 'processing'` with a CHECK. The insert succeeded without it only because of that default. The distinction matters for anyone reusing this fixture: a seeded exam lands as **`processing`**, not `published`, so it will **not** appear in any surface that lists published exams. The service lane never lists exams, so nothing here was affected — but a later fixture that needs a browsable exam must set `status: "published"` explicitly. Found while seeding for the L1 dev run, which needed exactly that.
