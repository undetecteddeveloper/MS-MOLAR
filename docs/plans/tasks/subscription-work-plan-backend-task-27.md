...
## Target Files
- [ ] `SOURCE/lib/tutor/__tests__/telemetryDistinguishability.int.test.ts` (new — placed so it runs under `npm run test:integration`, i.e. under `SOURCE/tests/integration/` if the config glob requires it; record the chosen location in Investigation Notes)
...
### 1. Red Phase
- [ ] Read all Investigation Targets; confirm gate B is green on dev before running anything
- [ ] Write the three-cause case with a fixture prefix and idempotent teardown; confirm it fails before the CHECK is widened on the target database
### 2. Green Phase
- [ ] Run against dev; confirm all three inserts are **accepted** and the `where error_code = …` query separates them
### 3. Refactor Phase
...
## Completion Criteria
- [ ] All three inserts accepted and separable by `error_code`
- [ ] The baseline caveat is recorded in the test file header: **count `success = false` overall, partition the after-population by `error_code`**
- [ ] Teardown is idempotent (the case passes twice in a row and in isolation)
