...
## Investigation Notes
_(Record here: which second-session mechanism was used; the raw RPC response; the exam id used and confirmation it was submitted only by the first student.)_

## Implementation Steps (TDD: Red-Green-Refactor)
### 1. Red Phase
- [ ] Read all Investigation Targets and record key observations
- [ ] Identify (or seed) an exam submitted only by the first student
- [ ] Obtain a second authenticated session (different JWT) via one of the two documented patterns
### 2. Green Phase
- [ ] Call `exam_hot_counts(p_since_recent, p_since_wide, p_max_rows)` as the second session
- [ ] Record the raw response rows
### 3. Refactor Phase
- [ ] Confirm the response row for the target exam has `total_count >= 1`
- [ ] Confirm `Object.keys(row)` (or SQL column list) is exactly `{exam_id, recent_count, wide_count, total_count}` — no extra column
...
## Completion Criteria
- [ ] Second-session RPC call executed against dev with real second-student credentials
- [ ] `total_count >= 1` confirmed for an exam submitted only by the first student
- [ ] Response key set confirmed exactly `{exam_id, recent_count, wide_count, total_count}`
- [ ] Investigation Notes record the raw response, not a paraphrase
- [ ] If either check fails: **task is NOT complete**, Phase 1 must not begin, and ADR-0021 must be re-opened rather than patched at a call site
