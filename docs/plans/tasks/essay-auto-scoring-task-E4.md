...
That string (`vi.ts:271`, `en.ts:334`, rendered at `app/(layer4)/_components/QuestionEditor.tsx:15`) tells the **exam author** that essays are "chưa chấm tự động". **It becomes false the moment Gate A passes.**

**Out of scope by boundary** — D6 keeps the author surface unchanged and the UI Spec's four screens exclude `(layer4)` — **but it is not allowed to stay silent.**
...
- **(c)** open a UI Spec section for `(layer4)`.
...
- [ ] *(only if option (c))* `docs/ui-spec/essay-auto-scoring-ui-spec.md` — a new `(layer4)` section
...
- `SOURCE/app/(layer4)/_components/QuestionEditor.tsx` (`:15` — where the string renders)
- `docs/ui-spec/essay-auto-scoring-ui-spec.md` (the four screens — **`(layer4)` is excluded**, which is why this is a boundary decision rather than a defect)
...
- [ ] If **(c)**: open the `(layer4)` UI Spec section and record it as follow-up work
...
- **Claim (OQ-5 / D-11)**: the exam-author surface does not silently carry a sentence this feature makes false.
  - **Primary failure mode**: the string left unexamined because `(layer4)` is out of the UI Spec's scope — so the boundary decision becomes an accidental omission rather than an accepted trade-off.
...
## Notes
- Impact scope: the `(layer4)` author screen only.
- Scope boundary — preserve unchanged unless option (b) is chosen: `SOURCE/lib/i18n/dictionaries/{en,vi}.ts`'s `upload.essayStored`; all other `(layer4)` surfaces.
