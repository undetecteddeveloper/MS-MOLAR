...
- `SOURCE/app/(layer2)/__tests__/recordSkillMastery.int.test.ts`: the repo precedent for a real-dev-database test. It lives **inside** the `app/**` CI glob and is credential-dependent; the subscription integration lane deliberately breaks with that placement (backend DD Implementation Approach Phase 4: "CI has no database"), which is the whole reason a second config exists.
