...
- [ ] `exam_difficulty_ratings` + `exams_with_difficulty` (or RPC) applied idempotently; RLS suite R-p…R-u green
- [ ] `SOURCE/lib/rating/` unit tests green, including the threshold-agreement test
...
- [ ] Security review: RLS AND-clauses (user_id + published + submitted-attempt) verified on both insert-own and update-own; `rateExam` never leaks raw DB errors; `user_id` never taken from input
