...
Related Issue/PR: —
Review Scope: planned-files scope — `SOURCE/supabase/schema.sql`, `SOURCE/supabase/test-rls.ts`, `SOURCE/features/exams/queries.ts`, `SOURCE/features/exams/actions.ts`, `SOURCE/types/exam.ts`, `SOURCE/lib/rating/**` (new), `SOURCE/components/rating/**` (new), `SOURCE/features/exams/components/{ExamCard,ExamBrowser,ExamFilters}.tsx`, `SOURCE/features/exams/components/rating/**` (new), `SOURCE/app/(exams)/exams/page.tsx`, `SOURCE/app/(exams)/exams/[id]/page.tsx`, `SOURCE/app/(exams)/exams/[id]/rate/page.tsx` (new), `SOURCE/app/(exams)/exams/[id]/attempt/[attemptId]/result/page.tsx`, `SOURCE/features/exams/__tests__/rating.int.test.ts`, `SOURCE/tests/e2e/fixture/rating.fixture.e2e.test.ts`, `SOURCE/supabase/__tests__/rating.rls.service.e2e.test.ts`.
...
- integration: `SOURCE/features/exams/__tests__/rating.int.test.ts` (3 tests: rateExam validation/upsert/error-mapping; listExams Hardest/Level query construction; RatingModalController open-condition)
...
| docs/design/rating-system-frontend-design.md | Agreement Checklist / Scope | new route `SOURCE/app/(exams)/exams/[id]/rate/page.tsx` | impl-target | Phase 2 Task 7 | covered | |
...
|---|---|---|---|---|---|---|
| `ExamFilters` (browser, client component) → `ExamsPage` (Next.js Server Component) | `SOURCE/features/exams/components/ExamFilters.tsx` | `SOURCE/app/(exams)/exams/page.tsx` | Query string `?sort=newest\|oldest\|hardest` | Page reads `searchParams.sort`; accepts only the three values, else `undefined`; passes as `ExamSort` to `listExams` | `listExams` re-queries with the matching order; Hardest visually de-selects Newest/Oldest | Phase 1 Task 5 (both sides) |
| `ExamFilters` (browser) → `ExamsPage` (server) | `SOURCE/features/exams/components/ExamFilters.tsx` | `SOURCE/app/(exams)/exams/page.tsx` | Query string `?level=easy\|medium\|hard` (lowercase slug) | Page reads `searchParams.level`; accepts only the three values, else `undefined`; passes as `ExamFilters.level` | Only in-bucket ≥3-rating exams returned | Phase 1 Task 5 (frontend side); Phase 1 Task 4 (backend `listExams` consumption) |
| `submitExam` redirect (Next.js server action) → `ResultPage` → `RatingModalController` (client) | `SOURCE/features/exams/actions.ts` (fresh-submit redirect only, line ~127) | `SOURCE/features/exams/components/rating/RatingModalController.tsx` | Query string `?rate=auto` appended once | Controller reads `searchParams.rate`; if `=== 'auto'`, opens modal once then `router.replace(pathname,{scroll:false})` strips it | Modal opens exactly once on fresh submit; refresh/back never carries the marker | Phase 3 Task 8 (both sides) |
| `RatingForm`/`submitRating` (client, browser) → `rateExam` (Next.js Server Action) | `SOURCE/features/exams/components/rating/submitRating.ts` | `SOURCE/features/exams/actions.ts` (`rateExam`) | — (captured by Expected Signal; Server Action RPC uses the shared TS function signature) | `rateExam` validates via `isValidPartScore`, upserts, returns `{error?}` | Response matches `{ error?: "ineligible"\|"invalid"\|"server" }`; form maps to copy without losing entered scores | Phase 2 Task 6 (server side), Phase 2 Task 7 (client adapter) |
...
#### Tasks
- [x] **Task 9 — Final QA gate** (split by layer per document-reviewer note I002 — `rating-system-backend-task-9.md` / `rating-system-frontend-task-9.md`): Re-run the full RLS suite (`test-rls.ts`, including R-p…R-u) as a final regression. Author and execute service-integration-e2e Test SE2 (`rating.rls.service.e2e.test.ts` — or its preferred home) against the real `exams_with_difficulty` view (or RPC): boundary buckets (3.9/4.0/6.9/7.0/1.0/10.0), tied-mean tie-break by `created_at`→`id`, Level=Hard exclusion of below-threshold/other-bucket rows, and the 2→3-rating flip (null → `{bucket,mean}` on the very next read, with no `exams` write / no trigger observed). Run the axe a11y audit on the rating form (modal + standalone), `RateButton` states, and the Level filter. Run full lint/typecheck/build/vitest (node+jsdom)/coverage. Verify every AC in both Design Docs' Acceptance Criteria sections against the implementation. Update any touched doc references (none expected beyond this plan and the two Design Docs' Update History).
  - **DONE (backend half) 2026-07-26**: full RLS suite (`test-rls.ts`, R-p…R-u incl.) re-confirmed green; SE2 authored as a standalone `tsx` script (`rating.rls.service.e2e.test.ts`) and all four proof obligations (a)-(d) pass against the live `exams_with_difficulty` view — exact boundary buckets 1.0/3.9/4.0/6.9/7.0/10.0, tied-mean + below-threshold `created_at`/id tie-break, Level=Hard exclusion, 2→3 flip with a byte-identical `exams` row before/after (no denormalized write) and static schema.sql evidence of no trigger. Coverage on `SOURCE/lib/rating/**`: 100% stmts/branches/funcs/lines. Security review complete (RLS AND-clauses, non-leaking `rateExam` errors, no client-controlled `user_id` — see task file Investigation Notes). Quality checks scoped to the rating-system change area are clean; pre-existing unrelated gaps in the untracked `app/(analytics)` and `lib/scoring` work were found and left untouched (documented in the task file, not this task's scope). The frontend half (axe a11y audit, `RateButton`/Level-filter a11y, `components/rating/**` coverage, cross-DD AC verification) remains `rating-system-frontend-task-9.md`'s scope. See `docs/plans/tasks/rating-system-backend-task-9.md` Investigation Notes for full evidence.
  - **DONE (frontend half) 2026-07-26**: no Playwright MCP available this session, so the axe audit was a static/manual verification (jsx-a11y ESLint rules from `eslint-config-next/core-web-vitals` — `aria-props`/`aria-proptypes`/`role-has-required-aria-props`/`role-supports-aria-props`/`aria-unsupported-elements`/`alt-text`, all clean on every audited file) plus a full manual ARIA/semantics + WCAG-relative-luminance contrast audit of `RatingForm`/`RatingOverview`/`PartCard`/`PartDetail`/`CircleScale`/`RatingModal`/`RatingModalController`/`RatePageShell`/`RateButton`/`DifficultyBadge`/`ExamFilters` against the actual `globals.css` design tokens. Found and fixed one genuine WCAG 1.4.3 (AA) violation: `RateButton`'s enabled-state "Rate →" text used literal copper (`--sidebar-accent` #b8863b) on `ExamCard`'s ivory `--block-bg`/`--card` background, measuring ~2.49:1 (worse with the old `hover:opacity-80`, ~3.87:1) — well under the 4.5:1 floor; copper only clears AA against the dark `--sidebar` surfaces it's used on elsewhere (PartCard/PartDetail/CircleScale, ~5.3-5.6:1). Fixed by switching to `text-brand` (5.37:1 on ivory), already this file tree's established on-light interactive-text color (`ExamCard`'s own hover state, `ExamFilters`' Clear link, `RatingModal`'s Close button) — no new design token introduced. One pre-existing, systemic near-miss noted but NOT fixed (out of this task's component-fix scope): `text-muted-foreground` (#6b655c) on ivory measures 4.45:1, just under 4.5:1 — this is the exact `ReportExam.tsx:74` precedent color, used app-wide (ExamCard's own Subject/Grade/School labels, `DifficultyBadge` card variant, `RatingModal`'s Close button base state), predates this feature, and would require a `globals.css` token change affecting the whole app, not a rating-component fix. Re-ran the full rating-scoped vitest suite after the fix (88/88 green); `tsc`/`eslint`/`prettier --check` clean on every touched/audited file (pre-existing unrelated `(analytics)`/`lib/scoring` gaps untouched, same as prior tasks). Coverage: `components/rating/CircleScale.tsx` 96.66%, `DifficultyBadge.tsx` 100%, `lib/rating/index.ts` 100% (all well above the 70% floor). Every frontend-owned AC in the frontend DD's Acceptance Criteria section walked against the implementation with concrete evidence (code reading + re-running `rating.int.test.ts` Test 3 (21/21) + `exams/page.tsx`'s searchParams parse + `DifficultyBadge`/`ExamFilters` wiring) — see `docs/plans/tasks/rating-system-frontend-task-9.md` Investigation Notes for the full per-AC evidence table. No Design Doc/UI Spec updates required (no contract/interface changed; the RateButton fix is a color-literal implementation detail).
...
- [x] Security review: RLS AND-clauses (user_id + published + submitted-attempt) verified on both insert-own and update-own; `rateExam` never leaks raw DB errors; `user_id` never taken from input — **backend half DONE 2026-07-26** (see Task 9 backend note above); frontend-side review (a11y/AT-exposure correctness of `RateButton`/`ExamFilters`/`RatingModal`) **DONE 2026-07-26** (see Task 9 frontend note above)
- [x] Quality checks (types, lint, format) — zero errors on every rating-system-owned file (pre-existing unrelated `(analytics)`/`lib/scoring` gaps documented and untouched, out of this feature's scope)
- [x] Execute all tests (unit, integration, fixture-e2e, service-integration-e2e) — unit/integration vitest green (rating-scoped 88/88); fixture-e2e FE1/FE2 and service-integration-e2e SE1/SE2 verified by code-level trace / driver scripts (no Playwright MCP / live-browser session available across Tasks 5-9, consistent deferral documented in each task)
- [x] Coverage 70%+ on `SOURCE/lib/rating/**` and `SOURCE/components/rating/**` — confirmed 100%/96.66%/100% respectively (see frontend Task 9 note above)
- [x] Document updates: none required beyond this plan (Design Docs already reflect the shipped contracts)
...
- Start: 2026-07-26
- Complete: 2026-07-26
- Notes: Task 9 split backend/frontend per document-reviewer note I002; both halves DONE 2026-07-26 (see Task 9 sub-bullets above).
