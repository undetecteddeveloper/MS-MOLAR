...
Related Issue/PR: —
Review Scope: planned-files scope — `SOURCE/features/history/queries.ts` (new), `SOURCE/app/(history)/layout.tsx` (new), `SOURCE/app/(history)/history/page.tsx` (new), `SOURCE/features/history/components/{HistoryList,HistoryRow}.tsx` (new), `SOURCE/app/(history)/history/{loading,error}.tsx` (new), `SOURCE/features/history/__tests__/history.int.test.ts`, `SOURCE/features/exams/queries.ts` (extend `getResult()`), `SOURCE/features/exams/__tests__/getResult.int.test.ts`, `SOURCE/components/history/ActionButton.tsx` (+`.test.tsx`), `SOURCE/lib/pdf/generateAttemptPdf.ts` (+`.test.ts`), `SOURCE/components/pdf/AttemptPdfTemplate.tsx` (+`.test.tsx`), `SOURCE/lib/history/format.ts` (+`.test.ts`), `SOURCE/features/exams/components/ResultActions.tsx, SOURCE/features/exams/components/ScoreCard.tsx, SOURCE/components/layout/SiteHeader.tsx`, `SOURCE/features/auth/components/HomeSidebar.tsx`, `SOURCE/app/(exams)/exams/[id]/attempt/[attemptId]/result/page.tsx`, `SOURCE/package.json`, `SOURCE/supabase/test-rls.ts` (append case H-a), `SOURCE/tests/e2e/fixture/history.fixture.e2e.test.ts`.
...
- integration: `SOURCE/features/history/__tests__/history.int.test.ts` (`listMyHistory()` — filtering/ordering/omission/no-N+1/throw, obligations a-g), `SOURCE/features/exams/__tests__/getResult.int.test.ts` (additive-extension regression, obligations a-c), `SOURCE/components/history/ActionButton.test.tsx` (4-phase state machine, obligations a-g)
...
| Vitest (node), `app/**/*.test.{ts,tsx}` | Call-construction/query-shape correctness | `SOURCE/vitest.config.ts` | `SOURCE/features/history/__tests__/history.int.test.ts`, `SOURCE/features/exams/__tests__/getResult.int.test.ts` |
...
|---|---|---|---|---|---|---|
| docs/design/history-backend-design.md | Agreement Checklist / Scope | New `listMyHistory()` + `MyHistoryEntry` type (`(history)/queries.ts`) | impl-target | Phase 1 Task 1.2 | covered | |
...
| docs/design/history-frontend-design.md | Agreement Checklist / Scope | `HistoryList`/`HistoryRow` + `(history)/history/{loading,error}.tsx` | impl-target | Phase 4 Task 4.1 | covered | |
...
### Phase 1: Backend — `listMyHistory()` + `getResult()` Extension + `(history)` Guard/Layout (Estimated commits: 3)
...
- [x] **Task 1.2** — Create `SOURCE/features/history/queries.ts`: `MyHistoryEntry` type + `listMyHistory()` (3 sequential batched selects: `exam_results` → `exam_attempts` (`.in().eq("status","submitted").order("submitted_at",{ascending:false})`) → `exams` (`.in().eq("status","published")`), matching `getExam()`'s explicit-filter convention, not RLS alone). Implement `SOURCE/features/history/__tests__/history.int.test.ts` (skeleton, no import/describe blocks yet — add both with the real code, Red→Green in one commit).
...
- [x] **Task 1.3** — Create `SOURCE/app/(history)/layout.tsx` (nullable-user `SiteHeader` shell, no redirect — mirrors `(analytics)/layout.tsx`/`(authoring)/layout.tsx` structurally) and `SOURCE/app/(history)/history/page.tsx` (page-level auth guard: `getCurrentUser()` + `redirect("/?auth=signin")` before any fetch, mirroring `(authoring)/upload/page.tsx:8-10`; then calls `listMyHistory()`).
  ...
  - Proof Obligations: AC-016 — guest guard redirects to `/?auth=signin` **before** `listMyHistory()` is ever called (zero attempt rows fetched); manually verified (no existing automated test for this exact guard pattern, matching the untested `(authoring)/upload/page.tsx` precedent — noted, not a gap).
...
#### Tasks
- [x] **Task 4.1** — Create `SOURCE/features/history/components/{HistoryList,HistoryRow}.tsx` (list container + empty state, bounded-height scroll container per D3; one row per entry — title, `X/10` score, submitted date + completion time, 2 `ActionButton`s, "View details" link) and `SOURCE/app/(history)/history/{loading,error}.tsx` (skeleton mirroring `(authoring)/me/exams/loading.tsx`; `error.tsx` boundary with `reset()`-wired Retry, per D7).
...
- [x] **Task 4.2** — Add the `+1 import + render line` to `SOURCE/app/(history)/history/page.tsx` (frontend DD's own stated scope item): `import { HistoryList } from "./_components/HistoryList"` and render it with `listMyHistory()`'s entries, replacing Task 1.3's temporary placeholder.
...
#### Tasks
- [x] **Task 5.1** — `SOURCE/components/layout/SiteHeader.tsx:27`: `{ label: "History", href: "#" }` → `{ label: "History", href: "/history" }`. No other change — `isActive` (`:63-65`) already handles any real `href` (Assumed Behavior #2 / code:F3).
...
- [x] **Task 5.2** — `SOURCE/features/auth/components/HomeSidebar.tsx:22`: `{ label: "History", href: "#" }` → `{ label: "History", href: "/history" }`. No `activeLabel` logic change (D5 — `HomeSidebar` never renders while a user is "on" `/history`, confirmed via Assumed Behavior #1/code:F4).
  - Depends on: Task 4.2.
